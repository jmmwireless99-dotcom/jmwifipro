...
export function KitifiFreeClientsApi(db) {
  ...
  return {
    ...
    register({ name, last_name, dob, cp_number, purok, barangay, municipal, province, mac, routerId }) {
      ...
      const cp = String(cp_number || "").trim();
      const pk = String(purok || "").trim();
      const existing = byMac(m, rid);
      if (existing) {
        run(
          `UPDATE kitifi_free_clients SET
             ...
             cp_number=CASE WHEN cp_number IS NULL OR cp_number='' THEN ? ELSE cp_number END,
             purok=CASE WHEN purok IS NULL OR purok='' THEN ? ELSE purok END,
             barangay=CASE WHEN barangay IS NULL OR barangay='' THEN ? ELSE barangay END,
             municipal=CASE WHEN municipal IS NULL OR municipal='' THEN ? ELSE municipal END,
