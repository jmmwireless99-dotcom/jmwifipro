...
} from "./kitifi-free-claim-policy.js";
...
export function kitifiFreeMode(routerId) {
  ...
  if (kitifiFreeUsesHotspotTrial(rid)) return "hotspot-trial";
  const per = String(Settings.get("kitifi_free_mode_" + rid, "") || "").trim().toLowerCase();
  if (per === "mikrotik" || per === "kitifi" || per === "hotspot-trial") return per;
...
export function kitifiFreeSettings(routerId) {
  ...
  const perEnabled = Settings.get("kitifi_free_enabled_" + rid, "");
  return {
    enabled: perEnabled !== "" ? perEnabled === "1" : Settings.get("kitifi_free_enabled", "1") === "1",
...
export function saveKitifiFreeSettings(patch = {}) {
  ...
  if (patch.enabled != null) {
    ...
    if (patch.router_id != null) ups["kitifi_free_enabled_" + Number(patch.router_id)] = on;
    else ups.kitifi_free_enabled = on;
  }
...
export function KitifiFreeClientsApi(db) {
  ...
  return {
    ...
    ensureGuest(mac, routerId) {
      ...
      run(
        `INSERT INTO kitifi_free_clients (name, last_name, dob, cp_number, purok, barangay, municipal, province, mac, router_id)
         VALUES (?,?,?,?,?,?,?,?,?,?)`,
        kitifiFreeGuestName(m),
        "",
        "",
        "",
        "",
        String(cfg.default_barangay || "").trim(),
        String(cfg.default_municipal || "").trim(),
        String(cfg.default_province || "").trim(),
        m,
        rid
      );
      return byMac(m, rid);
    ...
    statusForMac(mac, routerId) {
      const rid = Number(routerId) || kitifiPortalRouterId();
      const cfg = kitifiFreeSettings(rid);
      const skipRegister = kitifiFreeSkipRegister(rid);
      const client = byMac(mac, rid);
      const today = claimsToday(normalizeMac(mac), rid);
      const uptime = cfg.rate_time || uptimeLabel(cfg.hours);
      const decision = kitifiFreeCanClaim({
        enabled: cfg.enabled,
        skipRegister,
        client,
        claimsToday: today,
        limitPerDay: cfg.limit_per_day,
      });
      const message = kitifiFreeStatusMessage({
        enabled: cfg.enabled,
        skipRegister,
        client,
        claimsToday: today,
      ...
      return {
        enabled: cfg.enabled,
        registered: !!client,
        skip_register: skipRegister,
        hotspot_trial: kitifiFreeUsesHotspotTrial(rid),
        can_claim: kitifiFreeUsesHotspotTrial(rid) ? false : decision.ok,
        ...
        mode: kitifiFreeUsesHotspotTrial(rid) ? "hotspot-trial" : cfg.mode,
        message,
        router_id: rid,
      };
    },
    ...
    async claimMikrotik(conn, { mac, routerId, routerName }) {
      ...
      const m = normalizeMac(mac);
      if (!m) throw new Error("Valid device MAC is required.");
      if (kitifiFreeUsesHotspotTrial(rid)) {
        throw new Error("PANISIJAN free internet is MikroTik hotspot trial. Use CLAIM FREE INTERNET on the login page.");
      }
      const skipRegister = kitifiFreeSkipRegister(rid);
      const st = this.statusForMac(m, rid);
      if (!st.enabled) throw new Error("Free internet is disabled.");
      if (!st.registered && !skipRegister) throw new Error("Register first before claiming free WiFi.");
      if (!st.can_claim) throw new Error(st.message || "Cannot claim free WiFi now.");

      const cfg = kitifiFreeSettings(rid);
      const client = skipRegister ? this.ensureGuest(m, rid) : byMac(m, rid);
      if (!client) throw new Error("Register first before claiming free WiFi.");
      ...
      if (!hit) {
        await conn.talk([
          ...
          "=limit-uptime=" + limit,
          "=mac-address=" + m,
          "=comment=JM Free WiFi " + (skipRegister ? kitifiFreeGuestName(m) : fullName(client.name, client.last_name)),
        ]);
      }
    ...
    async claim(conn, { mac, routerId, routerName }) {
      const rid = Number(routerId) || kitifiPortalRouterId();
      if (kitifiFreeUsesHotspotTrial(rid) || kitifiFreeMode(rid) === "hotspot-trial") {
        throw new Error("PANISIJAN free internet is MikroTik hotspot trial. Use CLAIM FREE INTERNET on the login page.");
      }
