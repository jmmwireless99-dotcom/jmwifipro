...
export function kitifiFreeSettings(routerId) {
  ...
  return {
    ...
    limit_per_day: Math.max(1, Number(Settings.get("kitifi_free_limit_per_day", "1")) || 1),
    router_id: rid,
...
export function saveKitifiFreeSettings(patch = {}) {
  ...
  if (patch.limit_per_day != null) ups.kitifi_free_limit_per_day = String(Math.max(1, Number(patch.limit_per_day) || 1));
  ...
  function claimsToday(mac, routerId) {
    ...
    return Number(get(
      `SELECT COUNT(*) AS n FROM kitifi_free_claims
       WHERE mac=? AND router_id=? AND date(claimed_at)=date('now')`,
      m, Number(routerId)
    )?.n || 0);
  }
  ...
  return {
    ...
    stats(routerId = null) {
      ...
      if (rid) {
        return {
          ...
          claims_today: Number(get(
            "SELECT COUNT(*) AS n FROM kitifi_free_claims WHERE router_id=? AND date(claimed_at)=date('now')",
            rid
          )?.n || 0),
        };
      }
      return {
        ...
        claims_today: Number(get("SELECT COUNT(*) AS n FROM kitifi_free_claims WHERE date(claimed_at)=date('now')")?.n || 0),
      };
    },
    ...
    statusForMac(mac, routerId) {
      ...
      else if (today >= cfg.limit_per_day) message = "Free internet not available. Already claimed today.";
