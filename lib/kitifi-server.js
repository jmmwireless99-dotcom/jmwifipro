...
/** Paid GCash voucher hotspot profile — mikrotik-direct sites ignore KiTifi plan profile (e.g. KITIFI). */
export function kitifiPaidHotspotProfile(routerId, planProfile) {
  const rid = Number(routerId);
  const per = String(Settings.get("kitifi_gen_profile_" + rid, "") || "").trim();
  const defPer = String(Settings.get("kitifi_default_profile_" + rid, "") || "").trim();
  if (kitifiMikrotikOnlyRouter(rid)) {
    return per || defPer || "default";
  }
  return String(planProfile || per || kitifiGenProfile(rid) || kitifiDefaultProfile() || "default").trim();
}
...
export async function kitifiMikrotikGenerateVoucher(conn, { plan, profile, uptime, routerId, prefix } = {}) {
  ...
  const prof = kitifiPaidHotspotProfile(rid, profile || p.profile);
...
async function kitifiEnsureMikrotikHotspotUser(conn, { code, profile, uptime, routerId } = {}) {
  const name = String(code || "").trim();
  if (!name) return { ok: false, reason: "missing voucher code" };
  const prof = kitifiPaidHotspotProfile(routerId, profile);
...
export async function kitifiMikrotikVoucherConnect(conn, { mac, voucher, routerId, ip, uptime, profile } = {}) {
  ...
  const prof = kitifiPaidHotspotProfile(routerId, profile);
  try {
    await kitifiEnsureMikrotikHotspotUser(conn, { code, profile: prof, uptime, routerId });
...
export async function kitifiGenerateVoucher(conn, { planId, plan, profile, uptime, seller, r_id, prefix, name_length, char, seller_id, routerId } = {}) {
  ...
  if (kitifiMikrotikOnlyRouter(rid)) {
    return kitifiMikrotikGenerateVoucher(conn, {
      plan: p,
      profile: kitifiPaidHotspotProfile(rid, profile || p.profile),
      uptime: uptime || p.uptime || p.time,
      routerId: rid,
      prefix,
    });
