...
export async function kitifiHotspotActiveForMac(conn, macAddr) {
  const active = await conn.print("/ip/hotspot/active");
  return (active || []).find((a) => String(a["mac-address"] || "").toUpperCase() === macAddr) || null;
}

export async function kitifiLogoutHotspotMac(conn, macAddr) {
  const hit = await kitifiHotspotActiveForMac(conn, macAddr);
  if (!hit) return false;
  try {
    await conn.talk(["/ip/hotspot/active/remove", "=.id=" + hit[".id"]]);
    return true;
  } catch {
    return false;
  }
...
async function kitifiActiveLogin(conn, { code, clientIp, macAddr }) {
  await conn.talk([
...
/** Login MAC even if a trial/old session is already active. Works with the portal closed. */
export async function kitifiForceHotspotLogin(conn, { code, clientIp, macAddr }) {
  const existing = await kitifiHotspotActiveForMac(conn, macAddr);
  if (existing && String(existing.user || "").toUpperCase() === String(code).toUpperCase()) {
    return { ok: true, already: true, ip: clientIp || existing.address, mac: macAddr };
  }
  try {
    await kitifiActiveLogin(conn, { code, clientIp, macAddr });
    return { ok: true, ip: clientIp, mac: macAddr };
  } catch (e) {
    if (!isAlreadyLoggedInError(e.message)) throw e;
    await kitifiLogoutHotspotMac(conn, macAddr);
    try {
      await kitifiActiveLogin(conn, { code, clientIp, macAddr });
      return { ok: true, relogged: true, ip: clientIp, mac: macAddr };
    } catch (e2) {
      if (isAlreadyLoggedInError(e2.message)) {
        return { ok: true, already: true, ip: clientIp, mac: macAddr };
      }
      throw e2;
    }
  }
...
export async function kitifiMikrotikVoucherConnect(conn, { mac, voucher, routerId, ip, uptime, profile } = {}) {
  ...
  try {
    ...
    const login = await kitifiForceHotspotLogin(conn, { code, clientIp, macAddr });
    return {
      ok: true,
      ip: clientIp,
      mac: macAddr,
      already: !!login.already,
...
export async function kitifiHotspotLoginByMac(conn, { mac, voucher, routerId } = {}) {
  ...
  try {
    const login = await kitifiForceHotspotLogin(conn, { code, clientIp: ip, macAddr });
    return { ok: true, ip, mac: macAddr, already: !!login.already };
  } catch (e) {
    if (isAlreadyLoggedInError(e.message)) return { ok: true, already: true, ip, mac: macAddr };
