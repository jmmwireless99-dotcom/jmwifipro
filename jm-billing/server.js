...
const requestHandler = async (req, res) => {
  ...
  try {
    ...
    if (pathname.startsWith("/api/billing")) {
      ...
      if (req.method === "GET") {
        if (isReferrals) {
          if (pathname === "/api/billing/referrals/me") {
            if (!need("technician", "staff", "helper", "cashier", "agent")) return denied();
          } else if (!need("technician", "staff", "helper", "cashier")) return denied();
        } else if (isAgents) {
      ...
      else if (isJobOrders || isInventory || isTechs || isMapInfra) { if (!need("technician", "cashier", "helper", "staff")) return denied(); }
      else if (isHelpdeskOps) { if (!need("cashier", "staff", "helper", "technician")) return denied(); }
      else if (isCustomers || isCustomerTools) { if (!need("cashier", "staff")) return denied(); }
      else { if (!need("cashier")) return denied(); }                    // payments, expenses, plans…
    ...
    switch (sub) {
      ...
      case "/hotspot/vouchers": {
        ...
        if (vRouterId) {
          ...
          if (def && def.id) {
            vRouterId = def.id;
            vRouterName = def.name || "";
            try { vconn = connForRouter(def); await vconn.identity(); } catch (e) { return send(res, 502, { ok: false, error: "Can't reach default router: " + e.message }); }
          }
        }
        let out;
        if (useKitifi) {
        ...
        if (useOriginal) {
          ...
          out = await generateHotspotVouchers({
            count: b.count, profile, length: b.length, prefix: b.prefix, uptime: b.uptime || "", userOnly,
            createUser: async ({ code, password, profile: prof, uptime }) => {
              await vconn.createHotspotUser({ name: code, password, profile: prof, limitUptime: uptime || "" });
            },
          });
          Audit.add({ type: "manual", action: "vouchers", detail: `${out.count}x ${profile}${userOnly ? " (user-only)" : ""} original @ ${vRouterName || "default"}`, ok: true });
          return ok({ ...out, price: b.price || "", exp: b.exp || "", router: vRouterName, generator: "original", central: false, dryRun: !!RouterOSAPI.dryRun });
        }
        const profile = CLOUD_VOUCHER_PROFILE;
        if (vRouterId && !hotspotCentralEnabled()) {
          const vr = Routers.get(vRouterId);
          if (vr) {
            try { const chk = connForRouter(vr); await chk.identity(); } catch (e) { return send(res, 502, { ok: false, error: "Can't reach " + vr.name + ": " + e.message }); }
          }
        }
        const made = [], errors = [];
        for (let i = 0; i < Math.min(Math.max(Number(b.count) || 10, 1), 500); i++) {
        ...
        return ok({ created: made, count: made.length, profile, uptime: b.uptime || "", price: b.price || "", exp: b.exp || "", userOnly, errors, router: vRouterName, generator: "cloud", central: hotspotCentralEnabled(), dryRun: hotspotCentralEnabled() ? false : !!RouterOSAPI.dryRun });
      }
...
async function handleBilling(req, res, pathname) {
  ...
  try {
    ...
    if (sub === "/routers" && method === "GET") {
      ...
      return ok(Routers.list().map(({ password, ...r }) => ({ ...r, hasPassword: !!password, health: _routerHealth[r.id] || null })));
    }
    ...
    if (sub === "/routers/health" && method === "GET") {
      ...
      const list = Routers.list().map(({ password, ...r }) => ({ id: r.id, name: r.name, host: r.host, area: r.area, enabled: r.enabled, is_default: r.is_default, clients: r.clients, suspended: r.suspended, health: _routerHealth[r.id] || null }));
    ...
    if ((mm = m(/^\/routers\/(\d+)\/hotspot-profiles$/)) && method === "GET") {
      const r = Routers.get(Number(mm[1]));
    ...
    if (sub === "/report/sales-by-router" && method === "GET") {
      ...
      return ok(Reports.salesByRouter(period));
    }
    if (sub === "/report/profit-by-router" && method === "GET") {
      const period = (q.period && q.period !== "all") ? q.period : null;
      return ok(Reports.profitByRouter(period));
    }
    ...
    if (sub === "/olts" && method === "GET") return ok(Olts.list());
    ...
    if (sub === "/fiber-lines" && method === "GET") return ok(FiberLines.list());
