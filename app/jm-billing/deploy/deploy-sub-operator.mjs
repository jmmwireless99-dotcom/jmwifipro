#!/usr/bin/env node
// Safe deploy: sub-operator files ONLY. Never touches billing.db or .env on VPS.
// Usage: VPS_PASS=... node deploy/deploy-sub-operator.mjs
...
if (!PASS) {
  ...
  console.error("  VPS_PASS='your-root-password' node deploy/deploy-sub-operator.mjs");
  process.exit(1);
}
...
conn.on("ready", () => {
  conn.sftp(async (err, sftp) => {
    ...
    try {
      console.log("Backing up current files on VPS...");
      ...
      console.log(`Backup → ${REMOTE}/.backup/${STAMP}/\n`);
      console.log("Uploading sub-operator files (code only — NOT billing.db):");
      ...
      console.log("\nDeploy OK. Database and .env were NOT touched.");
      conn.end();
    } catch (e) {
