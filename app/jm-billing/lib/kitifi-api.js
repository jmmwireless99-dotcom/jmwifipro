...
export function createKitifiApi(deps) {
  ...
  async function handleHotspotGenerators(req, res, sub, method, body, q) {
    ...
    if (sub === "/hotspot-generators/config" && method === "GET") {
      const rid = Number(q.router_id) || Number(Settings.get("kitifi_router_id")) || kitifiPortalRouterId();
      const kitifi = kitifiConfig(rid);
      let remoteRates = [];
      let remoteError = "";
      try {
        const { conn } = routerConn(rid);
        await conn.identity();
        remoteRates = await kitifiRemoteRates(conn, null, {
          routerId: rid,
          seller_id: kitifi.seller_id,
        });
        conn.close?.();
      } catch (e) {
    ...
    if (sub === "/hotspot-generators/kitifi-settings" && method === "POST") {
      ...
      const saveRid = body.kitifi_router_id != null && body.kitifi_router_id !== "" ? String(body.kitifi_router_id) : "";
      if (saveRid && body.kitifi_seller_id != null && body.kitifi_seller_id !== "") {
        Settings.set("kitifi_seller_id_" + saveRid, String(body.kitifi_seller_id));
      }
    ...
    if (sub === "/hotspot-generators/kitifi-test" && method === "POST") {
      ...
      await conn.identity();
      await kitifiTestLogin(conn, rid);
      const rates = await kitifiRemoteRates(conn, null, {
        routerId: rid,
