...
import { enrichUserFromDb, userScope, isSiteScoped, canAccessRouter } from "./scope.js";

export { userScope, isSiteScoped, canAccessRouter };
export const enrichUser = enrichUserFromDb;
...
export const ROLES = ["admin", "operator", "cashier", "technician", "staff", "helper", "agent"];
...
export function login(username, password, ip) {
  ...
  return { token, user: enrichUserFromDb({ id: u.id, username: u.username, role: u.role }) };
}
...
export function sessionUser(token) {
  ...
  return enrichUserFromDb({ id: s.id, username: s.username, role: s.role });
}
