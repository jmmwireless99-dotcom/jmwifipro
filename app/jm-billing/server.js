...
import { parseCommand, draftReply, dailyDigest, customerChat, faqChatReply, aiEnabled, friendlyChatError, setAiConfigProvider } from "./lib/ai.js";
import { screenProof } from "./lib/proofcheck.js";
import { parsePaymentSms } from "./lib/smsparse.js";
import { loadEnv } from "./lib/env.js";
import { Plans, Customers, Invoices, Payments, summary, planDurationMins, planRateLimit, planDisplaySpeed, fmtPlanDuration } from "./lib/db.js";
...
import { createLink as pmCreateLink, createQrphPayment as pmCreateQrph, getPaymentIntent as pmGetIntent, verifyWebhook as pmVerify, parseEvent as pmParse } from "./lib/paymongo.js";
import { createLink as xdCreateLink, verifyWebhook as xdVerify, parseEvent as xdParse } from "./lib/xendit.js";
import { Collections, Audit, Sales } from "./lib/db.js";
import { HotspotEvents, Coins, HotspotSales, HotspotCentral, hotspotCentralEnabled, uptimeToSecs, mikrotikCentralRadiusScript } from "./lib/db.js";
import { VoucherOrders } from "./lib/voucher-sales.js";
import { VendoSales, NewUsers, SalesAdmin } from "./lib/db.js";
import { Vendos } from "./lib/db.js";
import { CoinLog } from "./lib/db.js";
import { startRadiusServer } from "./lib/radius-server.js";
...
import { Referrals } from "./lib/db.js";
import { Agents } from "./lib/db.js";
...
import { kpis, Naps, Olts, FiberLines, Outages, Reports, CSessions, ClientStatus, Sms, Routers } from "./lib/db.js";
...
// ---- Payment gateway selector (PayMongo or Xendit) ----
// Returns a unified interface so the rest of the server doesn't care which provider is active.
function paymentGateway() {
  const s = Settings.all();
  const which = (s.payment_gateway || "paymongo").toLowerCase();
  if (which === "xendit") {
    return {
      name: "xendit",
      mode: "link",
      configured: !!s.xendit_secret,
      secret: s.xendit_secret || process.env.XENDIT_SECRET || "",
      createLink: (opts) => xdCreateLink({ secret: s.xendit_secret || process.env.XENDIT_SECRET || "", baseUrl: s.xendit_base || undefined }, opts),
      verifyWebhook: (raw, headers) => xdVerify(raw, headers["x-callback-token"], s.xendit_callback_token || ""),
      parseEvent: (json) => xdParse(json),
      notConfiguredMsg: "Online payment isn't set up — Xendit key missing. Please pay at the counter.",
    };
  }
  const pmCfg = () => ({ secret: s.paymongo_secret || process.env.PAYMONGO_SECRET || "", baseUrl: s.paymongo_base || undefined });
  const pmMode = (s.paymongo_mode || "link").toLowerCase() === "qrph" ? "qrph" : "link";
  // default: PayMongo
  return {
    name: "paymongo",
    configured: !!s.paymongo_secret,
    mode: pmMode,
    secret: s.paymongo_secret || process.env.PAYMONGO_SECRET || "",
    createLink: (opts) => pmCreateLink(pmCfg(), opts),
    createQrph: (opts) => pmCreateQrph(pmCfg(), opts),
    getIntent: (id) => pmGetIntent(pmCfg(), id),
    verifyWebhook: (raw, headers) => pmVerify(raw, headers["paymongo-signature"], s.paymongo_webhook_secret || ""),
    parseEvent: (json) => pmParse(json),
    notConfiguredMsg: "Online payment isn't set up — please pay at the counter.",
  };
}

function portalCheckoutUrls() {
  const pub = String(Settings.get("public_url") || "").trim().replace(/\/$/, "");
  if (!pub) return {};
  return { successUrl: pub + "/account?paid=1", failedUrl: pub + "/account?paid=0" };
}

function portalPublicBase() {
  return String(Settings.get("public_url") || "").trim().replace(/\/$/, "");
}

// Captive / suspend redirect target — prefer https://jmwifi.pro/welcome when public_url is set.
function portalWelcomeLocation(req) {
  const qs = new URL(req.url, "http://localhost").searchParams;
  if (!qs.has("suspended")) qs.set("suspended", "1");
  const path = "/welcome?" + qs.toString();
  const base = portalPublicBase();
  return base ? base + path : path;
}

function portalPayWelcomeLocation(req) {
  const qs = new URL(req.url, "http://localhost").searchParams;
  if (!qs.has("suspended")) qs.set("suspended", "1");
  if (!qs.has("pay")) qs.set("pay", "1");
  const path = "/welcome?" + qs.toString();
  const base = portalPublicBase();
  return base ? base + path : path;
}

/** Android/iOS/Windows connectivity probes (e.g. generate_204). Non-204 → "Sign in to network". */
const CAPTIVE_PROBE_HOST = /connectivitycheck\.(gstatic|android)\.com|clients3\.google\.com|captive\.apple\.com|msftconnecttest\.com|detectportal\.firefox\.com|nmcheck\.gnome\.org/i;
function isCaptivePortalProbe(req, pathname) {
  if (req.method !== "GET" && req.method !== "HEAD") return false;
  const host = String(req.headers.host || "").split(":")[0].toLowerCase();
  if (pathname === "/generate_204" || pathname === "/gen_204") return true;
  return CAPTIVE_PROBE_HOST.test(host);
}

function referralPublicBase() {
  return portalPublicBase() || "https://jmwifi.pro";
}

function referralApplyUrl(code) {
  const c = String(code || "").trim().toUpperCase();
  if (!c) return "";
  const path = "/apply?ref=" + encodeURIComponent(c);
  return referralPublicBase().replace(/\/$/, "") + path;
}

function portalInvoicePayUrl(invoiceId) {
  const rel = "/pay/qr?invoice=" + encodeURIComponent(String(invoiceId));
  const base = portalPublicBase();
  return base ? base + rel : rel;
}

function isHostedCheckoutUrl(url) {
  return /paymongo|xendit|checkout\./i.test(String(url || ""));
}

// Unified online payment: QR Ph on our portal, or hosted checkout link (Xendit / PayMongo link mode).
async function createGatewayPayment(gw, { amountPhp, description, remarks, metadata = {}, invoiceId = null }) {
  const meta = { ...metadata };
  if (remarks && !meta.portal) meta.portal = remarks;
  if (gw.name === "paymongo" && gw.mode === "qrph" && gw.createQrph) {
    const qr = await gw.createQrph({ amountPhp, description, metadata: meta });
    const portalUrl = invoiceId ? portalInvoicePayUrl(invoiceId) : null;
    return {
      mode: "qrph",
      id: qr.id,
      payment_intent_id: qr.payment_intent_id,
  ...
  const link = await gw.createLink({ amountPhp, description, remarks, ...portalCheckoutUrls() });
  return {
    mode: "link",
    id: link.id,
    url: link.checkout_url || link.url || "",
    checkout_url: link.checkout_url || link.url || "",
...
// ---- Multi-router connection factory ----
// Build a RouterOS connection object from a stored router row (its own host/port/user/pass/ssl).
// Cached per router id so we reuse connections. Falls back to REST class only if global MODE=rest.
const _routerConns = new Map();   // id -> connection object
function connForRouter(r) {
  if (!r) return mt;   // no router row → use the legacy global connection (single-router compatibility)
  const key = r.id;
  const cached = _routerConns.get(key);
  // rebuild if connection details changed
  if (cached && cached._sig === routerSig(r)) return cached.conn;
  let conn;
  if (MODE === "rest") {
    conn = new MikroTik({ host: (r.host || "").split(":")[0], user: r.username, password: r.password, verifyTls: false });
  } else {
    conn = new RouterOSAPI({ host: (r.host || "").split(":")[0], user: r.username, password: r.password, port: Number(r.port) || (r.ssl ? 8729 : 8728), ssl: !!r.ssl });
  }
  _routerConns.set(key, { conn, _sig: routerSig(r) });
  return conn;
}
function routerSig(r) { return [r.host, r.port, r.username, r.password, r.ssl].join("|"); }
function invalidateRouterConn(id) { _routerConns.delete(id); }

async function netSetupForRouter(routerId) {
  const safe = async (fn) => { try { return (await fn()) || []; } catch { return []; } };
  let conn, router;
  try {
    ({ conn, router } = resolveRouterCtx(routerId));
  } catch (e) {
    return { error: e.message, router_id: routerId || null, router_name: "", interfaces: [], vlans: [], addresses: [], pools: [], dhcp: [], hotspots: [], profiles: [] };
  }
  const [ifaces, vlans, addrs, pools, dhcp, hs, hsprof] = await Promise.all([
    safe(() => conn.interfaces()), safe(() => conn.vlans()), safe(() => conn.ipAddresses()),
    safe(() => conn.pools()), safe(() => conn.dhcpServers()), safe(() => conn.hotspotServers()), safe(() => conn.hotspotServerProfiles()),
  ]);
  return {
...
// Pick MikroTik connection for a router id (blank = default site, or legacy global mt).
function resolveRouterCtx(routerId) {
  let router = null;
  if (routerId != null && routerId !== "") {
    router = Routers.get(Number(routerId));
    if (!router) throw new Error("Router/site not found.");
    if (router.enabled === 0) throw new Error(`Router "${router.name}" is paused — enable it first.`);
  } else {
    router = Routers.getDefault();
  }
  if (router) {
    return { conn: connForRouter(router), router };
  }
  if (hasRouterConfigured()) {
    const def = Routers.getDefault();
    if (def && def.host) return { conn: connForRouter(def), router: def };
  }
  return { conn: mt, router: { id: null, name: "Default site" } };
}

async function pppoeEditOnRouter(b) {
  const srcCtx = resolveRouterCtx(b.router_id);
  const moveTo = b.moveToRouter;
  if (moveTo != null && moveTo !== "" && String(moveTo) !== String(b.router_id || "")) {
    const dstCtx = resolveRouterCtx(moveTo);
    const oldName = b.oldName || b.name;
    const secrets = await srcCtx.conn.listPppoe();
    const u = (secrets || []).find((s) => s.name === oldName);
    if (!u) throw new Error(`User "${oldName}" not found on ${srcCtx.router.name || "source router"}.`);
    if (!b.password) throw new Error("Enter a password when moving this user to another router/site.");
    const newName = (b.name && b.name !== oldName) ? b.name : oldName;
    await dstCtx.conn.createPppoe({
      name: newName, password: b.password, profile: b.profile || u.profile || "default",
      service: b.service || u.service || "pppoe", comment: u.comment || "",
    });
    await srcCtx.conn.deletePppoe(oldName);
    const cust = Customers.list().find((c) => {
      const un = (c.username || "").toLowerCase();
      return un === oldName.toLowerCase() || un === newName.toLowerCase();
    });
    if (cust && dstCtx.router.id) Customers.setRouter(cust.id, dstCtx.router.id);
    Audit.add({ type: "manual", action: "pppoe-move-router", detail: `${oldName} → ${dstCtx.router.name}`, ok: true });
    return { moved: true, name: newName, router_id: dstCtx.router.id };
  }
  return srcCtx.conn.updatePppoe(b.oldName || b.name, b);
}

// Resolve the right RouterOS connection for a given customer.
// Uses the customer's assigned router; falls back to the default router; falls back to the
// legacy global `mt` if no routers exist (single-router installs still work unchanged).
function connForCustomer(c) {
  try {
    if (c && c.router_id) {
      const r = Routers.get(c.router_id);
      if (r) { if (r.enabled === 0) throw new Error(`Router "${r.name}" is paused — enable it to send commands.`); return connForRouter(r); }
    }
    const def = Routers.getDefault();
    if (def) { if (def.enabled === 0) throw new Error(`Default router "${def.name}" is paused.`); return connForRouter(def); }
  } catch (e) { if (/paused/.test(e.message)) throw e; }
  if (hasRouterConfigured()) {
    const def = Routers.getDefault();
    if (def && def.host) return connForRouter(def);
  }
  return mt;   // legacy single-router settings only
}

function hasRouterConfigured() {
  if ((mtConfig().host || "").trim()) return true;
  return Routers.list().some((r) => r.enabled !== 0 && String(r.host || "").trim());
}

// Test a router's connection by fetching its identity. Returns {ok, identity?|error}.
async function testRouterConnection(r) {
  if (!r || !r.host) return { ok: false, error: "No host set for this router." };
  try {
    const conn = connForRouter(r);
    const id = await conn.identity();
    const name = (id && (id.name || (id[0] && id[0].name))) || "(connected)";
    return { ok: true, identity: name, host: r.host };
  } catch (e) {
    return { ok: false, error: e.message || String(e) };
  }
}

// ---- One-time migration: single-router → multi-router ----
// If no routers exist yet but a legacy mikrotik_host is configured, create "Router 1" from it and
// assign every existing customer to it. This keeps single-router setups working with zero changes.
function migrateToMultiRouter() {
  try {
    const existing = Routers.list();
    if (existing.length > 0) return;   // already migrated / using multi-router
    const host = (() => { try { return Settings.get("mikrotik_host", "") || process.env.MIKROTIK_HOST || ""; } catch { return process.env.MIKROTIK_HOST || ""; } })();
    if (!host) return;                 // nothing to migrate (fresh install with no router yet)
    const hostOnly = host.split(":")[0].trim();
    // Don't auto-create a router from an empty/placeholder/default address — that just produces a
    // dead "Router 1" row (e.g. 192.168.88.1) that silently captures provisioning. Only migrate a
    // host that the operator actually configured AND that has real credentials.
    const PLACEHOLDER_HOSTS = new Set(["", "192.168.88.1", "192.168.1.1", "0.0.0.0", "127.0.0.1", "localhost", "10.0.0.1"]);
    const hasCreds = !!(process.env.MIKROTIK_USER && process.env.MIKROTIK_PASSWORD);
    if (PLACEHOLDER_HOSTS.has(hostOnly) || !hasCreds) {
      // Leave routers empty; the panel falls back to the legacy global connection, and the operator
      // can add their real router explicitly in Routers / Sites when ready.
      return;
    }
    const r = Routers.create({
      name: Settings.get("biz_name", "") ? (Settings.get("biz_name", "") + " Router") : "Router 1",
      host: hostOnly,
      port: Number(process.env.MIKROTIK_PORT) || 0,
      username: process.env.MIKROTIK_USER || "",
      password: process.env.MIKROTIK_PASSWORD || "",
      ssl: (process.env.MIKROTIK_TLS_VERIFY === "true" || (process.env.MIKROTIK_PORT === "8729")) ? 1 : 0,
      area: Settings.get("biz_address", "") || "",
      vpn_notes: "Auto-created from your existing single-router setup.",
      is_default: 1,
    });
    // assign all existing customers with no router to this one
    const unassigned = Customers.list().filter((c) => !c.router_id);
    for (const c of unassigned) { try { Customers.setRouter(c.id, r.id); } catch {} }
    Audit.add({ type: "auto", action: "router-migrate", detail: `Created default router "${r.name}" and assigned ${unassigned.length} existing customers.`, ok: true });
  } catch (e) { /* never block boot on migration */ }
}
migrateToMultiRouter();
...
function syncDryRun() {
  ...
  // keep the REST class in sync so dry-run behaves identically in both connection modes
  MikroTik.dryRun = on;
  MikroTik.onDryRun = RouterOSAPI.onDryRun;
}
syncDryRun();
...
const GET_ROUTES = {
  ...
  "/api/pppoe/active": () => listAllPppoeActive(),
  ...
  "/api/router/ipoe-redirect-diag": async () => {
    ...
    try { const l = await mt.print("/ip/firewall/address-list", { "?list": ipoeSuspendList() }); add("suspended-list", true, (l || []).length + " IP(s) on '" + ipoeSuspendList() + "'"); }
    catch (e) { add("suspended-list", false, e.message); }
    // can we WRITE? add a harmless test address-list entry then remove it
    try {
      await mt.addrListAdd(ipoeSuspendList() + "-test", "1.2.3.4", "diag test");
      await mt.addrListRemove(ipoeSuspendList() + "-test", "1.2.3.4");
...
async function listAllPppoeActive() {
  const routers = Routers.list().filter((r) => r.enabled !== 0 && r.host);
  if (routers.length <= 1) {
    const { conn } = resolveRouterCtx(null);
    return (await conn.listPppoeActive()) || [];
  }
  const all = [];
  for (const r of routers) {
    try {
      const conn = connForRouter(r);
      const act = (await conn.listPppoeActive()) || [];
      act.forEach((a) => all.push({ ...a, router_id: r.id, router_name: r.name }));
    } catch {}
  }
  return all;
}

async function listAllPppoeSecrets() {
  const routers = Routers.list().filter((r) => r.enabled !== 0 && r.host);
  if (routers.length <= 1) {
    const { conn, router } = resolveRouterCtx(null);
    const rid = router?.id || Routers.getDefault()?.id || null;
    return ((await conn.listPppoe()) || []).map((row) => ({ ...row, _router_id: rid }));
  }
  const all = [];
  for (const r of routers) {
    try {
      ...
      rows.forEach((row) => all.push({ ...row, _router_id: r.id }));
    } catch {}
  }
  return all;
}

async function usersStatus(routerId) {
  const { conn, router } = resolveRouterCtx(routerId);
  const [pppSec, pppAct, hsUsers, hsAct] = await Promise.all([
    conn.listPppoe(), conn.listPppoeActive(), conn.listHotspotUsers(), conn.listHotspotActive(),
  ]);
  const onPpp = new Set((pppAct || []).map((a) => a.name));
  const onHs = new Set((hsAct || []).map((a) => a.user));
  const pppoe = (pppSec || []).map((s) => ({
    name: s.name, profile: s.profile || "default", service: s.service || "any",
  ...
  return {
    pppoe, hotspot, pppoeOnline: onPpp.size, hotspotOnline: onHs.size,
    router_id: router.id || null, router_name: router.name || "Default site",
  };
}

async function pppProfilesForRouter(routerId) {
  const { conn } = resolveRouterCtx(routerId);
  return conn.pppProfiles();
}
...
self.addEventListener("fetch", (e) => {
  ...
  if (/\\/(icon-\\d+\\.png|icon-account-\\d+\\.png|manifest|pay-manifest|apply-manifest|account-manifest|welcome-manifest)\\b/.test(url.pathname) || url.pathname.endsWith(".webmanifest")) {
    e.respondWith(
...
function accountAppIconResponse(size) {
  const logo = String(Settings.get("brand_logo", "") || "").trim();
  if (logo.startsWith("data:image/")) {
    try {
      const { buffer, contentType } = dataUrlToBuffer(logo);
      if (buffer && buffer.length > 64) {
        return { buffer, contentType: contentType || "image/png" };
      }
    } catch {}
  }
  return { buffer: pwaPortalIconPng(size), contentType: "image/png" };
}
function pwaPortalIconPng(size) {
  // JM WIFI portal icon: purple card + pink signal bars (matches account page branding).
  const S = size, bg = [26, 14, 46], panel = [124, 58, 237], accent = [232, 25, 95];
  const buf = Buffer.alloc(S * S * 4);
  const set = (x, y, c, a = 255) => { if (x < 0 || y < 0 || x >= S || y >= S) return; const i = (y * S + x) * 4; buf[i] = c[0]; buf[i + 1] = c[1]; buf[i + 2] = c[2]; buf[i + 3] = a; };
  ...
  for (let b = 0; b < 3; b++) {
    const h = Math.round(S * heights[b]);
    for (let y = baseY - h; y <= baseY; y++) for (let x = bx; x < bx + bw; x++) set(x, y, accent);
    bx += bw + gap;
  }
  return encodePng(S, S, buf);
}
...
// Serve a customer-portal page (the same UI as the standalone VPS portal). These live in
// public/portal/ and are byte-identical to jeffnet-portal so the two stay in sync.

function serveHotspotAsset(res, file) {
  const safe = String(file || "").replace(/\\/g, "/").split("/").filter((p) => p && p !== "..").join("/");
  if (!safe) {
    res.writeHead(404, { "Content-Type": "text/plain" });
    return res.end("not found");
  }
  const full = path.join(__dirname, "public", "hotspot", safe);
  if (!full.startsWith(path.join(__dirname, "public", "hotspot"))) {
...
// Suspended subscribers are dst-nat'd to the portal IP but browsers keep the original Host
// (e.g. neverssl.com). Send those requests to the pay portal instead of the admin panel.
function portalHostNames() {
  const hosts = new Set(["localhost", "127.0.0.1"]);
  const pub = String(Settings.get("public_url") || "").trim().replace(/^https?:\/\//, "").split("/")[0].toLowerCase();
  if (pub) hosts.add(pub.split(":")[0]);
  for (const h of ["jmwifi.pro", "www.jmwifi.pro", "187.77.145.131"]) hosts.add(h);
  return hosts;
}
function isCaptivePortalRequest(req, pathname) {
  if (req.method !== "GET" && req.method !== "HEAD") return false;
  const host = String(req.headers.host || "").split(":")[0].toLowerCase();
  if (portalHostNames().has(host)) return false;
  if (/^\/(pay|account|apply|help|welcome|home|voucher|hotspot\/|api\/|pay\/|manifest|icon-|sw\.js)/.test(pathname)) return false;
  if (/\.(css|js|png|jpe?g|webp|svg|ico|woff2?|webmanifest)$/i.test(pathname)) return false;
  return true;
}
...
const requestHandler = async (req, res) => {
  ...
  try {
    if (isCaptivePortalProbe(req, pathname)) {
      res.writeHead(302, { Location: portalWelcomeLocation(req) });
      return res.end();
    }
    if (isCaptivePortalRequest(req, pathname)) {
      res.writeHead(302, { Location: portalWelcomeLocation(req) });
      return res.end();
    }
    ...
    // Customer welcome / home app — opens /welcome (Apply, Account, Pay, Help menu).
    if (req.method === "GET" && pathname === "/welcome-manifest.webmanifest") {
      const biz = Settings.get("biz_name", "Internet");
      const short = "JM WIFI";
      res.writeHead(200, { "Content-Type": "application/manifest+json", "Cache-Control": "public, max-age=300" });
      return res.end(JSON.stringify({
        id: "/welcome",
        name: biz,
        short_name: short,
        description: "Apply for internet, view your account, pay your bill, and get help.",
        start_url: "/welcome",
        scope: "/",
        display: "standalone",
        ...
        icons: [
          { src: "/icon-account-192.png", sizes: "192x192", type: "image/png", purpose: "any" },
          { src: "/icon-account-512.png", sizes: "512x512", type: "image/png", purpose: "any" },
          { src: "/icon-account-512.png", sizes: "512x512", type: "image/png", purpose: "maskable" },
        ],
      }));
    }
    // Customer account app (login + pay) — installable on Android / iOS home screen or TWA APK.
    if (req.method === "GET" && pathname === "/account-manifest.webmanifest") {
      const biz = Settings.get("biz_name", "Internet");
      const short = "JM WIFI";
      res.writeHead(200, { "Content-Type": "application/manifest+json", "Cache-Control": "public, max-age=300" });
      return res.end(JSON.stringify({
        ...
        name: biz + " — My Account",
        short_name: short,
        description: "Sign in, view your plan, and pay your bill with QR Ph.",
        start_url: "/account",
        scope: "/",
        display: "standalone",
        orientation: "portrait",
    ...
    // Legacy alias — same customer app, opens account page.
    if (req.method === "GET" && pathname === "/pay-manifest.webmanifest") {
      ...
      return res.end(JSON.stringify({
        ...
        icons: [
          { src: "/icon-account-192.png", sizes: "192x192", type: "image/png", purpose: "any" },
          { src: "/icon-account-512.png", sizes: "512x512", type: "image/png", purpose: "any maskable" },
        ],
      }));
    }
    ...
    if (req.method === "GET" && (pathname === "/icon-account-192.png" || pathname === "/icon-account-512.png")) {
      const size = pathname.includes("512") ? 512 : 192;
      const icon = accountAppIconResponse(size);
      res.writeHead(200, { "Content-Type": icon.contentType, "Cache-Control": "public, max-age=3600" });
      return res.end(icon.buffer);
    }
    // Digital Asset Links for Android TWA (full-screen APK). Set android_sha256 in settings after signing your APK.
    if (req.method === "GET" && pathname === "/.well-known/assetlinks.json") {
      const filePath = path.join(__dirname, "public", ".well-known", "assetlinks.json");
      if (fs.existsSync(filePath)) {
        res.writeHead(200, { "Content-Type": "application/json", "Cache-Control": "public, max-age=3600" });
        return res.end(fs.readFileSync(filePath));
      }
      const pkg = (Settings.get("android_package", "pro.jmwifi.client") || "pro.jmwifi.client").trim();
      const sha = (Settings.get("android_sha256", "") || "").trim().replace(/:/g, "").toUpperCase();
      res.writeHead(200, { "Content-Type": "application/json", "Cache-Control": "public, max-age=3600" });
      if (!sha) return res.end("[]");
      return res.end(JSON.stringify([{
    ...
    if (pathname === "/api/billing/paymongo/webhook" && req.method === "POST") {
      ...
      if (ev && /paid$/.test(ev.type || "")) {
        await handlePaidOnlineGatewayEvent(ev, "paymongo");
      }
      res.writeHead(200, { "Content-Type": "text/plain" });
      return res.end("OK");
    }

    // Xendit webhook (public; Xendit posts the invoice object here — verified by the x-callback-token header)
    if (pathname === "/api/billing/xendit/webhook" && req.method === "POST") {
      const raw = (await readBody(req)) || "";
      const token = Settings.get("xendit_callback_token");
      if (!token) { res.writeHead(503); return res.end("callback token not configured"); }
      if (!xdVerify(raw, req.headers["x-callback-token"], token)) { res.writeHead(401); return res.end("bad token"); }
      let json = null; try { json = JSON.parse(raw); } catch {}
      const ev = xdParse(json);
      if (ev && ev.paid) {
        await handlePaidOnlineGatewayEvent(ev, "xendit");
      }
      res.writeHead(200, { "Content-Type": "text/plain" });
      return res.end("OK");
    }
    if (req.method === "GET" && /^\/ref\/[A-Za-z0-9_-]+$/i.test(pathname)) {
      const code = pathname.slice(5).trim().toUpperCase();
      res.writeHead(302, { Location: "/apply?ref=" + encodeURIComponent(code) });
      return res.end();
    }
    if ((pathname === "/welcome" || pathname === "/home") && req.method === "GET") {
      return servePortalPage(res, "index.html");
    }
    if (pathname === "/account" && req.method === "GET") {
      return servePortalPage(res, "account.html");
    }
    if (pathname === "/portal/chat-bot.js" && req.method === "GET") {
      try {
        const buf = fs.readFileSync(path.join(__dirname, "public", "portal", "chat-bot.js"));
    ...
    if (pathname === "/pay/qr" && req.method === "GET") {
      return servePortalPage(res, "pay-qr.html");
    }

    // ---- Public customer "pay your bill" page + API (no login) ----
    if (pathname === "/portal" && req.method === "GET") {
      // /portal now redirects to the new account page (same login + pay flow)
    ...
    if (pathname === "/pay" && req.method === "GET") {
      // Legacy / SMS links — land on welcome portal (My Account → pay from there).
      res.writeHead(302, { Location: portalPayWelcomeLocation(req) });
      return res.end();
    }
    if (pathname === "/apply" && req.method === "GET") {
      return servePortalPage(res, "apply.html");
    }
    if (req.method === "GET" && pathname.startsWith("/hotspot/")) {
      const hf = pathname.slice("/hotspot/".length);
      return serveHotspotAsset(res, hf);
    }
    ...
    if (pathname === "/api/voucher/pay-start" && req.method === "POST") {
      ...
      if (!plan || String(plan.type || "").toLowerCase() !== "hotspot") {
        return send(res, 400, { ok: false, error: "Invalid hotspot package." });
      }
      const amount = Number(plan.price) || 0;
      if (amount < 20) return send(res, 400, { ok: false, error: "Minimum online payment is \u20B120." });
      const gw = paymentGateway();
      if (!gw.configured) return send(res, 503, { ok: false, error: "Online payment isn't set up yet. Use the coin vendo or contact staff." });
      const def = Routers.getDefault();
      const routerId = Number(b.router_id) || (def && def.id) || null;
      const portalTag = "voucher:" + planId + (routerId ? ":" + routerId : "");
      const desc = "Hotspot voucher — " + (plan.name || "WiFi") + " (" + pesos(amount) + ")";
      try {
        if (gw.name === "paymongo" && gw.createQrph) {
          ...
          return send(res, 200, {
            ok: true, mode: "qrph", gateway: gw.name,
            payment_intent_id: qr.payment_intent_id, qr_image_url: qr.qr_image_url,
            amount, id: qr.id, status: qr.status,
          });
        }
        const pay = await createGatewayPayment(gw, { amountPhp: amount, description: desc, remarks: portalTag });
        return send(res, 200, { ok: true, mode: pay.mode, url: pay.url, checkout_url: pay.checkout_url, gateway: gw.name, id: pay.id });
      } catch (e) {
        return send(res, 502, { ok: false, error: "Could not start payment: " + e.message });
      }
    }
    if (pathname === "/api/voucher/result" && req.method === "GET") {
      const pi = (new URL(req.url, "http://localhost").searchParams.get("pi") || "").trim();
      if (!pi) return send(res, 400, { ok: false, error: "Missing payment id." });
      const gw = paymentGateway();
      if (!gw.configured || gw.name !== "paymongo" || !gw.getIntent) {
        return send(res, 503, { ok: false, error: "Online payment not available." });
      }
      try {
        const intent = await gw.getIntent(pi);
        const paid = intent.status === "succeeded";
        if (!paid) return send(res, 200, { ok: true, paid: false, status: intent.status });
        const tag = parseVoucherPaymentTag(intent.metadata?.portal || intent.description || "");
        if (!tag) return send(res, 200, { ok: true, paid: true, code: null });
        const amountPeso = Math.round(Number(intent.amount) / 100);
        const out = await fulfillOnlineVoucher({ planId: tag.planId, routerId: tag.routerId, ref: pi, amountPeso });
        return send(res, 200, { ok: true, paid: true, code: out.code, plan: out.plan, skipped: !!out.skipped });
      } catch (e) {
    ...
    if (pathname === "/api/apply/info" && req.method === "GET") {
      ...
      // public can see active plans (name/price/speed/type/install fee)
      const plans = Plans.list()
        .filter((p) => String(p.status || "active").toLowerCase() !== "inactive")
        .map((p) => ({
        id: p.id, name: p.name, price: p.price,
        speed: clientSpeedLabel(p),
        validity_days: p.validity_days, validity_mins: planDurationMins(p),
        type: p.type || "pppoe", features: p.features || "",
        installation_fee: Plans.installFee(p.id, fee),
      }));
    ...
    if (pathname === "/api/apply" && req.method === "POST") {
      ...
      const jo = JobOrders.apply({
        ...
        install_fee: Plans.installFee(b.plan_id, s.install_fee),
      ...
      Audit.add({ type: "auto", action: "job-application", detail: `${parsed.fullName} (${b.contact}) — ${b.pay_choice === "now" ? "pay now" : "pay on install"}`, ok: true });
      ...
      return send(res, 200, { ok: true, id: jo.id, message: "Thank you! Your application was received. We'll contact you shortly to schedule your installation. After your account is activated, sign in at My Account using your first name and last name." });
    }

    // ===== Customer portal API (matches the standalone VPS portal contract) =====
    // These power the new welcome/apply/account/help pages served from public/portal/.

    // Plans + portal config (GCash/Maya/QR, business name) for the portal pages.
    if (pathname === "/api/plans" && req.method === "GET") {
      const s = Settings.all();
      const globalInstall = Number(s.install_fee || 0);
      const plans = (Plans.list() || [])
        .filter((p) => String(p.status || "active").toLowerCase() !== "inactive")
        .map((p) => portalPlanPayload(p));
      const gw = paymentGateway();
      const config = {
        biz_name: s.biz_name || "JM WIFI NETWORK",
        biz_address: s.biz_address || "",
        biz_contact: s.biz_contact || "",
        brand_logo: s.brand_logo || "",
        gcash_name: s.gcash_name || "", gcash_number: s.gcash_number || "", gcash_qr: s.gcash_qr || "",
        maya_name: s.maya_name || "", maya_number: s.maya_number || "", maya_qr: s.maya_qr || "",
        install_fee: Number(s.install_fee || 0), router_cost: Number(s.router_cost || 0),
        agreement: s.apply_agreement || "",
        payment_gateway: gw.configured ? gw.name : "",
        payment_mode: gw.mode || "link",
        payment_configured: !!gw.configured,
      };
      return send(res, 200, { ok: true, plans, config });
    }
    if (pathname === "/api/hotspot/walled-garden" && req.method === "GET") {
      const pub = String(Settings.get("public_url") || "").trim().replace(/^https?:\/\//, "").split("/")[0];
      const portalHost = pub || "jmwifi.pro";
      let portalIp = "187.77.145.131";
      try {
        const dnsMod = await import("node:dns/promises");
        const ips = await dnsMod.resolve4(portalHost);
        if (ips && ips[0]) portalIp = ips[0];
      } catch {}
      const script = hotspotWalledGardenCommands({ portalHost, portalIp });
      return send(res, 200, { ok: true, script, portalHost, portalIp, voucher_url: `https://${portalHost}/voucher` });
    }

    // My Account lookup: first name + last name (password field).
    if (pathname === "/api/myaccount" && req.method === "POST") {
      const raw = (await readBody(req)) || "";
      let b; try { b = JSON.parse(raw || "{}"); } catch { return send(res, 400, { ok: false, error: "Bad data." }); }
      const split = splitPortalLogin(b.first || b.first_name || b.code, b.password || b.last);
      b = { ...b, first: split.first, password: split.password };
      if (!b.first || !b.password) return send(res, 400, { ok: false, code: "missing", error: "Enter your first name and password (your last name)." });
      const login = resolvePortalLogin(b);
      if (!login.ok) return send(res, login.code === "wrong_password" ? 401 : 404, { ok: false, code: login.code, error: login.error });
      const cust = login.customer;
      const inv = Invoices.byCustomer(cust.id).find((i) => i.status !== "paid");
      let amountDue = Number(cust.plan_price) || 0;
      if (inv) {
        const invBal = Math.max(0, Number(inv.amount) - (Number(inv.paid_amount) || 0));
        if (invBal > 0) amountDue = invBal;
      }
      const account = {
        code: cust.account_code || cust.username || "", name: cust.name,
        plan: cust.plan_name || "", speed: cust.plan_speed || "", status: cust.status || "active",
        due: cust.expiry || "", balance: Number(cust.credit || 0), rate: Number(cust.plan_price || 0),
        amount_due: amountDue,
        username: cust.username || "", conn_type: cust.conn_type || "pppoe",
        plan_id: cust.plan_id || null,
        last_paid_plan: cust.last_paid_plan_name || "",
        is_expired: Customers.isExpiredAsOf(cust.expiry, billingNow()) || cust.status === "suspended",
        customer_id: cust.id,
        referral_code: cust.referral_code || Referrals.ensureCode(cust.id, cust.username || cust.name),
        referral_points: Number(cust.referral_points) || 0,
        point_peso: Referrals.pointPeso(),
      };
      return send(res, 200, { ok: true, account });
    }

    // Manual payment (GCash/Maya proof) — same engine as the cloud portal pull path.
    if (pathname === "/api/pay" && req.method === "POST") {
      const raw = (await readBody(req)) || "";
      if (raw.length > 8_000_000) return send(res, 413, { ok: false, error: "Image too large (max ~6MB)." });
      let b; try { b = JSON.parse(raw || "{}"); } catch { return send(res, 400, { ok: false, error: "Bad data." }); }
      // anti-abuse: honeypot + min fill time
      if (b._hp) return send(res, 200, { ok: true, message: "Thank you! We'll confirm your payment shortly." });
      // Duplicate guard: block a SECOND submission of the same reference (customer double-tapped).
      // Only blocks exact same reference; distinct payments always go through.
      const payRef = String(b.pay_reference || "").replace(/\s+/g, "");
      if (payRef) {
        try {
          const dup = (Proofs.allRefs() || []).some((p) => String(p.reference || "").replace(/\s+/g, "") === payRef);
          if (dup) return send(res, 200, { ok: true, message: "We already received a payment with that reference number." });
        } catch {}
      }
      // Route through the shared portal-payment engine (matches a SIM SMS or sends a Telegram approve button).
      try {
        await handlePortalPayment({
          name: b.name, contact: b.contact, message: b.message,
          pay_reference: b.pay_reference, proof_img: b.proof_img, purpose: b.purpose,
        });
      } catch (e) { Audit.add({ type: "auto", action: "portal-payment", detail: "error: " + e.message, ok: false }); }
      return send(res, 200, { ok: true, message: "Thank you! We'll confirm your payment shortly." });
    }

    // Help / report-a-problem ticket.
    if (pathname === "/api/help" && req.method === "POST") {
      const raw = (await readBody(req)) || "";
      let b; try { b = JSON.parse(raw || "{}"); } catch { return send(res, 400, { ok: false, error: "Bad data." }); }
      if (b._hp) return send(res, 200, { ok: true, message: "Thanks! We got your report and will follow up." });
      if (!b.name || !b.contact) return send(res, 400, { ok: false, error: "Please enter your name and contact." });
      try {
        Tickets.add({ name: b.name, contact: b.contact, category: b.topic || "support", message: b.message || "" });
      } catch (e) {}
      try { const c = tg(), chat = tgChat(); if (c && chat) await c.sendMessage(chat, `\uD83C\uDFAB <b>NEW HELP REQUEST</b>\n${escapeHtml(b.name)} \u00b7 ${escapeHtml(b.contact)}\n${escapeHtml(b.topic || "")}: ${escapeHtml(b.message || "")}`); } catch {}
      return send(res, 200, { ok: true, message: "Thanks! We got your report and will follow up." });
    }

    // Start a real online payment via PayMongo or Xendit (returns a checkout URL the customer opens).
    if (pathname === "/api/pay/gateway" && req.method === "GET") {
      const gw = paymentGateway();
      return send(res, 200, { ok: true, configured: !!gw.configured, gateway: gw.configured ? gw.name : null, mode: gw.mode || "link" });
    }
    if (pathname === "/api/pay/status" && req.method === "GET") {
      const pi = (new URL(req.url, "http://localhost").searchParams.get("pi") || "").trim();
      if (!pi) return send(res, 400, { ok: false, error: "Missing payment id." });
      const gw = paymentGateway();
      if (!gw.configured || gw.name !== "paymongo" || !gw.getIntent) return send(res, 503, { ok: false, error: "Online payment not available." });
      try {
        const intent = await gw.getIntent(pi);
        return send(res, 200, { ok: true, status: intent.status, paid: intent.status === "succeeded" });
      } catch (e) {
        return send(res, 502, { ok: false, error: e.message });
      }
    }
    if (pathname === "/api/pay-start" && req.method === "POST") {
      ...
      const cust = findCustomerByPortalLogin(b);
      if (!cust) return send(res, 404, { ok: false, error: "Account not found. Check your first name and password (last name)." });
      let planId = Number(b.plan_id) || null;
      let amount = parseMoney(b.amount);
      const isIpoePrepaid = (cust.conn_type || "pppoe") === "ipoe" && (purpose === "renew" || purpose === "bill");
      if (isIpoePrepaid && !planId && cust.plan_id) planId = Number(cust.plan_id) || null;
      if (isIpoePrepaid && planId) {
        const plan = validatePrepaidPlanForCustomer(Plans.get(planId), cust);
        amount = Number(plan.price) || 0;
      }
      if (amount < 20) return send(res, 400, { ok: false, error: "Minimum online payment is \u20B120." });
      const gw = paymentGateway();
      if (!gw.configured) return send(res, 503, { ok: false, error: "Online payment isn't set up yet. Please use GCash/Maya manual payment for now." });
      const portalTag = (cust.account_code || cust.username || "") + ":" + purpose + (planId ? ":" + planId : "");
      const desc = (purpose === "topup" ? "Wallet top-up" : "Plan " + purpose) + " — " + cust.name + (planId ? " (#" + planId + ")" : "");
      try {
        // Portal self-service: always show inline QR Ph on our page (never redirect to PayMongo checkout).
        if (gw.name === "paymongo" && gw.createQrph) {
          const qr = await gw.createQrph({
            amountPhp: amount,
            description: desc,
            metadata: { portal: portalTag },
          });
          return send(res, 200, {
            ok: true, mode: "qrph", gateway: gw.name,
            payment_intent_id: qr.payment_intent_id, qr_image_url: qr.qr_image_url,
        ...
        return send(res, 502, { ok: false, error: "Could not start the online payment: " + e.message });
      }
    }
    ...
    if (pathname === "/api/pay/plans" && req.method === "GET") {
      const qs = new URL(req.url, "http://localhost").searchParams;
      const ct = (qs.get("type") || "").trim().toLowerCase();
      const prepaidOnly = qs.get("prepaid") === "1" || qs.get("active") === "1";
      let plans = prepaidOnly && ct ? Plans.listActiveForConn(ct) : Plans.list();
      if (ct) plans = plans.filter((p) => planMatchesConnTypePortal(p, ct));
      if (prepaidOnly) plans = plans.filter((p) => String(p.status || "active").toLowerCase() !== "inactive");
      return send(res, 200, { ok: true, plans: plans.map(portalPlanPayload) });
    }
    if (pathname === "/api/pay/qr" && req.method === "GET") {
      const qs = new URL(req.url, "http://localhost").searchParams;
      const invoiceId = Number(qs.get("invoice") || 0);
      if (!invoiceId) return send(res, 400, { ok: false, error: "Missing invoice id." });
      const inv = Invoices.get(invoiceId);
      if (!inv || inv.status === "paid") return send(res, 400, { ok: false, error: "Invoice not payable." });
      const gw = paymentGateway();
      if (!gw.configured) return send(res, 503, { ok: false, error: gw.notConfiguredMsg || "Online payment not set up." });
      try {
        const s = Settings.all();
        const desc = `${s.biz_name || "Internet"} ${inv.period} — ${inv.customer_name}`;
        const pay = await createGatewayPayment(gw, {
          amountPhp: inv.amount,
          description: desc,
          remarks: `INV-${inv.id}`,
          invoiceId: inv.id,
        });
        Invoices.setLink(inv.id, pay.id, pay.url || pay.checkout_url || "");
        return send(res, 200, {
          ok: true,
          mode: pay.mode,
          amount: inv.amount,
          ...
          invoice_id: inv.id,
          payment_intent_id: pay.payment_intent_id || pay.id,
          qr_image_url: pay.qr_image_url || "",
          url: pay.url || pay.checkout_url,
        });
      } catch (e) {
        return send(res, 502, { ok: false, error: "Could not create QR payment: " + e.message });
      }
    }
    if (pathname === "/api/pay/link" && req.method === "POST") {
      ...
      if (!inv || inv.status === "paid") return send(res, 400, { ok: false, error: "invoice not payable" });
      const gw = paymentGateway();
      if (!gw.configured) return send(res, 400, { ok: false, error: gw.notConfiguredMsg });
      // Reuse existing link (PayMongo checkout or portal URL).
      if (inv.payment_url && (gw.mode !== "qrph" || !isHostedCheckoutUrl(inv.payment_url))) {
        return send(res, 200, { ok: true, url: inv.payment_url, mode: gw.mode || "link" });
      }
      try {
        const s = Settings.all();
        const pay = await createGatewayPayment(gw, {
          amountPhp: inv.amount,
          description: `${s.biz_name || "Internet"} ${inv.period} — ${inv.customer_name}`,
          remarks: `INV-${inv.id}`,
          invoiceId: inv.id,
        });
        Invoices.setLink(inv.id, pay.id, pay.url || pay.checkout_url || "");
    ...
    // ---- Pay bill / renew using wallet balance (customer portal) ----
    if ((pathname === "/api/pay/renew-wallet" || pathname === "/api/pay/wallet") && req.method === "POST") {
      const raw = (await readBody(req)) || "";
      let b; try { b = JSON.parse(raw || "{}"); } catch { return send(res, 400, { ok: false, error: "Bad data." }); }
      let cust = findCustomerByPortalLogin(b);
      if (!cust) {
        const u = (b.account || b.username || "").trim().toLowerCase();
        const norm = (x) => String(x || "").toLowerCase().replace(/[\s:-]/g, "");
        cust = u ? Customers.list().find((c) => (c.username || "").toLowerCase() === u || (c.account_code || "").toLowerCase() === u || ("ipoe-" + (c.username || "")).toLowerCase() === u || norm(c.mac) === norm(u) || ("ipoe-" + String(c.id).padStart(4, "0")).toLowerCase() === u) : null;
      }
      if (!cust) return send(res, 404, { ok: false, error: "Account not found." });
      const purpose = ["renew", "bill"].includes(b.purpose) ? b.purpose : "bill";
      const planId = Number(b.plan_id) || null;
      try {
        const result = await payWithWalletCredit(cust.id, purpose, planId || null);
        return send(res, 200, { ok: true, ...result });
      } catch (e) {
        return send(res, 400, { ok: false, error: e.message || "Could not pay from wallet." });
      }
    }

    // Pay bill or renew using referral points (customer-initiated).
    if (pathname === "/api/pay/referral-redeem" && req.method === "POST") {
      const raw = (await readBody(req)) || "";
      let b; try { b = JSON.parse(raw || "{}"); } catch { return send(res, 400, { ok: false, error: "Bad data." }); }
      const code = String(b.code || "").trim();
      const last = String(b.last || b.password || "").trim().toLowerCase();
      const purpose = ["renew", "bill"].includes(b.purpose) ? b.purpose : "bill";
      const planId = Number(b.plan_id) || null;
      if ((!b.first && !b.first_name && !code) || !last) return send(res, 400, { ok: false, error: "Enter your first name and password (last name)." });
      const cust = findCustomerByPortalLogin(b);
      if (!cust) return send(res, 404, { ok: false, error: "Account not found. Check your first name and password (last name)." });
      try {
        const result = await redeemReferralPoints(cust.id, purpose, planId || null);
        return send(res, 200, { ok: true, ...result });
      } catch (e) {
        return send(res, 400, { ok: false, error: e.message || "Could not redeem points." });
      }
    }

    // Public AI chat bot (OpenAI / Anthropic — when enabled in Settings)
    if (pathname === "/api/chat/status" && req.method === "GET") {
      return send(res, 200, { ok: true, enabled: true, ai: aiEnabled(), provider: Settings.get("ai_provider", "") || "faq" });
    }
    if (pathname === "/api/chat" && req.method === "POST") {
      const raw = (await readBody(req)) || "";
      if (raw.length > 32_000) return send(res, 413, { ok: false, error: "Message too long." });
      const b = JSON.parse(raw || "{}");
      const message = String(b.message || "").trim();
      if (!message) return send(res, 400, { ok: false, error: "Please type a message." });
      const chatCtx = {
        message,
        history: Array.isArray(b.history) ? b.history : [],
        bizName: Settings.get("biz_name", ""),
        publicUrl: Settings.get("public_url", "") || portalPublicBase(),
        bizContact: Settings.get("biz_contact", ""),
        gcashName: Settings.get("gcash_name", ""),
        gcashNumber: Settings.get("gcash_number", ""),
      };
      if (aiEnabled()) {
        try {
          const reply = await customerChat(chatCtx);
          if (reply) return send(res, 200, { ok: true, reply, mode: "ai" });
        } catch (e) {
          Audit.add({ type: "auto", action: "chat-error", detail: String(e.message || e).slice(0, 500), ok: false });
        }
      }
      return send(res, 200, { ok: true, reply: faqChatReply(chatCtx), mode: "faq" });
    }
    ...
    if (pathname === "/help" && req.method === "GET") {
      return servePortalPage(res, "help.html");
    }
    ...
    if (pathname === "/api/license/status" && req.method === "GET") {
      ...
      const maxR = License.maxRoutersFor(lic);
      return send(res, 200, { ok: true, data: {
        valid: LICENSE_OK, customer: lic.customer || "", model: lic.model || "",
        issued: lic.issued || "", expires: lic.expires || null, daysLeft,
        tier: License.tierLabel(lic), maxRouters: maxR, routerCount: (() => { try { return Routers.list().length; } catch { return 0; } })(),
        machineId: License.machineId(), reason: LICENSE_STATE.reason || "",
      } });
    }
    if (pathname === "/api/auth/me" && req.method === "GET") {
      const enriched = user ? Auth.enrichUser(user) : null;
      const warn = (enriched && enriched.role === "admin") ? Auth.isDefaultAdminPassword() : false;
      return send(res, 200, { ok: true, user: enriched, defaultPassword: warn });
    }
    ...
    if (pathname === "/api/auth/users" && req.method === "GET") {
      ...
      return send(res, 200, { ok: true, data: Accounts.list(), routers: Routers.list().map(({ password, ...r }) => ({ id: r.id, name: r.name, area: r.area })) });
    }
    if (pathname === "/api/auth/users" && req.method === "POST") {
      ...
      const scope = b.scope === "sites" ? "sites" : "all";
      const router_ids = Array.isArray(b.router_ids) ? b.router_ids.map(Number).filter((n) => n > 0) : [];
      if (scope === "sites" && !router_ids.length) return send(res, 400, { ok: false, error: "Pick at least one site for scoped accounts." });
      const u = Accounts.create({ username, salt, hash, role: b.role, scope, map_private: !!b.map_private, router_ids });
      if (b.role === "agent") Agents.ensureForUser(u.id, username);
      Audit.add({ type: "auth", action: "user-create", detail: `${username} (${b.role}, scope=${scope})`, ok: true });
    ...
    if ((um = pathname.match(/^\/api\/auth\/users\/(\d+)\/role$/)) && req.method === "POST") {
      ...
      if (b.role === "agent") {
        ...
        if (acct) Agents.ensureForUser(id, acct.username);
      }
      return send(res, 200, { ok: true });
    }
    if ((um = pathname.match(/^\/api\/auth\/users\/(\d+)\/sites$/)) && req.method === "POST") {
      if (!need()) return denied();
      const id = Number(um[1]);
      const target = Accounts.getById(id);
      if (!target) return send(res, 404, { ok: false, error: "user not found" });
      if (target.username === "admin") return send(res, 400, { ok: false, error: "Admin account is always global." });
      const b = JSON.parse((await readBody(req)) || "{}");
      const scope = b.scope === "sites" ? "sites" : "all";
      const router_ids = Array.isArray(b.router_ids) ? b.router_ids.map(Number).filter((n) => n > 0) : [];
      if (scope === "sites" && !router_ids.length) return send(res, 400, { ok: false, error: "Pick at least one site." });
      Accounts.setScope(id, scope, !!b.map_private);
      Accounts.setSites(id, scope === "sites" ? router_ids : []);
      Auth.dropSessionsForUser(id);
      Audit.add({ type: "auth", action: "user-sites", detail: `${target.username}: scope=${scope}, sites=${router_ids.join(",")}`, ok: true });
      return send(res, 200, { ok: true, data: Accounts.getByIdFull(id) });
    }
    ...
    if ((pm = pathname.match(/^\/api\/proofs\/(\d+)\/approve$/)) && req.method === "POST") {
      ...
      if (p.status === "pending") {
        // Portal payments (no invoice, has an amount) credit the wallet + renew if it covers the plan.
        if (!p.invoice_id && Number(p.amount) > 0 && /PORTAL PAYMENT/i.test(p.note || "")) await approvePortalPaymentProof(p, "panel");
        else await applyApprovedProof(p, "panel");
      }
      return send(res, 200, { ok: true });
    }
    ...
    if (req.method === "GET" && pathname === "/api/users/status") {
      try {
        ...
        return send(res, 200, { ok: true, data: await usersStatus(routerId) });
      } catch (e) {
        return send(res, 500, { ok: false, error: e.message });
      }
    }
    if (req.method === "GET" && pathname === "/api/pppoe/profiles") {
      try {
        const routerId = new URL(req.url, "http://localhost").searchParams.get("router_id");
        return send(res, 200, { ok: true, data: await pppProfilesForRouter(routerId) });
      } catch (e) {
        return send(res, 500, { ok: false, error: e.message });
      }
    }

    if (req.method === "GET" && pathname === "/api/router/netsetup") {
      try {
        const routerId = new URL(req.url, "http://localhost").searchParams.get("router_id");
        return send(res, 200, { ok: true, data: await netSetupForRouter(routerId) });
      } catch (e) {
        return send(res, 500, { ok: false, error: e.message });
      }
    }
    ...
    if (req.method === "GET" && GET_ROUTES[pathname]) {
      if (user.role === "agent") return denied();
      try {
    ...
    if (req.method === "POST" && pathname === "/api/ai") {
      if (!need("technician", "helper")) return denied();
    ...
    if (pathname.startsWith("/api/billing")) {
      const isCustomers = pathname.startsWith("/api/billing/customers");
      const isCustomerTools = pathname === "/api/billing/expiry-sweep/run"
        || pathname === "/api/billing/broadcast"
        || /^\/api\/billing\/routers\/\d+\/ppp-profiles/.test(pathname);
      ...
      const isMapInfra = pathname.startsWith("/api/billing/olts")
        || pathname.startsWith("/api/billing/naps")
        || pathname.startsWith("/api/billing/fiber-lines");
      const isHelpdeskOps = pathname.startsWith("/api/billing/outages");
      ...
      if (req.method === "GET") {
        if (isRoutersRead) {
          if (!need("operator", "cashier", "admin")) return denied();
        } else if (isReferrals) {
          if (pathname === "/api/billing/referrals/me") {
            if (!need("technician", "staff", "helper", "cashier", "operator", "agent")) return denied();
          } else if (!need("technician", "staff", "helper", "cashier", "operator")) return denied();
        } else if (isAgents) {
          if (agentSelfPath) { if (!need("agent", "admin")) return denied(); }
          else if (!need("admin")) return denied();
        }
      }
      else if (isAdminOnly) { if (!need()) return denied(); }            // admin only
      else if (isReferrals) { if (!need()) return denied(); }           // referral writes: admin only
      else if (isAgents) {
        if (agentSelfPath) { if (!need("agent", "admin")) return denied(); }
        else if (!need("admin")) return denied();
      }
      ...
      else if (isRoutersRead) { if (!need("operator", "cashier", "admin")) return denied(); }
    ...
    if (pathname.startsWith("/api/vendos")) {
      if (req.method !== "GET" && !need("technician", "helper")) return denied();
...
async function handleVendos(req, res, pathname) {
  ...
  try {
    ...
    // Discover NodeMCU boards from DHCP leases (hostname WIZ*/ESP*). ?all=1 scans all routers.
    if (sub === "/discover" && method === "GET") {
      const q = Object.fromEntries(new URL(req.url, "http://localhost").searchParams);
      const prefixes = (Settings.get("nodemcu_prefixes", "WIZNET,WIZ,ESP") || "WIZNET,WIZ,ESP").split(",").map((s) => s.trim().toUpperCase()).filter(Boolean);
      const showAll = !!q.showAll;   // fallback: list every bound lease so boards w/o a hostname can be added
      const targets = q.all ? Routers.list().filter((r) => r.enabled !== 0 && r.host)
        : [ q.router ? Routers.get(Number(q.router)) : (Routers.getDefault() || null) ].filter(Boolean);
      const scan = async (conn, leases, rid, rname) => {
        if (!showAll) return scanLeasesForNodeMcu(conn, leases, prefixes, rid, rname);
        // showAll: return all leases as candidates (no ping, to keep it fast)
        const have = new Set((Vendos.list() || []).map((v) => String(v.mac || "").toUpperCase()).filter(Boolean));
        return (leases || []).map((l) => {
          const mac = String(l["mac-address"] || l.mac || "").toUpperCase();
          const ip = l.address || l["active-address"] || "";
          return { name: l["host-name"] || l.hostname || ("device-" + mac.slice(-5)), mac, ip,
            bound: l.status === "bound" || !!l["active-address"], reachable: false, online: l.status === "bound",
            router_id: rid, router_name: rname, alreadyVendo: have.has(mac) };
        });
      };
      if (!targets.length) {
        try { const leases = await mt.dhcpLeases(); const found = await scan(mt, leases, null, null); return ok({ candidates: found, scanned: 1, showAll }); }
        catch (e) { return send(res, 502, { ok: false, error: e.message }); }
      }
      const all = [];
      for (const r of targets) {
        try { const conn = connForRouter(r); const leases = await conn.dhcpLeases(); const found = await scan(conn, leases, r.id, r.name); all.push(...found); }
        catch (e) { all.push({ error: e.message, router_id: r.id, router_name: r.name }); }
      }
      return ok({ candidates: all, scanned: targets.length, showAll });
    }
    // Live online/offline status for NodeMCU vendos (lease bound + reachable via router ping).
    if (sub === "/nodemcu-status" && method === "GET") {
      const list = (Vendos.list() || []).filter((v) => v.kind === "nodemcu" || /^(WIZ|ESP)/i.test(v.name || ""));
      const out = [];
      // Group by router so we read each router's leases + ARP once (same data discover uses).
      const byRouter = new Map();
      for (const v of list) {
        const key = v.router_id || 0;
        if (!byRouter.has(key)) byRouter.set(key, []);
        byRouter.get(key).push(v);
      }
      for (const [rid, vendos] of byRouter) {
        let conn = null, leases = [], arps = [], err = null;
        try {
          const r = rid ? Routers.get(rid) : Routers.getDefault();
          conn = r ? connForRouter(r) : mt;
          leases = (await conn.dhcpLeases().catch(() => [])) || [];
          arps = (await conn.arpTable?.().catch(() => [])) || [];
        } catch (e) { err = e.message; }
        for (const v of vendos) {
          const mac = String(v.mac || "").toUpperCase();
          // 1) lease present & bound (match by MAC OR by IP — same forgiving match as discover)
          const lease = leases.find((l) => (mac && String(l["mac-address"] || "").toUpperCase() === mac) || (v.ip && (l.address === v.ip || l["active-address"] === v.ip)));
          const bound = lease ? (lease.status === "bound" || lease.status === "active" || !!lease["active-address"]) : false;
          // 2) ARP entry (active on the LAN — catches static-IP boards)
          const arp = arps.some((a) => (mac && String(a["mac-address"] || "").toUpperCase() === mac) || (v.ip && a.address === v.ip));
          // 3) ping from the router (last resort; many boards block ICMP)
          let reachable = false;
          if (conn && v.ip) { try { const p = await conn.pingHost(v.ip, 2); reachable = !!p.reachable; } catch {} }
          const online = bound || arp || reachable;
          try { Vendos.setStatus && Vendos.setStatus(v.id, online); } catch {}
          out.push({ id: v.id, name: v.name, ip: v.ip, mac: v.mac, bound, arp, reachable, online, error: err });
        }
      }
      return ok({ vendos: out });
    }
    if ((mm = sub.match(/^\/(\d+)$/))) {
      const id = Number(mm[1]);
      if (method === "PUT") return ok(Vendos.update(id, body));
      if (method === "PATCH") {
        // partial update — merge onto the existing record so we don't wipe other fields
        const cur = Vendos.get(id);
        if (!cur) return send(res, 404, { ok: false, error: "Vendo not found" });
        return ok(Vendos.update(id, { ...cur, ...body }));
      }
...
async function importUsers(source) {
  ...
  else rows = await listAllPppoeSecrets();
  ...
  for (const r of rows) {
    ...
    const disabled = String(r.disabled) === "true" || String(r.disabled) === "yes";
    ...
    if (existing) {
      ...
      if (r.password && !existing.password) patch.password = r.password;
      if (comment && (existing.name === existing.username || existing.name === name)) patch.name = comment;
      if (r._router_id && !existing.router_id) patch.router_id = r._router_id;
      // Do not mark expired billing accounts active just because MikroTik still shows a plan profile.
      if (status === "suspended") patch.status = "suspended";
      else if (!Customers.isExpiredAsOf(existing.expiry, billingNow())) patch.status = "active";
      Customers.update(existing.id, patch);
      matched++;
      continue;
    }
    Customers.create({
      name: comment || name, username: name, password: r.password || "",
      plan_id,
      status,
      router_id: r._router_id || null,
      notes: `imported from ${source}${r.profile ? " · profile " + r.profile : ""}`,
    });
    existingByUser[name.toLowerCase()] = { username: name };
    created++;
  }
  Audit.add({ type: "manual", action: "import-users", detail: `${source}: ${created} created, ${matched} matched, ${skipped} skipped`, ok: true });
  return { source, created, matched, skipped, total: rows.length };
}
...
function welcomePageHtml() {
  ...
  <script src="/portal/chat-bot.js"></script>
  </body></html>`;
}
...
function applyPageHtml() {
  ...
  <div class="wrap">
    ...
    <div class="card" id="form-card"><div class="pad">
      ...
      <label>Facebook name (optional)</label><input id="f-facebook" placeholder="e.g. Juan Dela Cruz">
  ...
  <script>
    ...
    async function submitApply(btn){
      ...
      var body={name:name,contact:contact,email:$("f-email").value.trim(),facebook:$("f-facebook").value.trim(),address:$("f-address").value.trim(),area:$("f-area").value.trim(),
...
// Build the receipt/invoice number for a payment, using the configurable range + prefix in Settings.
//   receipt_prefix : text before the number (e.g. "INV-", "JNS-", "" for none)
//   receipt_title  : the big label on the receipt (e.g. "INVOICE", "OFFICIAL RECEIPT")
//   receipt_start  : first number in the series (e.g. 1000)
//   receipt_end    : last number (e.g. 1999); 0/blank = no end limit
//   receipt_pad    : zero-pad width (e.g. 5 → 01000); blank = auto from the start value width
// The number is stable per payment: number = start + (payment.id - firstPaymentId). We compute the
// offset from the earliest payment id so the very first receipt equals `receipt_start`.
function receiptNumberFor(p) {
  const s = Settings.all();
  const prefix = s.receipt_prefix != null ? s.receipt_prefix : "OR-";
  const start = Math.max(0, parseInt(s.receipt_start || "1", 10) || 1);
  const end = parseInt(s.receipt_end || "0", 10) || 0;
  // offset: how many payments before this one (so the first-ever payment maps to `start`)
  let base = 1;
  try { base = Payments.minId(); } catch {}
  const seq = start + (Number(p.id) - base);
  const pad = parseInt(s.receipt_pad || "", 10) || String(start).length;
  const numStr = String(Math.max(seq, 0)).padStart(pad, "0");
  const overEnd = end > 0 && seq > end;
  const nearEnd = end > 0 && !overEnd && (end - seq) <= 20;
  return { full: prefix + numStr, seq, prefix, numStr, overEnd, nearEnd, end, start };
}
function receiptTitle() { const t = Settings.get("receipt_title", "OFFICIAL RECEIPT"); return t || "OFFICIAL RECEIPT"; }

function receiptHtml(p) {
  const s = Settings.all();
  const biz = s.biz_name || "Internet Service";
  const E = (x) => escapeHtml(x == null ? "" : x);
  const rn = receiptNumberFor(p);
  const no = rn.full;
  ...
  <div class="r">
    <div class="top">
      ...
      <div class="tag">${E(title)}<br><b>${E(no)}</b><br><span style="color:#666;font-weight:400">${E(when)}</span></div>
    </div>
...
const CLOUD_VOUCHER_PROFILE = "Cloud-server";

async function issueCloudRadiusVoucher({ code, password, limitUptime = "1h", planId = 0, planName = "", amount = 0, paymentRef = "" }) {
  const pass = String(password || code || "").trim();
  const uptime = String(limitUptime || "1h").trim();
  const totalSeconds = VoucherOrders.uptimeToSecs(uptime);
  const existing = VoucherOrders.byCode(code);
  if (existing && existing.status === "issued") return existing;
  let order = existing;
  if (!order) {
    order = VoucherOrders.create({
      ...
      contact: "",
      limit_uptime: uptime,
      profile: CLOUD_VOUCHER_PROFILE,
    });
  }
  if (order.status === "pending") VoucherOrders.setPaid(order.id, paymentRef || "");
  VoucherOrders.update(order.id, { profile: CLOUD_VOUCHER_PROFILE, plan_name: planName || order.plan_name });
  VoucherOrders.setIssued(order.id, code, pass, totalSeconds);
  try {
    const { execSync } = await import("node:child_process");
    execSync("/usr/local/bin/jmwifi-radius-sync", { stdio: "ignore" });
  } catch (e) {
    Audit.add({ type: "auto", action: "voucher-radius-sync", detail: code + ": " + (e.message || String(e)), ok: false });
  }
  return VoucherOrders.get(order.id);
}
...
async function createHotspotVoucher({ code, password = "", profile = "default", uptime = "", routerId = null, source = "voucher", planId = 0, planName = "", amount = 0, paymentRef = "" }) {
  if (["online", "panel", "voucher"].includes(String(source || ""))) {
    ...
    return { code, central: true, radius: true, profile: CLOUD_VOUCHER_PROFILE, dryRun: false };
  }
  const limitSecs = uptimeToSecs(uptime);
  if (hotspotCentralEnabled()) {
    HotspotCentral.create({
      username: code, password: password || "", profile: profile || "default",
      limitSecs: limitSecs || 3600, source, routerId: routerId || null,
    });
    return { code, central: true, dryRun: false };
  }
  let rid = routerId;
  if (!rid) { const def = Routers.getDefault(); rid = def && def.id ? def.id : null; }
  const r = rid ? Routers.get(rid) : null;
  const conn = r ? connForRouter(r) : mt;
  await conn.createHotspotUser({ name: code, password: password || "", profile: profile || "default", limitUptime: uptime || "" });
  return { code, central: false, dryRun: !!RouterOSAPI.dryRun };
}

async function fulfillOnlineVoucher({ planId, routerId, ref, amountPeso }) {
  const plan = Plans.get(planId);
  if (!plan || String(plan.type || "").toLowerCase() !== "hotspot") throw new Error("Invalid hotspot plan.");
  const refTag = "ref:" + (ref || "");
  const prior = (Audit.list(400) || []).find((a) => a.action === "voucher-online" && String(a.detail || "").includes(refTag));
  if (prior) {
    const cm = String(prior.detail || "").match(/code=([A-Z0-9]+)/i);
    return { code: cm ? cm[1] : null, plan: plan.name, skipped: true };
  }
  const uptime = minsToHotspotUptime(plan.validity_mins || (Number(plan.validity_days) || 0) * 1440);
  let code = genVoucherCode(8, "JM");
  let lastErr = null;
  for (let i = 0; i < 5; i++) {
    try {
...
async function handleRouter(req, res, pathname) {
  ...
  try {
    switch (sub) {
      case "/pppoe/add": { const { conn } = resolveRouterCtx(b.router_id); return ok(await conn.createPppoe(b)); }
      case "/pppoe/delete": { const { conn } = resolveRouterCtx(b.router_id); return ok(await conn.deletePppoe(b.name)); }
      case "/pppoe/edit": return ok(await pppoeEditOnRouter(b));
      case "/pppoe/disable": { const { conn } = resolveRouterCtx(b.router_id); return ok(await conn.setPppoeDisabled(b.name, true)); }
      case "/pppoe/enable": { const { conn } = resolveRouterCtx(b.router_id); return ok(await conn.setPppoeDisabled(b.name, false)); }
      ...
      case "/hotspot/vouchers": {
        ...
        const profile = CLOUD_VOUCHER_PROFILE;
        ...
        if (vRouterId) {
          ...
          vRouterName = vr.name;
          if (!hotspotCentralEnabled()) {
            try { const vconn = connForRouter(vr); await vconn.identity(); } catch (e) { return send(res, 502, { ok: false, error: "Can't reach " + vr.name + ": " + e.message }); }
          }
        }
        const made = [], errors = [];
        for (let i = 0; i < count; i++) {
          ...
          try {
            await createHotspotVoucher({ code, password, profile, uptime: b.uptime || "", routerId: vRouterId, source: "panel" });
            made.push(code);
          } catch (e) { errors.push(e.message); }
        }
        Audit.add({ type: "manual", action: "vouchers", detail: `${made.length}x ${profile}${userOnly ? " (user-only)" : ""}${hotspotCentralEnabled() ? " central" : vRouterName ? " @ " + vRouterName : ""}`, ok: true });
        return ok({ created: made, count: made.length, profile, uptime: b.uptime || "", price: b.price || "", exp: b.exp || "", userOnly, errors, router: vRouterName, central: hotspotCentralEnabled(), dryRun: hotspotCentralEnabled() ? false : !!RouterOSAPI.dryRun });
      }
      ...
      case "/ppp-profile/add": { const { conn } = resolveRouterCtx(b.router_id); return ok(await conn.createPppProfile(b)); }
      case "/ppp-profile/edit": { const { conn } = resolveRouterCtx(b.router_id); return ok(await conn.updatePppProfile(b.name, b)); }
      case "/hotspot-profile/add": { const { conn } = resolveRouterCtx(b.router_id); return ok(await conn.createHotspotProfile(b)); }
      case "/hotspot-profile/edit": { const { conn } = resolveRouterCtx(b.router_id); return ok(await conn.updateHotspotProfile(b.name, b)); }
...
async function handleBilling(req, res, pathname) {
  ...
  const ok = (data) => send(res, 200, { ok: true, data });
  const cookies = Auth.parseCookies(req);
  const panelUser = Auth.sessionUser(cookies.sid);
  const panelRole = panelUser?.role || "";
  const scopedUser = Auth.enrichUser(panelUser);
  const siteScoped = Auth.isSiteScoped(scopedUser);
  const scopedIds = siteScoped ? Auth.userScope(scopedUser).router_ids : null;
  const scopedAreas = siteScoped ? scopedRouterAreas(scopedIds) : null;
  const filterCustomers = (list) => (siteScoped ? filterByRouterIds(list, scopedIds, (c) => c.router_id) : list);
  const guardRouter = (rid) => guardRouterAccess(scopedUser, rid, send, res);
  const customerInScope = (c) => !siteScoped || (c && c.router_id && scopedIds.includes(Number(c.router_id)));
  const jobOrderInScope = (jo) => {
    if (!siteScoped) return true;
    if (jo.customer_id) {
      const c = Customers.get(jo.customer_id);
      return customerInScope(c);
    }
    return jo.area && scopedAreas.has(String(jo.area).trim());
  };
  const getRouterScoped = (id) => {
    const r = Routers.get(Number(id));
    if (!r) { send(res, 404, { ok: false, error: "Router not found." }); return null; }
    if (guardRouter(r.id)) return null;
    return r;
  };
  const isPersonalReferralRole = ["technician", "staff", "helper"].includes(panelRole);
  const isAgentRole = panelRole === "agent";
  const isAdminRole = panelRole === "admin";

  try {
    let mm;
    if (sub === "/summary" && method === "GET") return ok(summary(siteScoped ? scopedIds : null));
    ...
    if (sub === "/hotspot-central/status" && method === "GET") {
      return ok({
        ...
        stats: HotspotCentral.stats(),
        radius_port: Number(Settings.get("radius_port", "1812")) || 1812,
        radius_host: Settings.get("radius_host", "") || "",
      });
    }
    if (sub === "/hotspot-central/setup" && method === "GET") {
      const pub = String(Settings.get("public_url") || "").trim().replace(/^https?:\/\//, "").split("/")[0];
      const host = Settings.get("radius_host", "") || pub.split(":")[0] || "187.77.145.131";
      const script = mikrotikCentralRadiusScript({
        radiusHost: host,
        secret: Settings.get("radius_secret", "") || "jmwifi-radius",
        authPort: Number(Settings.get("radius_port", "1812")) || 1812,
      });
      return ok({ script, radius_host: host, note: "Safe setup: adds RADIUS only. Does NOT remove /ip/hotspot/user or /ip/hotspot/user/profile entries." });
    }
    if (sub === "/hotspot-central/import" && method === "POST") {
      if (!isAdminRole) return send(res, 403, { ok: false, error: "Admin only." });
      const rid = body.router_id ? Number(body.router_id) : null;
      const targets = rid ? [Routers.get(rid)].filter(Boolean) : Routers.list().filter((r) => r.enabled !== 0);
      let imported = 0, skipped = 0, errors = [];
      for (const r of targets) {
        try {
          const conn = connForRouter(r);
          const users = (await conn.listHotspotUsers()) || [];
          for (const u of users) {
            const lim = uptimeToSecs(u["limit-uptime"] || "");
            const used = uptimeToSecs(u.uptime || "");
            const rem = lim > 0 ? Math.max(0, lim - used) : lim;
            const res = HotspotCentral.importUser({
              username: u.name, password: u.password || "", profile: u.profile || "default",
              limitSecs: rem || lim || 3600, routerId: r.id, note: "imported from " + r.name,
            });
      ...
      Audit.add({ type: "manual", action: "hotspot-central-import", detail: `imported ${imported}, skipped ${skipped}`, ok: true });
      return ok({ imported, skipped, errors, note: "Read-only copy from routers — nothing deleted on MikroTik." });
    }
    if (sub === "/hotspot-central/accounts" && method === "GET") {
      return ok(HotspotCentral.list(Number(q.limit) || 200));
    }
    if (sub === "/hotspot-central/packages" && method === "GET") {
      const packs = Plans.list().filter((p) => String(p.type || "").toLowerCase() === "hotspot").map(adminHotspotPackage);
      return ok({ packages: packs, central: hotspotCentralEnabled() });
    }
    if (sub === "/hotspot-central/packages" && method === "POST") {
      if (!isAdminRole) return send(res, 403, { ok: false, error: "Admin only." });
      const body2 = { ...body, type: "hotspot", client_type: "hotspot" };
      if (!body2.name) return send(res, 400, { ok: false, error: "Package name required." });
      // speed = display label; download/upload = real MT rate (hidden from clients)
      if (body2.display_speed != null) body2.speed = body2.display_speed;
      return ok(adminHotspotPackage(Plans.create(body2)));
    }
    if ((mm = m(/^\/hotspot-central\/packages\/(\d+)$/))) {
      const pid = Number(mm[1]);
      if (method === "PUT") {
        if (!isAdminRole) return send(res, 403, { ok: false, error: "Admin only." });
        const patch = { ...body, type: "hotspot", client_type: "hotspot" };
        if (patch.display_speed != null) patch.speed = patch.display_speed;
        const updated = Plans.update(pid, patch);
        if (!updated) return send(res, 404, { ok: false, error: "Package not found." });
        return ok(adminHotspotPackage(updated));
      }
      if (method === "DELETE") {
        if (!isAdminRole) return send(res, 403, { ok: false, error: "Admin only." });
        Plans.remove(pid);
    ...
    // Customers
    if (sub === "/customers" && method === "GET") return ok(filterCustomers(Customers.list()));
    if (sub === "/map" && method === "GET") {
      if (siteScoped) return ok(mapDataForScope(scopedIds));
      const olts = Olts.list(), naps = Naps.list(), lines = FiberLines.list();
    ...
    if (sub === "/naps" && method === "GET") {
      const allNaps = Naps.list();
      if (!siteScoped) return ok(allNaps);
      const map = mapDataForScope(scopedIds);
      return ok(map.naps);
    }
    ...
    if (sub === "/routers/health" && method === "GET") {
      if (q.refresh) { try { await pollAllRouters(); } catch {} }
      let routers = Routers.list();
      if (siteScoped) routers = routers.filter((r) => scopedIds.includes(r.id));
      const list = routers.map(({ password, ...r }) => ({ id: r.id, name: r.name, host: r.host, area: r.area, enabled: r.enabled, is_default: r.is_default, clients: r.clients, suspended: r.suspended, health: _routerHealth[r.id] || null }));
      const upCount = list.filter((r) => r.health && r.health.up).length;
      return ok({ routers: list, up: upCount, total: list.length, onlineClients: list.reduce((s, r) => s + ((r.health && r.health.online) || 0), 0) });
    }
    if (sub === "/routers/health/refresh" && method === "POST") {
      try { await pollAllRouters(); } catch (e) { return send(res, 502, { ok: false, error: e.message }); }
      return ok({ ok: true });
    }
    // Per-router hotspot profiles (for the voucher generator's profile picker).
    if ((mm = m(/^\/routers\/(\d+)\/hotspot-profiles$/)) && method === "GET") {
      const r = getRouterScoped(mm[1]); if (!r) return;
      try {
        const conn = connForRouter(r);
        const profs = (await conn.hotspotUserProfiles()) || [];
        return ok((profs || []).map((p) => ({
          name: p.name,
    ...
    if ((mm = m(/^\/routers\/(\d+)\/hotspot-profiles$/)) && method === "POST") {
      const r = getRouterScoped(mm[1]); if (!r) return;
      if (!body.name) return send(res, 400, { ok: false, error: "Package name is required." });
      try {
        const conn = connForRouter(r);
        if (body._edit) await conn.updateHotspotProfile(body.name, body);
        else await conn.createHotspotProfile(body);
        Audit.add({ type: "manual", action: body._edit ? "hotspot-package-edit" : "hotspot-package-add", detail: `${body.name} @ ${r.name}`, ok: true });
        return ok({ ok: true, name: body.name, dryRun: !!RouterOSAPI.dryRun });
      } catch (e) { return send(res, 502, { ok: false, error: e.message }); }
    }
    // Per-router PPPoE profiles: list / add / edit / delete.
    if ((mm = m(/^\/routers\/(\d+)\/ppp-profiles$/)) && method === "GET") {
      const r = getRouterScoped(mm[1]); if (!r) return;
      try {
        const conn = connForRouter(r);
        const profs = (await conn.pppProfiles()) || [];
        return ok((profs || []).map((p) => ({ name: p.name, rateLimit: p["rate-limit"] || "", localAddress: p["local-address"] || "", remoteAddress: p["remote-address"] || "" })));
      } catch (e) { return send(res, 502, { ok: false, error: e.message }); }
    }
    if ((mm = m(/^\/routers\/(\d+)\/ppp-profiles$/)) && method === "POST") {
      const r = getRouterScoped(mm[1]); if (!r) return;
      if (!body.name) return send(res, 400, { ok: false, error: "Profile name is required." });
      try {
        const conn = connForRouter(r);
        if (body._edit) await conn.updatePppProfile(body.name, body);
        else await conn.createPppProfile(body);
        Audit.add({ type: "manual", action: body._edit ? "ppp-profile-edit" : "ppp-profile-add", detail: `${body.name} @ ${r.name}`, ok: true });
        return ok({ ok: true, name: body.name, dryRun: !!RouterOSAPI.dryRun });
      } catch (e) { return send(res, 502, { ok: false, error: e.message }); }
    }
    if ((mm = m(/^\/routers\/(\d+)\/ppp-profiles\/([^/]+)$/)) && method === "DELETE") {
      const r = getRouterScoped(mm[1]); if (!r) return;
      const name = decodeURIComponent(mm[2]);
      try {
        const conn = connForRouter(r);
        await conn.deletePppProfile(name);
        Audit.add({ type: "manual", action: "ppp-profile-delete", detail: `${name} @ ${r.name}`, ok: true });
        return ok({ ok: true });
      } catch (e) { return send(res, 502, { ok: false, error: e.message }); }
    }
    // Per-router manual hotspot user add (with chosen profile).
    if ((mm = m(/^\/routers\/(\d+)\/hotspot-user$/)) && method === "POST") {
      const r = getRouterScoped(mm[1]); if (!r) return;
      if (!body.name) return send(res, 400, { ok: false, error: "Username is required." });
      try {
        const out = await createHotspotVoucher({
          code: body.name, password: body.password || "", profile: body.profile || "default",
          uptime: body.uptime || "", routerId: r.id, source: "manual",
        });
        Audit.add({ type: "manual", action: "hotspot-user-add", detail: `${body.name} (${body.profile || "default"}) @ ${hotspotCentralEnabled() ? "central" : r.name}`, ok: true });
        return ok({ ok: true, name: body.name, central: !!out.central, dryRun: !!out.dryRun });
      } catch (e) { return send(res, 502, { ok: false, error: e.message }); }
    }
    // List hotspot users on a router (for the manage view's editable list).
    if ((mm = m(/^\/routers\/(\d+)\/hotspot-users$/)) && method === "GET") {
      const r = getRouterScoped(mm[1]); if (!r) return;
      try {
        const conn = connForRouter(r);
        const [users, active] = await Promise.all([ conn.listHotspotUsers(), conn.listHotspotActive().catch(() => []) ]);
        const activeNames = new Set((active || []).map((a) => a.user));
        return ok((users || []).map((u) => ({
          name: u.name, profile: u.profile || "default", password: u.password || "",
          limitUptime: u["limit-uptime"] || "", uptime: u.uptime || "", bytesIn: u["bytes-in"] || 0, bytesOut: u["bytes-out"] || 0,
          disabled: u.disabled === "true" || u.disabled === true, online: activeNames.has(u.name),
          comment: u.comment || "",
        })));
      } catch (e) { return send(res, 502, { ok: false, error: e.message }); }
    }
    // Edit a hotspot user (name/password/profile/uptime).
    if ((mm = m(/^\/routers\/(\d+)\/hotspot-users\/([^/]+)$/)) && (method === "PATCH" || method === "PUT")) {
      const r = getRouterScoped(mm[1]); if (!r) return;
      const oldName = decodeURIComponent(mm[2]);
      try {
        const conn = connForRouter(r);
        await conn.updateHotspotUser(oldName, { name: body.name, password: body.password, profile: body.profile, limitUptime: body.uptime });
        Audit.add({ type: "manual", action: "hotspot-user-edit", detail: `${oldName} @ ${r.name}`, ok: true });
        return ok({ ok: true, dryRun: !!RouterOSAPI.dryRun });
      } catch (e) { return send(res, 502, { ok: false, error: e.message }); }
    }
    // Delete a hotspot user.
    if ((mm = m(/^\/routers\/(\d+)\/hotspot-users\/([^/]+)$/)) && method === "DELETE") {
      const r = getRouterScoped(mm[1]); if (!r) return;
      const name = decodeURIComponent(mm[2]);
      try {
        const conn = connForRouter(r);
        await conn.deleteHotspotUser(name);
        Audit.add({ type: "manual", action: "hotspot-user-delete", detail: `${name} @ ${r.name}`, ok: true });
        return ok({ ok: true, dryRun: !!RouterOSAPI.dryRun });
      } catch (e) { return send(res, 502, { ok: false, error: e.message }); }
    }
    // Per-router live data for the expandable drop-down: device info + PPPoE/hotspot/IPoE customers.
    if ((mm = m(/^\/routers\/(\d+)\/live$/)) && method === "GET") {
      const r = getRouterScoped(mm[1]); if (!r) return;
      const out = { id: r.id, name: r.name, host: r.host, area: r.area, reachable: false, info: {}, pppoe: [], ipoe: [], vendos: [], error: null };
      // vendos (JuanFi) assigned to this router/site
      try { out.vendos = (Vendos.byRouter(r.id) || []).map((v) => ({ id: v.id, name: v.name, ip: v.ip, online: !!v.online, last_seen: v.last_seen || "" })); } catch {}
      // customers assigned to this router (from our DB) — these we can show even if router is down
      const mine = (Customers.list() || []).filter((c) => c.router_id === r.id);
      out.dbCustomers = mine.map((c) => ({ id: c.id, name: c.name, username: c.username, conn_type: c.conn_type || "pppoe",
        plan_name: c.plan_name || "", plan_price: c.plan_price || 0, status: c.status, expiry: c.expiry || "",
        credit: Number(c.credit || 0), account_code: c.account_code || "", mac: c.mac || "", static_ip: c.static_ip || "" }));
      try {
        const conn = connForRouter(r);
        const ident = await conn.identity();
        out.reachable = true;
        out.info.identity = (ident && (ident.name || ident)) || r.name;
        try { const rb = await conn.routerboard(); if (rb) { out.info.model = rb.model || rb["board-name"] || ""; out.info.serial = rb["serial-number"] || ""; out.info.firmware = rb["current-firmware"] || ""; } } catch {}
        try { const res = await conn.systemResource(); if (res) {
          out.info.cpu = res["cpu-load"] != null ? Number(res["cpu-load"]) : null;
          out.info.uptime = res.uptime || "";
          out.info.version = res.version || "";
          out.info.board = res["board-name"] || out.info.model || "";
          const free = Number(res["free-memory"] || 0), total = Number(res["total-memory"] || 0);
          if (total) { out.info.memUsedPct = Math.round((1 - free / total) * 100); out.info.memTotalMB = Math.round(total / 1048576); }
        } } catch {}
        // live PPPoE (secrets + who's active)
        try {
          const secrets = (await conn.listPppoe()) || [];
          const active = new Set(((await conn.listPppoeActive()) || []).map((a) => a.name));
          out.pppoe = secrets.map((s) => ({ name: s.name, profile: s.profile || "", disabled: s.disabled === "true" || s.disabled === true, online: active.has(s.name), comment: s.comment || "" }));
        } catch (e) { out.pppoeError = e.message; }
        // live hotspot users (treated under IPoE/hotspot tab)
        try {
          const hs = (await conn.listHotspotUsers()) || [];
          const hsActive = new Set(((await conn.listHotspotActive()) || []).map((a) => a.user || a.name));
          out.hotspot = hs.map((u) => ({ name: u.name, profile: u.profile || "", disabled: u.disabled === "true" || u.disabled === true, online: hsActive.has(u.name) }));
        } catch (e) { out.hotspotError = e.message; }
      } catch (e) { out.error = e.message; }
      return ok(out);
    }
    if (sub === "/routers" && method === "POST") {
      if (siteScoped) return send(res, 403, { ok: false, error: "Only the main admin can add routers." });
      if (!body.name) return send(res, 400, { ok: false, error: "Router name is required." });
      Audit.add({ type: "manual", action: "router-add", detail: body.name + " (" + (body.host || "") + ")", ok: true });
      const { password, ...r } = Routers.create(body);
      try { rescheduleRouterPoll(); } catch {}
      return ok({ ...r, hasPassword: !!password });
    }
    if ((mm = m(/^\/routers\/(\d+)$/)) && (method === "PATCH" || method === "PUT")) {
      if (siteScoped) return send(res, 403, { ok: false, error: "Only the main admin can edit router settings." });
      try {
        const { password, ...updated } = Routers.update(Number(mm[1]), body);
        Audit.add({ type: "manual", action: "router-edit", detail: "#" + mm[1] + " → " + (updated.name || ""), ok: true });
        return ok({ ...updated, hasPassword: !!password });
      } catch (e) { return send(res, 400, { ok: false, error: e.message }); }
    }
    if ((mm = m(/^\/routers\/(\d+)$/)) && method === "DELETE") {
      if (siteScoped) return send(res, 403, { ok: false, error: "Only the main admin can delete routers." });
      Routers.remove(Number(mm[1]));
      delete _routerHealth[Number(mm[1])];
      try { rescheduleRouterPoll(); } catch {}
      Audit.add({ type: "manual", action: "router-delete", detail: "#" + mm[1], ok: true });
      return ok({ deleted: true });
    }
    if ((mm = m(/^\/routers\/(\d+)\/test$/)) && method === "POST") {
      const r = getRouterScoped(mm[1]); if (!r) return;
      try {
        const verdict = await testRouterConnection(r);
        Routers.setStatus(r.id, verdict.ok ? "ok" : ("fail: " + verdict.error));
        return ok(verdict);
      } catch (e) { Routers.setStatus(r.id, "fail: " + e.message); return ok({ ok: false, error: e.message }); }
    }
    if ((mm = m(/^\/customers\/(\d+)\/router$/)) && method === "POST") {
      // Full move: deprovision on old router, reassign, provision on new. body.force = "move anyway".
      // body.dbonly = just change the DB field (no hardware ops) — for first-time assignment of an
      // already-correctly-provisioned customer, or when you'll provision manually.
      const cid = Number(mm[1]);
      if (body.dbonly) { Customers.setRouter(cid, body.router_id); return ok({ ok: true, data: Customers.get(cid), message: "Router updated (database only — no hardware change)." }); }
      try {
        const r = await moveCustomerToRouter(cid, body.router_id, { force: !!body.force });
        if (!r.ok) return send(res, 409, { ok: false, error: r.error, oldErrors: r.oldErrors });
        return ok({ ok: true, data: Customers.get(cid), message: r.message, warned: r.warned, oldErrors: r.oldErrors });
      } catch (e) { return send(res, 400, { ok: false, error: e.message }); }
    }
    // Bulk reassign: move many customers from one router to another at once.
    if (sub === "/customers/bulk-router" && method === "POST") {
      const ids = Array.isArray(body.ids) ? body.ids.map(Number).filter(Boolean) : [];
      if (!ids.length) return send(res, 400, { ok: false, error: "No customers selected." });
      const out = { moved: [], failed: [] };
      for (const cid of ids) {
        try { const r = await moveCustomerToRouter(cid, body.router_id, { force: !!body.force });
          if (r.ok) out.moved.push(cid); else out.failed.push({ id: cid, error: r.error }); }
        catch (e) { out.failed.push({ id: cid, error: e.message }); }
      }
      Audit.add({ type: "manual", action: "bulk-move-router", detail: `${out.moved.length} moved, ${out.failed.length} failed → router ${body.router_id || "default"}`, ok: out.failed.length === 0 });
      return ok(out);
    }
    ...
    if (sub === "/report/cashflow" && method === "GET") {
      ...
      const routerId = q.get("router_id");
      return ok(Reports.cashflow(period, routerId));
    }
    // Sales per router/site (filterable by ?period=YYYY-MM, or all-time if omitted/period=all)
    if (sub === "/report/sales-by-router" && method === "GET") {
      const period = (q.period && q.period !== "all") ? q.period : null;
      const data = Reports.salesByRouter(period);
      if (siteScoped) data.routers = (data.routers || []).filter((r) => scopedIds.includes(r.router_id));
      return ok(data);
    }
    if (sub === "/report/profit-by-router" && method === "GET") {
      const period = (q.period && q.period !== "all") ? q.period : null;
      const data = Reports.profitByRouter(period);
      if (siteScoped) data.routers = (data.routers || []).filter((r) => scopedIds.includes(r.router_id));
      return ok(data);
    }
    if ((mm = m(/^\/routers\/(\d+)\/profit$/)) && method === "GET") {
      if (guardRouter(Number(mm[1]))) return;
      const period = (q.period && q.period !== "all") ? q.period : null;
      const all2 = Reports.profitByRouter(period);
      const row = (all2.routers || []).find((r) => r.router_id === Number(mm[1])) || { router_id: Number(mm[1]), pppoe: 0, ipoe: 0, vendo: 0, expenses: 0, income: 0, net: 0 };
      return ok({ period: all2.period, site: row });
    }
    ...
    if (sub === "/olts" && method === "GET") {
      if (!siteScoped) return ok(Olts.list());
      return ok(mapDataForScope(scopedIds).olts);
    }
    if (sub === "/olts" && method === "POST") {
      if (!body.name) return send(res, 400, { ok: false, error: "OLT name is required." });
      Audit.add({ type: "manual", action: "olt-add", detail: body.name, ok: true });
      return ok(Olts.create(body));
    }
    if ((mm = m(/^\/olts\/(\d+)$/)) && (method === "PATCH" || method === "PUT")) {
      try {
        const updated = Olts.update(Number(mm[1]), body);
        Audit.add({ type: "manual", action: "olt-edit", detail: "#" + mm[1] + " → " + (updated.name || ""), ok: true });
        return ok(updated);
      } catch (e) { return send(res, 400, { ok: false, error: e.message }); }
    }
    if ((mm = m(/^\/olts\/(\d+)$/)) && method === "DELETE") {
      Olts.remove(Number(mm[1]));
      Audit.add({ type: "manual", action: "olt-delete", detail: "#" + mm[1], ok: true });
      return ok({ deleted: true });
    }
    if (sub === "/fiber-lines" && method === "GET") {
      if (!siteScoped) return ok(FiberLines.list());
      return ok(mapDataForScope(scopedIds).fiber_lines);
    }
    if (sub === "/fiber-lines" && method === "POST") {
      const lt = String(body.line_type || "").toLowerCase();
      const color = String(body.color || "").trim();
      if (lt === "manual") {
        const path = Array.isArray(body.path) ? body.path : [];
        if (path.length < 2) return send(res, 400, { ok: false, error: "Manual line needs at least 2 points on the map." });
        for (const pt of path) {
          if (!Array.isArray(pt) || pt.length < 2 || !Number.isFinite(+pt[0]) || !Number.isFinite(+pt[1])) {
            return send(res, 400, { ok: false, error: "Each point must be [lat, lng]." });
          }
        }
        try {
          Audit.add({ type: "manual", action: "fiber-add", detail: `manual line (${path.length} pts)`, ok: true });
          return ok(FiberLines.create({ line_type: "manual", path: path.map((p) => [+p[0], +p[1]]), color, notes: body.notes || "" }));
        } catch (e) {
          return send(res, 400, { ok: false, error: e.message || "Could not save line." });
        }
      }
      const fk = String(body.from_kind || "").toLowerCase();
      const tk = String(body.to_kind || "").toLowerCase();
      if (!body.from_id || !body.to_id) return send(res, 400, { ok: false, error: "from_id and to_id are required." });
      if (lt === "olt_nap") {
        if (fk !== "olt" || tk !== "nap") return send(res, 400, { ok: false, error: "OLT→NAP line must be olt → nap." });
      } else if (lt === "nap_nap") {
        if (fk !== "nap" || tk !== "nap") return send(res, 400, { ok: false, error: "NAP→NAP line must be nap → nap." });
      } else return send(res, 400, { ok: false, error: "line_type must be olt_nap, nap_nap, or manual." });
      if (Number(body.from_id) === Number(body.to_id) && fk === tk) return send(res, 400, { ok: false, error: "Can't connect a node to itself." });
      Audit.add({ type: "manual", action: "fiber-add", detail: `${fk}#${body.from_id} → ${tk}#${body.to_id}`, ok: true });
      return ok(FiberLines.create({ ...body, line_type: lt, color }));
    }
    if ((mm = m(/^\/fiber-lines\/(\d+)$/)) && method === "PATCH") {
      const id = Number(mm[1]);
      const action = String(body.action || "").toLowerCase();
      if (action === "reverse") {
        const row = FiberLines.reverse(id);
        if (!row) return send(res, 404, { ok: false, error: "Line not found." });
        Audit.add({ type: "manual", action: "fiber-reverse", detail: "#" + id, ok: true });
        return ok(row);
      }
      const row = FiberLines.update(id, body);
      if (!row) return send(res, 404, { ok: false, error: "Line not found." });
      Audit.add({ type: "manual", action: "fiber-update", detail: "#" + id, ok: true });
      return ok(row);
    }
    if ((mm = m(/^\/fiber-lines\/(\d+)$/)) && method === "DELETE") {
      FiberLines.remove(Number(mm[1]));
      Audit.add({ type: "manual", action: "fiber-delete", detail: "#" + mm[1], ok: true });
      return ok({ deleted: true });
    }
    ...
    if (sub === "/customers" && method === "POST") {
      const initPts = body.referral_points;
      delete body.referral_points;
      const created = Customers.create(body);
      if (initPts != null && initPts !== "" && Number(initPts) > 0) {
        try { Referrals.setBalance(created.id, Number(initPts), "initial points on signup"); } catch {}
      }
    ...
    if ((mm = m(/^\/customers\/(\d+)$/))) {
      ...
      if (method === "PUT" || method === "POST") {
        const pts = body.referral_points;
        delete body.referral_points;
        const updated = Customers.update(cid, body);
        if (pts != null && pts !== "" && updated) {
          try { Referrals.setBalance(cid, Number(pts), "updated from client account"); } catch {}
        }
        if (updated && updated.expiry && updated.status === "active") {
          try { await maybeSuspendCustomerIfExpired(cid); } catch {}
        }
        return ok(Customers.get(cid));
      }
      if (method === "DELETE") {
        const c = Customers.get(cid);
        // Best-effort: remove the account from the router so it isn't orphaned. If the router is
        // offline or the account is already gone, we still delete from the panel (non-blocking).
        let removedFromRouter = false, routerNote = "";
        if (c) {
          try {
            const conn = connForCustomer(c);
            const isIpoe = (c.conn_type || "pppoe") === "ipoe";
            const isHotspot = (c.plan_type || "pppoe") === "hotspot";
            if (isIpoe) {
              if (c.mac) { await conn.leaseRemoveByMac(c.mac); removedFromRouter = true; }
            } else if (isHotspot) {
              if (c.username) { await conn.deleteHotspotUser(c.username); removedFromRouter = true; }
            } else {
              if (c.username) { await conn.deletePppoe(c.username); removedFromRouter = true; }
            }
          } catch (e) { routerNote = e.message; }
        }
        Customers.remove(cid);
        try { Audit.add({ type: "manual", customer_id: cid, customer_name: c ? c.name : "", action: "customer-delete", detail: removedFromRouter ? "removed from router + panel" : ("panel only" + (routerNote ? " (router: " + routerNote + ")" : "")), ok: true }); } catch {}
        return ok({ deleted: true, removedFromRouter, routerNote });
      }
    }
    if ((mm = m(/^\/customers\/(\d+)\/expiry$/)) && (method === "PATCH" || method === "PUT")) {
      const cid = Number(mm[1]);
      const cur = Customers.get(cid);
      if (!cur) return send(res, 404, { ok: false, error: "customer not found" });
      const expiry = body.expiry != null ? String(body.expiry || "") : "";
      Customers.setExpiry(cid, expiry);
      Audit.add({ type: "manual", customer_id: cid, customer_name: cur.name, action: "expiry-set", detail: expiry || "(cleared)", ok: true });
      let updated = Customers.get(cid);
      if (updated.expiry) {
        try { updated = (await maybeSuspendCustomerIfExpired(cid)) || updated; } catch (e) {
          Audit.add({ type: "auto", customer_id: cid, customer_name: cur.name, action: "suspend", detail: "after expiry set: " + e.message, ok: false });
        }
      }
      return ok(updated);
    }
    if ((mm = m(/^\/customers\/(\d+)\/(suspend|enable|provision|renew)$/)) && method === "POST") {
      const result = await customerAction(Number(mm[1]), mm[2]);
      return ok({ result, dryRun: !!RouterOSAPI.dryRun });
    }
    ...
    if (sub === "/expiry-sweep/run" && method === "POST") {
      return ok(await runExpirySweep());
    }
    if (sub === "/reconcile-active/run" && method === "POST") {
      return ok(await reconcileActiveCustomersOnRouter());
    }
    ...
    if (sub === "/portal/test" && method === "POST") {
      // Detailed connection test — tells us EXACTLY what's wrong (URL, DNS, auth, etc.)
      const cfg = _portalCfg();
      if (!cfg.url) return ok({ ok: false, step: "config", error: "No portal URL set." });
      if (!cfg.token) return ok({ ok: false, step: "config", error: "No portal token set." });
      const steps = {};
      steps.url = cfg.url;
      steps.tokenLength = cfg.token.length;
      // 1) hit the diag endpoint (no auth needed to read it; it reports match state)
      try {
        const d = await _portalGet(cfg.url, "/sync/diag?token=" + encodeURIComponent(cfg.token), cfg.token);
        steps.diagStatus = d.status;
        steps.diag = d.json;
      } catch (e) { steps.diagError = e.message; }
      // 2) try the real pull
      try {
        const r = await _portalGet(cfg.url, "/sync/pull", cfg.token);
        steps.pullStatus = r.status;
        steps.pullOk = r.json && r.json.ok === true;
        if (r.json && r.json.error) steps.pullError = r.json.error;
        if (r.json && Array.isArray(r.json.items)) steps.pullItems = r.json.items.length;
        if (!r.json && r.raw) steps.pullRaw = r.raw;  // not JSON — capture what came back
      } catch (e) { steps.pullError = e.message; }
      // verdict
      let verdict;
      if (steps.pullStatus === 200 && steps.pullOk) verdict = "WORKING — pull succeeded.";
      else if (steps.pullStatus === 401) verdict = "UNAUTHORIZED — token mismatch. Check the diag block: if server_has_token is false, set SYNC_TOKEN on Cloudflare and REDEPLOY. If lengths differ, re-paste the same token both sides.";
      else if (steps.pullRaw && /<!doctype|<html/i.test(steps.pullRaw)) verdict = "PORTAL RETURNED A WEB PAGE, NOT DATA — the /sync/pull function isn't running. Your functions/ folder didn't deploy, or an old build is live. Redeploy the WHOLE project (functions + public), not just public.";
      else if (steps.pullError && /timeout/i.test(steps.pullError)) verdict = "TIMEOUT — the panel couldn't reach the portal. Check the URL and that this computer has internet.";
      else if (steps.pullError && /ENOTFOUND|EAI_AGAIN/i.test(steps.pullError)) verdict = "DNS FAILED — the portal URL hostname couldn't be resolved. Check the URL spelling.";
      else if (steps.pullError && /ECONNREFUSED|certificate|SSL|TLS/i.test(steps.pullError)) verdict = "CONNECTION/SSL ERROR — " + steps.pullError;
      else verdict = "Could not confirm. See details below.";
      return ok({ ok: steps.pullStatus === 200 && steps.pullOk, verdict, steps });
    }
    if (sub === "/portal/sync-now" && method === "POST") {
      const r = await runPortalSync();
      const p = await pushCustomerSummary();
      const c = await pushPortalConfig();
      reschedulePortalSync(); // re-arm in case settings changed
      return ok({ ...r, push: p, config: c });
    }
    ...
    if ((mm = m(/^\/payments\/(\d+)\/receipt\/email$/)) && method === "POST") {
      ...
      try {
        await sendMail({ host: s.smtp_host, port: s.smtp_port, secure: s.smtp_secure === "1" || String(s.smtp_port) === "465", user: s.smtp_user, pass: s.smtp_pass, from: s.smtp_from || s.smtp_user },
          { to, subject: `${receiptTitle()} ${receiptNumberFor(p).full} — ${s.biz_name || "Internet Service"}`,
            text: `${receiptTitle()} ${receiptNumberFor(p).full} for ₱${Number(p.amount || 0).toLocaleString()} (${p.method || "cash"}). Thank you.`,
            html: receiptHtml(p) });
        return ok({ sent: true, to });
      } catch (e) { return send(res, 500, { ok: false, error: e.message }); }
    }
    if (sub === "/sales" && method === "GET") {
      const routerId = q.router_id != null && q.router_id !== "" ? q.router_id : null;
      return ok(Sales.series(q.range || "monthly", routerId));
    }
    if (sub === "/sales/reset/stats" && method === "GET") {
      return ok(SalesAdmin.stats());
    }
    if (sub === "/sales/reset" && method === "POST") {
      const scope = body.scope || "all";
      if (!["payments", "hardware", "proofs", "coins", "all"].includes(scope)) {
        return send(res, 400, { ok: false, error: "Invalid scope. Use payments, hardware, proofs, coins, or all." });
      }
      const r = SalesAdmin.reset(scope);
      Audit.add({ type: "manual", action: "reset-sales", detail: `${r.scope}: ${JSON.stringify(r.deleted)}`, ok: true });
      return ok(r);
    }

    // ---- Referrals & points ----
    if (sub === "/referrals/me" && method === "GET") {
      if (isAgentRole) {
        const ag = Agents.meForUser(panelUser?.id);
        if (!ag) {
          return ok({
            linked: false,
            account_type: "agent",
            username: panelUser?.username || "",
            settings: { signupPoints: Agents.signupPoints(), pointPeso: Agents.pointPeso() },
            publicUrl: referralPublicBase(),
          });
        }
        const code = String(ag.agent_code || Agents.ensureCode(ag.id, ag.name)).trim().toUpperCase();
        return ok({
          linked: true,
          account_type: "agent",
          id: ag.id,
          name: ag.name,
          username: ag.username || panelUser?.username || "",
          referral_code: code,
          referral_link: referralApplyUrl(code),
          referral_points: Number(ag.points) || 0,
          point_peso: ag.point_peso,
          peso_value: ag.peso_value,
          signup_points: Agents.signupPoints(),
          ledger: ag.ledger || [],
          settings: { signupPoints: Agents.signupPoints(), pointPeso: Agents.pointPeso() },
          publicUrl: referralPublicBase(),
          agent: {
            can_withdraw: !!ag.can_withdraw,
            withdraw_day: ag.withdraw_day,
            next_withdraw: ag.next_withdraw,
            min_withdraw_peso: ag.min_withdraw_peso,
            gcash: ag.gcash || "",
            clients: (ag.clients || []).length,
          },
        });
      }
      const me = Referrals.meForPanelUser(panelUser?.username);
      if (!me) {
        return ok({
          linked: false,
          username: panelUser?.username || "",
          settings: { signupPoints: Referrals.signupPoints(), pointPeso: Referrals.pointPeso() },
          publicUrl: referralPublicBase(),
        });
      }
    ...
    if (sub === "/referrals" && method === "GET") {
      const customers = Referrals.list(q.q || "").map((c) => {
        const code = String(c.referral_code || Referrals.ensureCode(c.id, c.username || c.name)).trim().toUpperCase();
        return { ...c, referral_code: code, referral_link: referralApplyUrl(code) };
      });
      return ok({
    ...
    if (sub === "/referrals/settings" && method === "POST") {
      const signup = body.signup_points != null ? body.signup_points : body.referral_signup_points;
      const peso = body.point_peso != null ? body.point_peso : body.referral_point_peso;
      if (signup != null) Settings.set("referral_signup_points", Math.max(0, Number(signup) || 0));
      if (peso != null) Settings.set("referral_point_peso", Math.max(0.01, Number(peso) || 1));
      Audit.add({ type: "manual", action: "referral-settings", detail: `signup=${Referrals.signupPoints()} peso=${Referrals.pointPeso()}`, ok: true });
      return ok({ signupPoints: Referrals.signupPoints(), pointPeso: Referrals.pointPeso() });
    }
    if ((mm = m(/^\/referrals\/(\d+)\/ledger$/)) && method === "GET") {
      const id = Number(mm[1]);
      if (isPersonalReferralRole) {
        const mine = Referrals.findCustomerForPanelUser(panelUser?.username);
        if (!mine || mine.id !== id) return send(res, 403, { ok: false, error: "You can only view your own referral ledger." });
      }
      const c = Customers.get(id);
      if (!c) return send(res, 404, { ok: false, error: "Customer not found" });
      return ok({
        customer: { id: c.id, name: c.name, referral_code: c.referral_code || Referrals.ensureCode(c.id, c.username || c.name), referral_points: Number(c.referral_points) || 0 },
        ledger: Referrals.ledger(Number(mm[1])),
      });
    }
    if ((mm = m(/^\/referrals\/(\d+)\/adjust$/)) && method === "POST") {
      const id = Number(mm[1]);
      const reason = String(body.reason || body.note || "admin adjustment").trim();
      let result;
      if (body.balance != null || body.set_balance != null) {
        result = Referrals.setBalance(id, body.balance != null ? body.balance : body.set_balance, reason);
      } else if (body.delta != null || body.points != null) {
        result = Referrals.adjust(id, body.delta != null ? body.delta : body.points, reason);
      } else {
        return send(res, 400, { ok: false, error: "Provide balance or delta (points to add/subtract)." });
      }
      if (!result) return send(res, 404, { ok: false, error: "Customer not found" });
      Audit.add({ type: "manual", customer_id: id, customer_name: result.name, action: "referral-adjust",
        detail: `${result.delta >= 0 ? "+" : ""}${result.delta} → ${result.referral_points} pts (${reason})`, ok: true });
      return ok(result);
    }

    // ---- Agents (home plan + prepaid only; 1 pt = ₱1; withdraw on day 30) ----
    if (sub === "/agents/me" && method === "GET") {
      if (!isAgentRole && !isAdminRole) return send(res, 403, { ok: false, error: "Agents only." });
      if (isAgentRole && panelUser?.id) Agents.ensureForUser(panelUser.id, panelUser.username);
      const me = Agents.meForUser(panelUser?.id);
      if (!me) return send(res, 404, { ok: false, error: "No agent profile linked to this login. Ask admin to create one under Agents → Add agent (use the same login username)." });
      const code = String(me.agent_code || "").trim().toUpperCase();
      return ok({ ...me, agent_code: code, referral_link: referralApplyUrl(code), publicUrl: referralPublicBase(), signup_points: Agents.signupPoints() });
    }
    if (sub === "/agents/me" && method === "PUT") {
      if (!isAgentRole) return send(res, 403, { ok: false, error: "Agents only." });
      const ag = Agents.getByUserId(panelUser?.id);
      if (!ag) return send(res, 404, { ok: false, error: "Agent profile not found." });
      const updated = Agents.update(ag.id, { contact: body.contact, gcash: body.gcash, name: body.name });
      return ok(updated);
    }
    if (sub === "/agents/me/withdraw" && method === "POST") {
      if (!isAgentRole) return send(res, 403, { ok: false, error: "Agents only." });
      const ag = Agents.getByUserId(panelUser?.id);
      if (!ag) return send(res, 404, { ok: false, error: "Agent profile not found." });
      const r = Agents.requestWithdrawal(ag.id, body.note);
      if (r.error) return send(res, 400, { ok: false, error: r.error });
      Audit.add({ type: "manual", action: "agent-withdraw-request", detail: `${ag.name} ₱${r.amount_peso} (${r.points_used} pts)`, ok: true });
      return ok(r);
    }
    if (sub === "/agents/summary" && method === "GET") {
      if (!isAdminRole) return send(res, 403, { ok: false, error: "Admin only." });
      return ok(Agents.summary());
    }
    if (sub === "/agents" && method === "GET") {
      if (!isAdminRole) return send(res, 403, { ok: false, error: "Admin only." });
      return ok({ agents: Agents.list(), summary: Agents.summary(), withdrawals: Agents.allWithdrawals("pending") });
    }
    if (sub === "/agents" && method === "POST") {
      ...
      if (!userId && loginName) {
        let u = Accounts.getByName(loginName);
        if (!u && body.password) {
          const { hashPassword } = await import("./lib/auth.js");
          const { salt, hash } = hashPassword(String(body.password));
          u = Accounts.create({ username: loginName, salt, hash, role: "agent" });
        } else if (u) Accounts.setRole(u.id, "agent");
        userId = u?.id || null;
      }
      if (userId) {
        const existing = Agents.getByUserId(userId);
        if (existing) return send(res, 400, { ok: false, error: "This user already has an agent profile." });
      }
      const ag = Agents.create({ user_id: userId, name: body.name, contact: body.contact, gcash: body.gcash, agent_code: body.agent_code, enabled: body.enabled !== 0 });
      if (userId) Accounts.setRole(userId, "agent");
      const code = String(ag.agent_code || "").trim().toUpperCase();
      Audit.add({ type: "manual", action: "agent-create", detail: ag.name + " " + ag.agent_code, ok: true });
      return ok({ ...ag, agent_code: code, referral_link: referralApplyUrl(code), publicUrl: referralPublicBase() });
    }
    if ((mm = m(/^\/agents\/(\d+)$/)) && method === "PUT") {
      if (!isAdminRole) return send(res, 403, { ok: false, error: "Admin only." });
      const updated = Agents.update(Number(mm[1]), body);
      if (!updated) return send(res, 404, { ok: false, error: "Agent not found." });
      if (body.user_id && updated.user_id) Accounts.setRole(updated.user_id, "agent");
      return ok(updated);
    }
    if ((mm = m(/^\/agents\/(\d+)$/)) && method === "DELETE") {
      if (!isAdminRole) return send(res, 403, { ok: false, error: "Admin only." });
      Agents.remove(Number(mm[1]));
      return ok({ removed: true });
    }
    if ((mm = m(/^\/agents\/(\d+)\/adjust$/)) && method === "POST") {
      if (!isAdminRole) return send(res, 403, { ok: false, error: "Admin only." });
      const id = Number(mm[1]);
      const reason = String(body.reason || body.note || "admin adjustment").trim();
      let result;
      if (body.balance != null || body.set_balance != null) {
        result = Agents.setBalance(id, body.balance != null ? body.balance : body.set_balance, reason);
      } else if (body.delta != null || body.points != null) {
        result = Agents.adjust(id, body.delta != null ? body.delta : body.points, reason);
      } else return send(res, 400, { ok: false, error: "Provide balance or delta (points)." });
      if (!result) return send(res, 404, { ok: false, error: "Agent not found." });
      const ag = Agents.get(id);
      Audit.add({ type: "manual", action: "agent-adjust", detail: `${ag?.name}: ${result.delta >= 0 ? "+" : ""}${result.delta} → ${result.points} pts`, ok: true });
      return ok(result);
    }
    if ((mm = m(/^\/agents\/(\d+)\/ledger$/)) && method === "GET") {
      if (!isAdminRole && !isAgentRole) return send(res, 403, { ok: false, error: "Forbidden." });
      const id = Number(mm[1]);
      if (isAgentRole) {
        const mine = Agents.getByUserId(panelUser?.id);
        if (!mine || mine.id !== id) return send(res, 403, { ok: false, error: "Your ledger only." });
      }
      return ok({ ledger: Agents.ledger(id, 80), agent: Agents.get(id) });
    }
    if (sub === "/agents/withdrawals" && method === "GET") {
      if (!isAdminRole) return send(res, 403, { ok: false, error: "Admin only." });
      const st = q.status || "";
      return ok(Agents.allWithdrawals(st));
    }
    if ((mm = m(/^\/agents\/withdrawals\/(\d+)\/review$/)) && method === "POST") {
      if (!isAdminRole) return send(res, 403, { ok: false, error: "Admin only." });
      const r = Agents.reviewWithdrawal(Number(mm[1]), body.status || body.action, panelUser?.username, body.admin_note || body.note);
      if (r.error) return send(res, 400, { ok: false, error: r.error });
      Audit.add({ type: "manual", action: "agent-withdraw-" + (r.status || "review"), detail: `#${r.id} ${r.agent_id} ₱${r.amount_peso}`, ok: true });
      return ok(r);
    }
    if (sub === "/agents/settings" && method === "POST") {
      if (!isAdminRole) return send(res, 403, { ok: false, error: "Admin only." });
      const map = {
        agent_point_peso: body.point_peso,
        agent_min_withdraw: body.min_withdraw,
        agent_withdraw_day: body.withdraw_day,
        agent_signup_points: body.signup_points,
        agent_payment_point_rate: body.payment_point_rate,
      };
      for (const [k, v] of Object.entries(map)) {
        if (v != null && v !== "") Settings.set(k, String(v));
      }
      Audit.add({ type: "manual", action: "agent-settings", detail: JSON.stringify(Agents.summary().settings), ok: true });
      return ok(Agents.summary().settings);
    }
    ...
    if (sub === "/settings" && method === "POST") {
      const patch = { ...body };
      for (const k of ["smtp_pass", "paymongo_secret", "paymongo_webhook_secret", "xendit_secret", "xendit_callback_token", "telegram_bot_token", "semaphore_api_key", "mikrotik_password", "ai_api_key", "portal_token", "radius_secret"])
      ...
      if ("watchdog_mins" in patch) { rescheduleWatchdog(); rescheduleRouterPoll(); }
    ...
    if ((mm = m(/^\/invoices\/(\d+)\/payment-link$/)) && method === "POST") {
      ...
      const s = Settings.all();
      const gw = paymentGateway();
      if (!gw.configured) return send(res, 400, { ok: false, error: (gw.name === "xendit" ? "Xendit" : "PayMongo") + " secret key not set — open Settings" });
      try {
        const pay = await createGatewayPayment(gw, {
          amountPhp: inv.amount,
          description: `${s.biz_name || "Internet"} ${inv.period} — ${inv.customer_name}`,
        ...
        Invoices.setLink(inv.id, pay.id, pay.url || pay.checkout_url || "");
        Audit.add({ type: "manual", customer_id: inv.customer_id, customer_name: inv.customer_name, action: "payment-link", detail: pay.id, ok: true });
        return ok({ url: pay.url || pay.checkout_url, id: pay.id, mode: pay.mode, qr_image_url: pay.qr_image_url || "" });
      } catch (e) {
        return send(res, 500, { ok: false, error: gw.name + ": " + e.message });
      }
    }
    ...
    if (sub === "/expenses" && method === "GET") {
      ...
      let list = Expenses.list(period ? { period } : {});
      if (siteScoped) list = list.filter((e) => e.router_id && scopedIds.includes(Number(e.router_id)));
      return ok({
    ...
    if (sub === "/joborders" && method === "GET") {
      ...
      if (q.get("job_type")) filter.job_type = q.get("job_type");
      if (q.get("scheduled") === "today") filter.scheduledToday = true;
      const list = JobOrders.list(filter).filter(jobOrderInScope);
      return ok({
        list,
        summary: JobOrders.summary(),
        alerts: JobOrders.alerts(),
        jobTypes: JobOrders.JOB_TYPES,
        itemStatuses: JobOrders.ITEM_STATUSES,
        techRoles: JobOrders.TECH_ROLES,
        sources: JobOrders.SOURCES,
      });
    }
    if (sub === "/joborders" && method === "POST") {
      try {
        const s = Settings.all();
        const jo = JobOrders.create({
          name: body.first_name || body.name,
          last_name: body.last_name,
          contact: body.contact,
          email: body.email,
          facebook: body.facebook,
          address: body.address,
          street_sitio: body.street_sitio,
          address_line2: body.address_line2,
          address_line3: body.address_line3,
          address_line4: body.address_line4,
          address_line5: body.address_line5,
          full_address: body.full_address,
          area: body.area,
          lat: body.lat,
          lng: body.lng,
          conn_type: body.conn_type || "pppoe",
          notes: body.notes || "",
          pay_choice: body.pay_choice === "now" ? "now" : "on_install",
          pay_status: body.pay_status,
          scheduled_date: body.scheduled_date,
          marking_order: body.marking_order,
          marking_completed: body.marking_completed,
          marking_repair: body.marking_repair,
          source: "manual",
          techs: body.techs,
          items: body.items,
        });
        const itemSummary = (jo.items || []).map((it) => JobOrders.typeLabel(it.job_type)).join(", ");
        Audit.add({ type: "manual", action: "jo-create-manual", detail: `JO #${jo.id} — ${itemSummary || JobOrders.typeLabel(jo.job_type)} — ${jobOrderFullName(jo)}`, ok: true });
        return ok(jo);
      } catch (e) { return send(res, 400, { ok: false, error: e.message }); }
    }
    ...
    if ((mm = m(/^\/joborders\/(\d+)\/assign$/)) && method === "POST") {
      const id = Number(mm[1]);
      if (Array.isArray(body.techs)) return ok(JobOrders.setTechs(id, body.techs));
      return ok(JobOrders.setTech(id, body.tech || ""));
    }
    ...
    // No serialized unit for this install — advance the job without blocking account creation.
    if ((mm = m(/^\/joborders\/(\d+)\/skip-equipment$/)) && method === "POST") {
      const jo = JobOrders.get(Number(mm[1]));
      if (!jo) return send(res, 404, { ok: false, error: "Job order not found" });
      if (!jo.tech) return send(res, 400, { ok: false, error: "Assign a technician first." });
      let installId = jo.install_id;
      if (!installId) {
        const ins = Inventory.createInstall({ customer_id: jo.customer_id || null, tech: jo.tech || "", notes: "Job Order #" + jo.id + " (no equipment)" });
        installId = ins.id;
        JobOrders.setInstall(jo.id, installId);
      }
      if (jo.status === "applied" || jo.status === "assigned") JobOrders.setStatus(jo.id, "released");
      Audit.add({ type: "manual", action: "jo-skip-equipment", detail: `JO #${jo.id} — no equipment released`, ok: true });
      return ok(JobOrders.get(jo.id));
    }
    if ((mm = m(/^\/joborders\/(\d+)\/signoff$/)) && method === "POST") {
      ...
      const codeUp = String(jo.referral_code || "").trim().toUpperCase();
      if (codeUp) {
        const agentAward = Agents.awardSignup(codeUp, jo.customer_id || null, jo.id, connType);
        if (agentAward && agentAward.points) {
          Audit.add({ type: "auto", action: "agent-install-bonus",
            detail: `+${agentAward.points} pts to agent #${agentAward.agentId} for install JO#${jo.id} (${connType})`, ok: true });
        }
      }
    ...
    if ((mm = m(/^\/joborders\/(\d+)\/approve-account$/)) && method === "POST") {
      ...
      const connType = (jo.plan_type === "ipoe") ? "ipoe" : (jo.plan_type === "hotspot") ? "hotspot" : "pppoe";
      const fullName = jobOrderFullName(jo);
      const fields = {
        name: fullName, contact: jo.contact || "", address: jo.address || "", area: jo.area || "",
      ...
      if (connType === "ipoe") {
        ...
        const preferred = applyAccountUsername(jo.name, fullName);
        const username = uniqueAccountUsername(String(body.username || jo.username || preferred).trim() || preferred);
        fields.username = username;
        fields.password = applyAccountPassword(jo.last_name, jo.password);
      }
      const cust = Customers.create(fields);
      const codeUp = String(jo.referral_code || "").trim().toUpperCase();
      if (codeUp) {
        const ag = Agents.getByCode(codeUp);
        if (ag) Agents.linkCustomer(cust.id, ag.id);
        else {
          const refAward = Referrals.awardSignup(jo.referral_code, cust.id, jo.id, fullName);
          if (refAward) {
            Audit.add({ type: "auto", customer_id: refAward.referrerId, action: "referral-bonus",
              detail: `+${refAward.points} pts for referring ${fullName} (JO#${jo.id})`, ok: true });
          }
        }
      }
...
// Robust peso-amount parser. Handles "1000", "1,000", "1,000.00", "₱1,000", "1 000",
// and the European "1.000,00" form. Strips currency words/symbols. Returns a whole-peso integer.
// This is deliberately strict so a stray separator can never turn 1000 into 101 or 1.
function parseMoney(raw) {
  let s = String(raw == null ? "" : raw).trim();
  if (!s) return 0;
  // drop everything except digits, dot, comma, space; remove spaces (thousands grouping)
  ...
  if (lastDot === -1 && lastComma === -1) {
    ...
    // BOTH separators present → the later one is the decimal sep, the other is thousands
    if (lastComma > lastDot) { s = s.replace(/\./g, "").replace(/,/g, "."); }   // European 1.000,00
    else { s = s.replace(/,/g, ""); }                                            // 1,000.00
  } else if (lastComma >= 0) {
    // ONLY comma(s). Decimal only if exactly one comma followed by 1-2 digits (e.g. "1,50").
    const after = s.length - lastComma - 1;
    const commaCount = (s.match(/,/g) || []).length;
    if (commaCount === 1 && (after === 1 || after === 2)) { s = s.replace(",", "."); }  // "1,50" -> 1.50
    else { s = s.replace(/,/g, ""); }                                                    // "1,000" -> 1000
  } else {
    // ONLY dot(s). Decimal only if exactly one dot followed by 1-2 digits (e.g. "1000.50").
    const after = s.length - lastDot - 1;
    const dotCount = (s.match(/\./g) || []).length;
    if (dotCount === 1 && (after === 1 || after === 2)) { /* keep as decimal */ }
    else { s = s.replace(/\./g, ""); }                                                   // "1.000" -> 1000
  }
  const n = Number(s);
  if (!isFinite(n) || n < 0) return 0;
  return Math.round(n);   // whole pesos
}
...
// Warn the operator (once per threshold) when the receipt series is near or past its end.
let _lastReceiptWarnSeq = 0;
function checkReceiptRangeWarning(p) {
  try {
    const rn = receiptNumberFor(p);
    if (!rn.end) return;
    if (rn.overEnd && _lastReceiptWarnSeq !== rn.seq) {
      _lastReceiptWarnSeq = rn.seq;
      tgNotify(`⚠️ <b>Receipt numbers EXCEEDED your range</b>\nNumber ${rn.full} is past your end (${rn.end}). Update the receipt range in Settings → Receipt / Invoice numbering.`);
      Audit.add({ type: "auto", action: "receipt-range", detail: `over end: ${rn.full} > ${rn.end}`, ok: false });
    } else if (rn.nearEnd && _lastReceiptWarnSeq !== rn.seq) {
      _lastReceiptWarnSeq = rn.seq;
      tgNotify(`🔔 <b>Receipt numbers almost used up</b>\nNow at ${rn.full}; your range ends at ${rn.end} (${rn.end - rn.seq} left). Consider setting a new range in Settings.`);
    }
  } catch {}
}
...
async function maybeAutoReceipt(customerId) {
  ...
  await sendMail({ host: s.smtp_host, port: s.smtp_port, secure: s.smtp_secure === "1" || String(s.smtp_port) === "465", user: s.smtp_user, pass: s.smtp_pass, from: s.smtp_from || s.smtp_user },
    { to: c.contact, subject: `${receiptTitle()} ${receiptNumberFor(p).full} — ${s.biz_name || "Internet Service"}`,
      text: `${receiptTitle()} ${receiptNumberFor(p).full} for ₱${Number(p.amount || 0).toLocaleString()}. Thank you.`, html: receiptHtml(full) });
  Audit.add({ type: "auto", customer_id: customerId, action: "auto-receipt", detail: receiptNumberFor(p).full + " → " + c.contact, ok: true });
}
...
async function runReminders() {
  ...
  for (const d of days) {
    ...
    for (const c of Customers.expiringOn(target)) {
      ...
      const pay = url ? ` Pay here: ${url}/welcome?pay=1&u=${encodeURIComponent(c.username || "")}` : "";
...
async function dailyReport() {
  ...
  const expiredNow = Customers.expiredAsOf(billingNow()).length;
...
// Parse portal payment tag from PayMongo remarks or Xendit external_id (e.g. "ABC123:topup" or "ABC123:topup-lx7k2m").
function parsePortalPaymentRemarks(remarks) {
  const s = String(remarks || "").trim();
  const m = s.match(/^([^:]+):(topup|renew|bill)(?::(\d+))?$/i);
  if (!m) return null;
  return { code: m[1].trim(), purpose: m[2].toLowerCase(), planId: m[3] ? Number(m[3]) : null };
}

function findCustomerByPortalCode(code) {
  const lc = String(code || "").trim().toLowerCase();
  if (!lc) return null;
  return Customers.list().find((c) =>
    (c.account_code || "").toLowerCase() === lc ||
    (c.username || "").toLowerCase() === lc);
}

// Credit a confirmed online portal payment (PayMongo/Xendit webhook). Idempotent by gateway ref.
async function creditPortalOnlinePayment({ cust, amount, purpose, ref, gateway, planId = null }) {
  if (!cust || amount <= 0) return { credited: 0, renewedTo: null, skipped: true };
  if (ref) {
    const ledger = Customers.creditLedger(cust.id);
    const refTag = "ref:" + ref;
    if (ledger.some((e) => String(e.reason || "").includes(refTag))) return { credited: 0, renewedTo: null, skipped: true };
  }
  const isIpoePrepaid = (cust.conn_type || "pppoe") === "ipoe" && (purpose === "renew" || purpose === "bill");
  let renewedTo = null;
  const effectivePlanId = planId || (isIpoePrepaid ? Number(cust.plan_id) || null : null);
  if (isIpoePrepaid && effectivePlanId) {
    const plan = Plans.get(effectivePlanId);
    const planPrice = Number(plan?.price) || 0;
    if (planPrice > 0 && amount >= planPrice) {
      try {
        const r = await renewCustomerWithPlan(cust, effectivePlanId, {
          method: gateway || "online", reference: ref || "", note: "Online payment (" + (gateway || "online") + ")",
        });
        renewedTo = r.expiry;
        Payments.record({ customer_id: cust.id, amount: planPrice, method: gateway || "online", reference: ref || "", note: `Prepaid ${r.plan}` });
        if (amount > planPrice) Customers.addCredit(cust.id, amount - planPrice, "Online overpayment ref:" + (ref || ""));
        Audit.add({ type: "auto", customer_id: cust.id, customer_name: cust.name, action: "portal-payment", detail: `₱${planPrice} prepaid ${r.plan} → ${renewedTo}`, ok: true });
        tgNotify(`💰 IPoE prepaid (${gateway || "online"})\n${cust.name}\nPlan: ${r.plan}\n₱${planPrice}\nUntil: ${renewedTo}`).catch(() => {});
        return { credited: amount, renewedTo, skipped: false };
      } catch (e) {
        Audit.add({ type: "auto", customer_id: cust.id, action: "portal-payment", detail: "prepaid renew failed: " + e.message, ok: false });
      }
    }
  }
  Customers.addCredit(cust.id, amount, "Online payment (" + (purpose || "topup") + ") ref:" + (ref || gateway || "online"));
  const planPrice = Number(cust.plan_price || 0);
  if (planPrice > 0 && amount >= planPrice && (purpose === "renew" || purpose === "bill")) {
    try {
      Customers.addCredit(cust.id, -planPrice, "no-income: wallet renewal ref:" + (ref || ""));
      const fresh = Customers.get(cust.id);
      const now = await routerNow();
      const base = (fresh.expiry && fresh.expiry >= now) ? fresh.expiry : now;
      renewedTo = addMinutes(base, Number(fresh.plan_mins) || 43200);
      Customers.setExpiry(fresh.id, renewedTo);
      Customers.setStatusAndAuto(fresh.id, "active", false);
      try { await applyProfileSwitch(Customers.get(cust.id), "active"); } catch (e) {
        Audit.add({ type: "auto", action: "portal-payment", detail: `renewed billing for ${cust.name} but router push pending: ${e.message}`, ok: false });
      }
    } catch (e) { /* top-up still stands */ }
  }
  const tgMsg = `💰 Online payment (${gateway || "online"})\n${cust.name} (${cust.account_code || cust.username || ""})\nAmount: ₱${amount}\nPurpose: ${purpose || "topup"}` + (renewedTo ? `\n✅ Auto-renewed until ${renewedTo}` : "");
  tgNotify(tgMsg).catch(() => {});
  Audit.add({ type: "auto", customer_id: cust.id, customer_name: cust.name, action: "portal-payment", detail: `credited ₱${amount}${renewedTo ? ", renewed to " + renewedTo : ""} ref ${ref || ""}`, ok: true });
  return { credited: amount, renewedTo, skipped: false };
}

async function handlePaidOnlineGatewayEvent(ev, gateway) {
  if (!ev) return;
  let tagSrc = ev.remarks || "";
  if (gateway === "paymongo" && ev.paymentIntentId) {
    try {
      const gw = paymentGateway();
      if (gw.getIntent) {
        const pi = await gw.getIntent(ev.paymentIntentId);
        tagSrc = pi.metadata?.portal || pi.description || tagSrc;
      }
    } catch {}
  }
  const voucher = parseVoucherPaymentTag(tagSrc);
  if (voucher) {
    const amountPeso = ev.amount != null ? Math.round(Number(ev.amount) / 100) : 0;
    try {
      await fulfillOnlineVoucher({ planId: voucher.planId, routerId: voucher.routerId, ref: ev.resourceId || ev.paymentIntentId || "", amountPeso });
    } catch (e) {
      Audit.add({ type: "auto", action: "voucher-online", detail: "webhook failed: " + e.message, ok: false });
    }
    return;
  }
  let portal = parsePortalPaymentRemarks(tagSrc || ev.remarks);
  if (portal) {
    const cust = findCustomerByPortalCode(portal.code);
    const amountPeso = ev.amount != null ? Math.round(Number(ev.amount) / 100) : 0;
    if (!cust) {
      Audit.add({ type: "auto", action: "portal-payment", detail: `${gateway} webhook: no customer for ${portal.code} (ref ${ev.resourceId || ""})`, ok: false });
      return;
    }
    if (amountPeso > 0) await creditPortalOnlinePayment({ cust, amount: amountPeso, purpose: portal.purpose, ref: ev.resourceId || "", gateway, planId: portal.planId || null });
    return;
  }
  ...
  if (inv && inv.status !== "paid") {
    Invoices.pay(inv.id, { method: "gcash", reference: ev.resourceId || gateway, note: (gateway === "xendit" ? "Xendit" : "PayMongo") + " online payment" });
...
// Credit a portal-payment proof to the customer's wallet and auto-renew if it covers the plan.
// Returns { credited, renewedTo }. Used by both the panel approve button and Telegram papprove.
async function approvePortalPaymentProof(proof, via) {
  const amt = Math.round(Number(proof.amount) || 0);
  const cu = proof.customer_id ? Customers.get(proof.customer_id) : null;
  try { Proofs.setStatus(proof.id, "approved", "portal payment (" + via + ")"); } catch {}
  if (!cu || amt <= 0) { Audit.add({ type: "auto", action: "portal-payment-approved", detail: `logged (no account/amount) ${proof.username || ""}`, ok: true }); return { credited: 0, renewedTo: null }; }
  Customers.addCredit(cu.id, amt, "Portal payment (approved " + via + ") ref:" + (proof.reference || ""));
  let renewedTo = null, shortfall = 0;
  const planPrice = Number(cu.plan_price || 0);
  // Only auto-renew for RENEW/BILL payments. A pure TOP-UP just adds to the wallet — it must NOT
  // spend on a renewal (that was the bug where a ₱1000 top-up became ₱101 after renewing a ₱899 plan).
  const isTopup = /·\s*TOPUP/i.test(proof.note || "");
  const isRenew = /·\s*(RENEW|BILL)/i.test(proof.note || "");
  // A renew/bill payment can use the customer's WHOLE wallet (this payment + any existing credit).
  const walletNow = Number(Customers.get(cu.id).credit || 0);
  if (!isTopup && planPrice > 0 && walletNow >= planPrice) {
    try {
      Customers.addCredit(cu.id, -planPrice, "no-income: wallet renewal ref:" + (proof.reference || ""));
      const fresh = Customers.get(cu.id); const now = await routerNow();
      const base = (fresh.expiry && fresh.expiry >= now) ? fresh.expiry : now;
      renewedTo = addMinutes(base, Number(fresh.plan_mins) || 43200);
      Customers.setExpiry(fresh.id, renewedTo); Customers.setStatusAndAuto(fresh.id, "active", false);
      try { await applyProfileSwitch(Customers.get(cu.id), "active"); } catch (e) { Audit.add({ type: "auto", action: "portal-payment", detail: `billing renewed, router push pending: ${e.message}`, ok: false }); }
    } catch (e) {}
  } else if (isRenew && planPrice > 0 && walletNow < planPrice) {
    // Customer tried to renew/pay the bill but doesn't have enough yet. Money is safe in the wallet.
    shortfall = planPrice - walletNow;
  }
  Audit.add({ type: via === "panel" ? "manual" : "auto", customer_id: cu.id, customer_name: cu.name, action: "portal-payment-approved", detail: `₱${amt}${renewedTo ? ", renewed to " + renewedTo : (shortfall ? `, short ₱${shortfall} to renew` : "")} (${via})`, ok: true });
  // Customer SMS: renewed / topped up / not-enough-to-renew
  if (Settings.get("notify_customers", "1") === "1" && cu.contact) {
    const bal = Number(Customers.get(cu.id).credit || 0);
    let body;
    if (renewedTo) body = `Hi ${cu.name}, we received your ₱${amt} payment. Your service is renewed until ${renewedTo}. Thank you!`;
    else if (shortfall > 0) body = `Hi ${cu.name}, we received your ₱${amt}. Your plan is ₱${planPrice} and your wallet now has ₱${bal} — please add ₱${shortfall} more to renew. Your money is safely kept in your wallet.`;
    else body = `Hi ${cu.name}, we received your ₱${amt} payment. It was added to your wallet (balance: ₱${bal}). Thank you!`;
    notifyCustomerMsg(cu.contact, cu.name, "Payment received", body).catch(() => {});
  }
  return { credited: amt, renewedTo, shortfall };
}
...
async function tgPollOnce() {
  ...
  for (const up of r.json.result) {
    ...
    if (action === "papprove") {
      // Portal payment approval: credit wallet by the amount, auto-renew if it covers the plan.
      const res2 = await approvePortalPaymentProof(proof, "telegram");
      const cu = proof.customer_id ? Customers.get(proof.customer_id) : null;
      try { await c.answerCallback(cb.id, res2.credited ? ("Credited ₱" + res2.credited + (res2.renewedTo ? " + renewed" : "")) : "Logged"); } catch {}
      try { if (cb.message) await c.editMessageText(cb.message.chat.id, cb.message.message_id, res2.credited ? `💰 Credited ₱${res2.credited} to ${cu ? cu.name : proof.username}.` + (res2.renewedTo ? ` Renewed until ${res2.renewedTo}.` : "") : `✅ Logged portal payment from ${proof.username}.`); } catch {}
      continue;
    }
    if (action === "approve") {
    ...
    if (usedGb >= capGb) {
      try {
        ...
        if (Settings.get("notify_customers", "1") === "1" && c.contact) {
          const url = (Settings.get("public_url") || "").replace(/\/$/, "");
          notifyCustomerMsg(c.contact, c.name, "Data limit reached", `Hi ${c.name}, you've reached your ${capGb}GB data limit and service is paused until renewal.` + (url ? ` ${url}/welcome?pay=1&u=${encodeURIComponent(c.username)}` : "")).catch(() => {});
        }
...
async function suspendForExpiry(c) {
  c = Customers.get(c.id) || c;
  if (await tryAutoRenewFromCredit(c)) return { renewed: true };
  if ((c.conn_type || "pppoe") !== "ipoe" && await isOnSuspendedRouterProfile(c)) {
    if (c.status !== "suspended") Customers.setStatusAndAuto(c.id, "suspended", true);
    return { skipped: true, reason: "already_on_suspended_profile" };
  }
  await applyProfileSwitch(c, "suspend");
  Customers.setStatusAndAuto(c.id, "suspended", true);
  ...
  if (Settings.get("notify_customers", "1") === "1" && c.contact) {
    const url = (Settings.get("public_url") || "").replace(/\/$/, "");
    const pay = url ? ` Settle here: ${url}/welcome?pay=1&u=${encodeURIComponent(c.username || "")}` : "";
...
// Every N minutes: suspend expired PPPoE clients still on any non-SUSPENDED router profile.
let _sweeping = false;
async function runExpirySweep() {
  if (_sweeping) return { skipped: true, reason: "busy" };
  if (!hasRouterConfigured()) return { skipped: true, reason: "no router" };
  _sweeping = true;
  try {
    const now = billingNow();
    const grace = Math.max(0, Number(Settings.get("expiry_grace_mins", "0")) || 0);
    const cutoff = grace ? addMinutes(now, -grace) : now;
    const candidates = Customers.expiredPppoeForValidation(cutoff);
    const ipoeCandidates = Customers.expiredAsOf(cutoff).filter((c) => (c.conn_type || "pppoe") === "ipoe");
    let n = 0, ipoeN = 0, skipped = 0, renewed = 0;
    const errors = [];
    for (const c of candidates) {
      if (!usesPppProfile(c)) continue;
      try {
        if (await isOnSuspendedRouterProfile(c)) {
          if (c.status !== "suspended") Customers.setStatusAndAuto(c.id, "suspended", true);
          skipped++;
          continue;
        }
        const r = await suspendForExpiry(c);
    ...
    for (const c of ipoeCandidates) {
      try {
        if (c.status === "suspended" && c.auto_suspended) { skipped++; continue; }
        const r = await suspendForExpiry(c);
        if (r?.renewed) renewed++;
        else if (r?.skipped) skipped++;
        else ipoeN++;
        ...
        errors.push({ id: c.id, name: c.name, error: e.message });
        Audit.add({ type: "auto", customer_id: c.id, customer_name: c.name, action: "ipoe-suspend", detail: e.message, ok: false });
      }
    }
    if (n || skipped || renewed || ipoeN) console.log(`[auto] expiry sweep suspended ${n} (+${ipoeN} IPoE), skipped ${skipped} (already SUSPENDED profile), renewed ${renewed}`);
    try { await reconcileActiveCustomersOnRouter(); } catch {}
    return { suspended: n, ipoe_suspended: ipoeN, skipped, renewed, checked: candidates.length + ipoeCandidates.length, now, errors };
  } finally { _sweeping = false; }
}

async function maybeSuspendCustomerIfExpired(id) {
  if (!hasRouterConfigured()) return null;
  const c = Customers.get(id);
  if (!c || c.status !== "active" || !String(c.expiry || "").trim()) return null;
  const now = billingNow();
  const due = Customers.expiredAsOf(now).find((x) => x.id === id);
  if (!due) return null;
  await suspendForExpiry(due);
...
function splitPortalLogin(first, lastOrPassword) {
  let f = String(first || "").trim();
  let l = String(lastOrPassword || "").trim();
  if (f.includes(" ")) {
    const parts = f.split(/\s+/).filter(Boolean);
    if (!l) l = parts[parts.length - 1];
    f = parts[0];
  }
  return { first: f, password: l };
}
function findCustomerByFirstLast(first, lastOrPassword) {
  const { first: f, password: l } = splitPortalLogin(first, lastOrPassword);
  const fn = normName(f);
  const ln = normName(l);
  if (!fn || !ln) return null;
  const all = Customers.list() || [];
  const fullInput = normName([f, l].join(" "));

  // Exact full-name match (Joel Madera + Madera, or full name typed in first field only).
  let hit = all.find((c) => normName(c.name) === fullInput);
  if (hit) return hit;

  const matches = all.filter((c) => {
    const p = customerNameParts(c.name);
    const firstOk = normName(p.first) === fn || normName(c.username) === fn || normName(c.account_code) === fn;
    if (!firstOk) return false;
    return normName(p.last) === ln || normName(c.password) === ln || normName(p.first) === ln;
  });
  if (matches.length === 1) return matches[0];
  if (matches.length > 1) {
    return matches.find((c) => normName(c.password) === ln)
      || matches.find((c) => normName(customerNameParts(c.name).last) === ln)
      || matches[0];
  }

  // PPPoE-style: username + password (for legacy accounts where name is the username).
  hit = all.find((c) => normName(c.username) === fn && (normName(c.password) === ln || normName(customerNameParts(c.name).last) === ln));
  if (hit) return hit;

  // Single-word subscriber names (MONDARES, CASTILLO999, etc.).
  return all.find((c) => {
    const n = normName(c.name);
    const u = normName(c.username);
    if (fn !== n && fn !== u) return false;
    return ln === n || ln === u || ln === normName(c.password);
  }) || null;
}
function findCustomersByPortalFirst(first) {
  const { first: f } = splitPortalLogin(first, "placeholder");
  const fn = normName(f);
  const full = normName(first);
  if (!fn && !full) return [];
  const all = Customers.list() || [];
  const out = new Map();
  for (const c of all) {
    const p = customerNameParts(c.name);
    const hit = normName(p.first) === fn
      || normName(c.username) === fn
      || normName(c.name) === full
      || normName(c.name).startsWith(full + " ")
      || normName(c.account_code) === fn;
    if (hit) out.set(c.id, c);
  }
  return [...out.values()];
}
function customerPasswordMatches(c, password) {
  const ln = normName(password);
  const p = customerNameParts(c.name);
  return normName(p.last) === ln || normName(c.password) === ln || normName(p.first) === ln;
}
function resolvePortalLogin(body = {}) {
  const split = splitPortalLogin(body.first || body.first_name || body.code, body.password || body.last);
  if (!split.first || !split.password) {
    return { ok: false, code: "missing", error: "Enter your first name and password (your last name)." };
  }
  const cust = findCustomerByPortalLogin({ ...body, first: split.first, password: split.password });
  if (cust) return { ok: true, customer: cust };
  if (findCustomersByPortalFirst(body.first || body.first_name || body.code).length) {
    return { ok: false, code: "wrong_password", error: "Wrong password. Your password is your last name on the account." };
  }
  return { ok: false, code: "not_found", error: "Account not found. Check your first name or contact us for help." };
}
function findCustomerByPortalLogin(body = {}) {
  const split = splitPortalLogin(body.first || body.first_name || body.code, body.password || body.last);
  const byName = findCustomerByFirstLast(split.first, split.password);
  if (byName) return byName;
  // Legacy fallback: account code / username + last name.
  const code = String(body.code || split.first || "").trim();
  const last = String(body.last || body.password || split.password || "").trim().toLowerCase();
  if (!code || !last) return null;
  const lc = code.toLowerCase();
  const fullCode = normName(code);
  return (Customers.list() || []).find((c) => {
    if (normName(c.name) === fullCode) return true;
    const codes = [c.account_code, c.username, ("ipoe-" + (c.username || "")), ("ipoe-" + String(c.id).padStart(4, "0"))].map((x) => String(x || "").toLowerCase());
    if (!codes.includes(lc)) return false;
    const lastName = customerNameParts(c.name).last.toLowerCase();
    return lastName === last || normName(c.password) === last || String(c.name || "").toLowerCase().includes(last);
  }) || null;
}
function parseApplyBody(b) {
  const first = String(b.first_name || b.name || "").trim();
  ...
  const username = applyAccountUsername(first, fullName);
  const password = applyAccountPassword(last);
  return { first, last, username, password, fullName };
}
function applyAccountUsername(first, fullName) {
  const raw = String(first || "").trim().replace(/\s+/g, "");
  if (raw) return raw;
  const fromFull = String(fullName || "").trim().split(/\s+/)[0] || "";
  return fromFull.replace(/\s+/g, "") || "client";
}
function applyAccountPassword(lastName, stored) {
  const last = String(lastName || stored || "").trim();
  return last || makePassword();
}
function uniqueAccountUsername(preferred) {
  let u = String(preferred || "").trim();
  if (!u) u = "client";
  if (!Customers.byUsername(u)) return u;
  let n = 2;
  while (Customers.byUsername(u + n)) n++;
  return u + n;
}
function jobOrderFullName(jo) {
  return [String(jo.name || "").trim(), String(jo.last_name || "").trim()].filter(Boolean).join(" ").trim() || String(jo.name || "").trim();
}
...
// ===== Layer 4: per-router health monitoring =====
// Live status per router (in-memory; persisted summary written to routers.last_status/last_seen).
//   _routerHealth[id] = { up, online, total, cpu, uptime, identity, lastCheck, error, downSince }
const _routerHealth = {};
let _rpRunning = false;
// Scan a router's DHCP leases for NodeMCU boards (hostname starts with one of the prefixes, e.g.
// WIZ / ESP). Returns candidates with bound + reachable status, and whether already added as a vendo.
async function scanLeasesForNodeMcu(conn, leases, prefixes, routerId, routerName) {
  const have = new Set((Vendos.list() || []).map((v) => String(v.mac || "").toUpperCase()).filter(Boolean));
  const out = [];
  for (const l of (leases || [])) {
    const host = String(l["host-name"] || l.hostname || "").toUpperCase();
    const comment = String(l.comment || "").toUpperCase();
    // Match if any prefix appears ANYWHERE in the hostname or comment (more forgiving than startsWith,
    // since boards name themselves things like "smart-esp-01", "vendo-WIZ", "ESP_1A2B3C", etc.)
    const match = prefixes.some((p) => (host + " " + comment).includes(p));
    if (!match) continue;
    const mac = String(l["mac-address"] || l.mac || "").toUpperCase();
    const ip = l.address || l["active-address"] || "";
    const bound = l.status === "bound" || l.status === "active" || !!l["active-address"];
    let reachable = false;
    if (ip) { try { const p = await conn.pingHost(ip, 2); reachable = !!p.reachable; } catch {} }
    out.push({
      name: l["host-name"] || l.hostname || ("NodeMCU-" + mac.slice(-5)),
      mac, ip, bound, reachable, online: bound || reachable,
      router_id: routerId, router_name: routerName,
      alreadyVendo: have.has(mac),
    });
  }
  return out;
}
...
async function pollAllRouters() {
  ...
  try {
    const routers = Routers.list().filter((r) => r.enabled !== 0 && r.host);
    for (const r of routers) {
      const prev = _routerHealth[r.id] || { up: null, downSince: null };
      let up = false, online = 0, cpu = null, uptime = null, ident = null, error = null;
      let pppoeOnline = 0, pppoeTotal = null, hsActive = 0;
      try {
        const conn = connForRouter(r);
        ident = await conn.identity();
        up = true;
        // best-effort extras — don't fail the whole poll if one call errors
        try { const act = await conn.listPppoeActive(); pppoeOnline = (act || []).length; online += pppoeOnline; } catch {}
        try { const secrets = await conn.listPppoe(); pppoeTotal = (secrets || []).length; } catch {}
        try { const hs = await conn.listHotspotActive(); hsActive = (hs || []).length; online += hsActive; } catch {}
        try { const res = await conn.systemResource(); if (res) { cpu = res["cpu-load"] != null ? Number(res["cpu-load"]) : null; uptime = res.uptime || null; } } catch {}
      } catch (e) { up = false; error = e.message || String(e); }
      const counts = { pppoeOnline, pppoeTotal, pppoeOffline: (pppoeTotal != null ? Math.max(0, pppoeTotal - pppoeOnline) : null), hsActive };

      const now = localNowStr();
      // transition detection → Telegram alert per site
      if (prev.up === true && !up) {
        _routerHealth[r.id] = { ...prev, up, error, lastCheck: now, downSince: now, counts };
        Routers.setStatus(r.id, "fail: " + (error || "unreachable"));
        Audit.add({ type: "auto", action: "router-down", detail: `${r.name} (${r.host}) unreachable: ${error || ""}`, ok: false });
        tgNotify(`🔴 <b>SITE DOWN</b>\nRouter: <b>${escapeHtml(r.name)}</b> (${escapeHtml(r.host)})\nClients here: ${r.clients || 0}\nTime: ${now}\n${escapeHtml(error || "")}`);
      } else if (prev.up === false && up) {
        const downMs = prev.downSince ? (Date.now() - Date.parse(prev.downSince.replace(" ", "T"))) : 0;
        _routerHealth[r.id] = { up, online, cpu, uptime, identity: ident, lastCheck: now, error: null, downSince: null, counts };
        Routers.setStatus(r.id, "ok");
        Audit.add({ type: "auto", action: "router-up", detail: `${r.name} restored`, ok: true });
        tgNotify(`🟢 <b>SITE RESTORED</b>\nRouter: <b>${escapeHtml(r.name)}</b> (${escapeHtml(r.host)})\nWas down ~${downMs ? Math.round(downMs / 60000) + " min" : "?"}\nTime: ${now}`);
      } else {
        _routerHealth[r.id] = { up, online, cpu, uptime, identity: ident, lastCheck: now, error, downSince: up ? null : (prev.downSince || now), counts };
        if (up) Routers.setStatus(r.id, "ok");
      }
    }
    return { polled: routers.length };
  } finally { _rpRunning = false; }
}
...
// Per-router health poll timer (Layer 4). Polls every router on its own schedule.
let _rpTimer = null;
function rescheduleRouterPoll() {
  if (_rpTimer) { clearInterval(_rpTimer); _rpTimer = null; }
  // Dedicated router API sync interval (seconds). Default 60s, floor 20s so we don't hammer
  // RouterOS over VPN. This drives the live PPPoE/hotspot counts shown in the Routers table.
  const secs = Math.max(20, Number(Settings.get("router_sync_secs", "60")) || 60);
  // only run if there are 2+ routers (single-router installs use the normal watchdog)
  try { if (Routers.list().length >= 2) _rpTimer = setInterval(() => { pollAllRouters().catch(() => {}); }, secs * 1000); } catch {}
  return secs;
}
...
function rescheduleSweep() {
  ...
  let mins = Number(Settings.get("expiry_check_mins", "1"));
  if (!Number.isFinite(mins) || mins < 1) mins = 1;
  _sweepTimer = setInterval(() => { runExpirySweep().catch(() => {}); }, mins * 60 * 1000);
  setTimeout(() => runExpirySweep().catch(() => {}), 8000);
  setTimeout(() => { reconcileActiveCustomersOnRouter().catch(() => {}); }, 15000);
  return mins;
}
...
function _portalCfg() {
  return {
    url: (Settings.get("portal_url", "") || "").trim().replace(/\/+$/, ""),   // e.g. https://portal.example.com
    token: (Settings.get("portal_token", "") || "").trim(),
...
function _portalGet(base, pathName, token) {
  return new Promise((resolve, reject) => {
    ...
    const req = lib.request(u, { method: "GET", headers: { Authorization: "Bearer " + token, "X-Sync-Token": token }, timeout: 12000 }, (res) => {
      let d = ""; res.on("data", (c) => d += c); res.on("end", () => { let j = null; try { j = JSON.parse(d || "{}"); } catch {} resolve({ status: res.statusCode, json: j, raw: d.slice(0, 200) }); });
    });
...
function _portalPost(base, pathName, token, bodyObj) {
  return new Promise((resolve, reject) => {
    ...
    const req = lib.request(u, { method: "POST", headers: { Authorization: "Bearer " + token, "X-Sync-Token": token, "Content-Type": "application/json", "Content-Length": Buffer.byteLength(payload) }, timeout: 12000 }, (res) => {
      let d = ""; res.on("data", (c) => d += c); res.on("end", () => { let j = null; try { j = JSON.parse(d || "{}"); } catch {} resolve({ status: res.statusCode, json: j, raw: d.slice(0, 200) }); });
    });
    req.on("timeout", () => req.destroy(new Error("portal timeout")));
...
async function runPortalSync() {
  ...
  try {
    ...
    for (const item of r.json.items) {
      try {
        ...
        if (item.kind === "apply") {
          ...
          JobOrders.apply({
            name: d.name, contact: d.contact, email: d.email, facebook: d.facebook, address: d.address, area: d.area,
          ...
          // Portal payment (customer paid via GCash/Maya directly and gave a reference).
          // Route it through the SAME safe engine as the built-in form: try to match a real
          // GCash/Maya SMS on your SIM (auto-approve if found + opted in), else send a Telegram
          // approve button so you confirm with one tap. Never blind-trust a typed reference.
          handlePortalPayment(d).catch((e) => Audit.add({ type: "auto", action: "portal-payment", detail: "error: " + e.message, ok: false }));
          imported++;
        }
        acked.push(item.id);
    ...
    // also pull confirmed online payments (PayMongo) and credit accounts
    try { await runPortalPaymentsPull(cfg); } catch (e) { Audit.add({ type: "auto", action: "portal-pay-pull", detail: "error: " + e.message, ok: false }); }
...
// Pull PAID online payments from the portal and apply them to accounts.
// Each paid payment: (1) tops up the customer's wallet, (2) auto-renews if the amount covers the
// plan price, (3) sends a Telegram alert. Idempotent: we ack each payment so it's credited once.
async function runPortalPaymentsPull(cfg) {
  const r = await _portalGet(cfg.url, "/sync/pull-payments", cfg.token);
  if (r.status === 401) { Audit.add({ type: "auto", action: "portal-pay-pull", detail: "unauthorized", ok: false }); return; }
  if (!r.json || !r.json.ok || !Array.isArray(r.json.items) || !r.json.items.length) return;
  const acked = [];
  for (const pay of r.json.items) {
    try {
      const code = String(pay.code || "").trim();
      const last = String(pay.last || "").trim().toLowerCase();
      const amount = Math.round(Number(pay.amount || 0));
      if (!code || amount <= 0) { acked.push(pay.id); continue; }
      // find the customer by account code (+ last name as a safety check)
      const cust = Customers.list().find((c) =>
        (c.account_code || "").toLowerCase() === code.toLowerCase() &&
        String(c.name || "").trim().split(/\s+/).slice(-1)[0].toLowerCase() === last);
      if (!cust) { Audit.add({ type: "auto", action: "portal-payment", detail: `paid ${amount} but no match for ${code}/${last} (ref ${pay.ref})`, ok: false }); acked.push(pay.id); continue; }
      // 1) top up wallet (records income, tagged as an online payment)
      Customers.addCredit(cust.id, amount, "Online payment (" + (pay.purpose || "topup") + ") ref:" + pay.ref);
      let renewedTo = null;
      // 2) auto-renew if this payment covers the plan price (and purpose isn't a plain top-up only)
      const planPrice = Number(cust.plan_price || 0);
      if (planPrice > 0 && amount >= planPrice && (pay.purpose === "renew" || pay.purpose === "bill")) {
        try {
          // spend the plan price from wallet, then renew one plan period
          Customers.addCredit(cust.id, -planPrice, "no-income: wallet renewal ref:" + pay.ref);
          // Set the BILLING renewal first (expiry + active) so the paid account is correct even if the
          // router isn't reachable this moment; then attempt the live router profile switch.
          const fresh = Customers.get(cust.id);
          const now = await routerNow();
          const base = (fresh.expiry && fresh.expiry >= now) ? fresh.expiry : now;
          renewedTo = addMinutes(base, Number(fresh.plan_mins) || 43200);
          Customers.setExpiry(fresh.id, renewedTo);
          Customers.setStatusAndAuto(fresh.id, "active", false);
          try { await applyProfileSwitch(Customers.get(cust.id), "active"); } catch (e) { Audit.add({ type: "auto", action: "portal-payment", detail: `renewed billing for ${cust.name} but router push pending: ${e.message}`, ok: false }); }
        } catch (e) { /* if renew fails, the top-up still stands */ }
      }
      // 3) Telegram alert
      const tgMsg = `💰 Online payment received\n${cust.name} (${code})\nAmount: ₱${amount}\nPurpose: ${pay.purpose || "topup"}` + (renewedTo ? `\n✅ Auto-renewed until ${renewedTo}` : `\nWallet topped up (no auto-renew)`);
      tgNotify(tgMsg).catch(() => {});
      Audit.add({ type: "auto", action: "portal-payment", detail: `credited ₱${amount} to ${cust.name} (${code})${renewedTo ? ", renewed to " + renewedTo : ""} ref ${pay.ref}`, ok: true });
      acked.push(pay.id);
    } catch (e) { /* skip a bad one, keep going */ }
  }
  if (acked.length) await _portalPost(cfg.url, "/sync/ack-payments", cfg.token, { ids: acked });
}

// Handle a manual portal payment (customer paid GCash/Maya directly + gave a reference).
// Tries SMS auto-match first; if found and you opted in, auto-credits. Otherwise sends a Telegram
// approve button. This reuses the same safe path as the built-in form — never blind-trusts input.
async function handlePortalPayment(d) {
  const name = String(d.name || "").trim();
  const contact = String(d.contact || "").trim();
  const reference = String(d.pay_reference || "").trim();
  const amount = parseMoney(d.message);
  const purpose = ["topup", "renew", "bill"].includes(d.purpose) ? d.purpose : "topup";
  // find the customer by username / account code / contact / name
  const u = name.toLowerCase();
  const cust = Customers.list().find((c) =>
    (c.username || "").toLowerCase() === u ||
    (c.account_code || "").toLowerCase() === u ||
    (c.contact || "").replace(/\D/g, "") === contact.replace(/\D/g, "") && contact.replace(/\D/g, "").length >= 7 ||
    String(c.name || "").trim().toLowerCase() === u);
  // create a proof record (so it shows in the Portal payments inbox + Telegram approve works)
  let smsMatch = null;
  try {
    const hit = Sms.findPayment(reference, amount);
    if (hit) {
      const amtOk = !amount || Math.abs(Number(hit.amount) - amount) < 0.01 || !hit.amount;
      smsMatch = { ok: true, amount: hit.amount, reference: hit.reference, name: hit.name, amountOk: amtOk };
    }
  } catch {}
  const proof = Proofs.add({
    invoice_id: null, customer_id: cust ? cust.id : null,
    username: cust ? (cust.username || cust.account_code || cust.name) : name,
    image: String(d.proof_img || ""), note: "PORTAL PAYMENT" + (purpose === "topup" ? " · TOPUP" : purpose === "renew" ? " · RENEW" : " · BILL") + (contact ? " · " + contact : "") + (amount ? " · ₱" + amount : ""),
    reference, amount,
    flags: JSON.stringify(smsMatch && smsMatch.ok ? [{ code: "sms-confirmed", level: smsMatch.amountOk ? "ok" : "warn", msg: smsMatch.amountOk ? `✓ Matched a ${smsMatch.name || "GCash"} SMS on your SIM (₱${Number(smsMatch.amount || 0).toLocaleString()}${smsMatch.reference ? ", ref " + smsMatch.reference : ""}).` : `SMS reference matched but amount differs — verify.` }] : []),
  });
  Audit.add({ type: "auto", action: "portal-payment", detail: `${name}${contact ? " (" + contact + ")" : ""} ₱${amount} ref:${reference || "-"}${cust ? " → " + cust.name : " (no match)"}`, ok: true });

  // Auto-approve if a matching SMS arrived AND you turned on auto-approve AND we matched a customer
  if (smsMatch && smsMatch.ok && smsMatch.amountOk && Settings.get("sms_autoapprove", "0") === "1" && cust && amount > 0) {
    try {
      const res2 = await approvePortalPaymentProof(Proofs.get(proof.id), "sms-auto");
      tgNotify(`✅ <b>AUTO-APPROVED (SMS)</b>\n${name} — ₱${amount} confirmed by SMS on your SIM${reference ? ", ref " + reference : ""}.` + (res2.renewedTo ? `\nRenewed until ${res2.renewedTo}` : `\nWallet topped up.`)).catch(() => {});
      return;
    } catch (e) { Audit.add({ type: "auto", action: "portal-payment", detail: "auto-approve failed: " + e.message, ok: false }); }
  }

  // Otherwise: send a Telegram approve/reject button (you confirm the money is really in your GCash)
  const c = tg(), chat = tgChat();
  if (c && chat) {
    try {
      const caption = `🧾 <b>PORTAL PAYMENT</b>\nFrom: <b>${escapeHtml(name)}</b>${contact ? "\nContact: " + escapeHtml(contact) : ""}` +
        (amount ? `\nAmount: ₱${amount.toLocaleString()}` : "") +
        (reference ? `\nReference: <b>${escapeHtml(reference)}</b>` : "") +
        (cust ? `\nAccount: ${escapeHtml(cust.name)} (${escapeHtml(cust.account_code || cust.username || "")})` : `\n⚠️ No matching account — review.`) +
        (smsMatch && smsMatch.ok ? `\n\n✅ <b>A matching SMS is on your SIM</b> — looks legit.` : `\n\nCheck your GCash/Maya app, then approve.`);
      const kb = { inline_keyboard: [[{ text: "✅ APPROVE", callback_data: "papprove:" + proof.id }, { text: "❌ REJECT", callback_data: "reject:" + proof.id }]] };
      let r;
      if (d.proof_img && String(d.proof_img).startsWith("data:image/")) {
        // send the payment screenshot as a photo with the approve buttons
        try { const { buffer } = dataUrlToBuffer(String(d.proof_img)); r = await c.sendPhoto(chat, buffer, caption, kb, "payment.jpg"); }
        catch { r = await c.sendMessage(chat, caption, { reply_markup: kb }); }
      } else {
        r = await c.sendMessage(chat, caption, { reply_markup: kb });
      }
      if (r && r.json && r.json.ok) { try { Proofs.setMsgId(proof.id, r.json.result.message_id); } catch {} }
    } catch (e) { Audit.add({ type: "auto", action: "telegram-error", detail: e.message, ok: false }); }
  }
}
...
function reschedulePortalSync() {
  ...
  if (cfg.enabled && cfg.url && cfg.token) {
    ...
    _portalTimer = setInterval(() => { runPortalSync().catch(() => {}); pushCustomerSummary().catch(() => {}); pushPortalConfig().catch(() => {}); }, mins * 60 * 1000);
...
async function pushCustomerSummary() {
  ...
  try {
    ...
    const summary = rows.map((c) => {
      ...
      return {
        code: c.account_code || "",
        username: (c.username || "").toLowerCase(),  // PPPoE/IPoE login name (so they can sign in with what they know)
        last: lastName.toLowerCase(),            // for matching (case-insensitive)
        name: c.name || "",
        plan: c.plan_name || "",
        speed: c.plan_speed || "",
        conn_type: c.conn_type || "pppoe",
        status: c.auto_suspended ? "suspended" : (c.status || "active"),
        due: c.expiry || "",
        balance: Number(c.credit || 0),          // wallet credit (positive = they have credit)
        rate: Number(c.plan_price || 0),         // monthly plan price (for the renew/pay amount)
      };
    }).filter((x) => (x.code || x.username) && x.last);  // need a code OR username, plus a last name
...
// Push PUBLIC config to the portal: the internet plans (name, price, speed, validity), plus the
// install fee and the agreement text shown on the apply form. This is marketing/public info (safe
// to be public), so it's pushed whenever sync is on — it keeps the portal's plan list in step with
// what you actually offer, so customers pick a real plan and see the real price.
async function pushPortalConfig() {
  const cfg = _portalCfg();
  if (!cfg.enabled || !cfg.url || !cfg.token) return { ok: false, skipped: true };
  try {
    const plans = (Plans.list() || [])
      .filter((p) => Settings.get("portal_hide_plan_" + p.id, "0") !== "1")  // optional per-plan hide
      .map((p) => ({
        id: p.id,
        name: p.name || "",
        ...
        type: p.type || "pppoe",
        features: p.features || "",   // newline- or comma-separated bullet points
      }));
    const config = {
      install_fee: Number(Settings.get("install_fee", "0") || 0),
      router_cost: Number(Settings.get("router_cost", "0") || 0),
      agreement: Settings.get("apply_agreement", "") || "",
      biz_name: Settings.get("biz_name", "") || "JM WIFI NETWORK",
      biz_address: Settings.get("biz_address", "") || "",
      biz_contact: Settings.get("biz_contact", "") || "",
      gcash_name: Settings.get("gcash_name", "") || "",
      gcash_number: Settings.get("gcash_number", "") || "",
      gcash_qr: Settings.get("gcash_qr", "") || "",
      maya_name: Settings.get("maya_name", "") || "",
      maya_number: Settings.get("maya_number", "") || "",
      maya_qr: Settings.get("maya_qr", "") || "",
    };
    const r = await _portalPost(cfg.url, "/sync/push-config", cfg.token, { plans, config });
    if (r.status === 401) { Audit.add({ type: "auto", action: "portal-config", detail: "unauthorized — check token", ok: false }); return { ok: false, error: "unauthorized" }; }
    return { ok: true, plans: plans.length };
  } catch (e) {
    Audit.add({ type: "auto", action: "portal-config", detail: "error: " + e.message, ok: false });
    return { ok: false, error: e.message };
  }
}
...
async function runCollections({ dryRun = false, graceDays = 0 } = {}) {
  ...
  for (const c of Collections.toSuspend(graceDays)) {
    ...
    try {
      await applyProfileSwitch(c, "suspend");
      Customers.setStatusAndAuto(c.id, "suspended", true);
      Audit.add({ type: "auto", customer_id: c.id, customer_name: c.name, action: "suspend", detail: "overdue → " + suspendedProfileName(), ok: true });
  ...
  for (const c of Collections.toReconnect(today)) {
    if (dryRun) { reconnected.push({ id: c.id, name: c.name, username: c.username, preview: true }); continue; }
    if (c.expiry && Customers.isExpiredAsOf(c.expiry, today)) continue;
    try {
      await applyProfileSwitch(c, "active");
      Customers.setStatusAndAuto(c.id, "active", false);
      Audit.add({ type: "auto", customer_id: c.id, customer_name: c.name, action: "reconnect", detail: "paid → plan profile", ok: true });
...
async function routerToday() {
  try { const r = await clockFromConn(mt); if (r) return r.slice(0, 10); } catch {}
  try {
    const def = Routers.getDefault();
    if (def && def.host) { const r = await clockFromConn(connForRouter(def)); if (r) return r.slice(0, 10); }
  } catch {}
...
function billingTimezone() {
  try {
    const tz = (Settings.get("billing_timezone") || "Asia/Manila").trim();
    return tz || "Asia/Manila";
  } catch { return "Asia/Manila"; }
}
// Billing expiry uses operator local time (default Philippines), not raw VPS UTC.
function billingNow() {
  try {
    return new Date().toLocaleString("sv-SE", { timeZone: billingTimezone() }).slice(0, 19);
  } catch { return localNowStr(); }
}
async function clockFromConn(conn) {
  const c = await conn.clock();
  const d = parseRosDate(c && c.date);
  const t = (c && c.time && /^\d{2}:\d{2}/.test(c.time)) ? c.time.slice(0, 8) : null;
  if (d) return `${d} ${t || "00:00:00"}`;
  return null;
}
// Full datetime "YYYY-MM-DD HH:MM:SS" from the router clock (fallback: server local time).
async function routerNow() {
  try { const r = await clockFromConn(mt); if (r) return r; } catch {}
  try {
    const def = Routers.getDefault();
    if (def && def.host) { const r = await clockFromConn(connForRouter(def)); if (r) return r; }
  } catch {}
  for (const r of Routers.list().filter((x) => x.enabled !== 0 && String(x.host || "").trim())) {
    try { const t = await clockFromConn(connForRouter(r)); if (t) return t; } catch {}
  }
  return localNowStr();
}
function addDays(ymd, days) { const d = new Date(ymd + "T00:00:00"); d.setDate(d.getDate() + (Number(days) || 0)); return d.toISOString().slice(0, 10); }
function addMinutes(dt, mins) {
  const s = String(dt || "").trim().replace("T", " ");
  let base;
  if (/^\d{4}-\d{2}-\d{2} \d{2}:\d{2}:\d{2}$/.test(s)) base = new Date(s.replace(" ", "T"));
  else if (/^\d{4}-\d{2}-\d{2} \d{2}:\d{2}$/.test(s)) base = new Date(s.replace(" ", "T") + ":00");
  else if (/^\d{4}-\d{2}-\d{2}$/.test(s)) base = new Date(s + "T23:59:59");
  else return localNowStr();
  if (isNaN(base.getTime())) return localNowStr();
  base.setMinutes(base.getMinutes() + (Number(mins) || 0));
  return `${base.getFullYear()}-${pad2(base.getMonth()+1)}-${pad2(base.getDate())} ${pad2(base.getHours())}:${pad2(base.getMinutes())}:${pad2(base.getSeconds())}`;
}
...
function rateLimitFor(c) {
  const rl = planRateLimit({
    download_speed: c.plan_download_speed,
    upload_speed: c.plan_upload_speed,
    speed: c.plan_speed,
  }) || c.plan_rate || c.rate_limit || "";
  if (rl && !rl.includes("/")) return rl + "/" + rl;
  return rl;
}
async function ipoeProvision(c) {
  ...
  const conn = connForCustomer(c);   // ← this customer's router
  ...
  let servers = [];
  try { servers = (await conn.dhcpServers()) || []; } catch (e) { throw new Error("Can't reach the router to provision (" + e.message + ")."); }
  ...
  try {
    ...
    if (srv) {
      const addrs = (await conn.ipAddresses().catch(() => [])) || [];
  ...
  const expTag = c.expiry ? String(c.expiry).slice(0, 10) : "";
  const planTag = c.plan_name || "";
  const leaseComment = ["ipoe", c.name, planTag, expTag ? "exp " + expTag : ""].filter(Boolean).join(" | ");
  // remove any stale lease/queue for this MAC first (idempotent)
  try { await conn.leaseRemoveByMac(mac); } catch {}
  try {
    await conn.leaseAddStatic({ address: c.static_ip, mac, server, rateLimit: rl, comment: leaseComment });
  } catch (e) {
  ...
  if (rl) {
    ...
    try { await conn.queueSimpleRemoveByName(qname); } catch {}
  }
  // make sure they're not left on the suspended list
  try { await conn.addrListRemove(ipoeSuspendList(), c.static_ip); } catch {}
}
async function ipoeSuspend(c) {
  if (!c.static_ip) throw new Error("IPoE customer has no static IP to suspend.");
  const conn = connForCustomer(c);
  const list = ipoeSuspendList();
  const tag = [c.name, c.plan_name || "", c.expiry ? "exp " + String(c.expiry).slice(0, 10) : ""].filter(Boolean).join(" | ");
  await conn.addrListAdd(list, c.static_ip, "expired " + tag);
  Audit.add({ type: "auto", customer_id: c.id, action: "ipoe-suspend", detail: c.static_ip + " added to '" + list + "'", ok: true });
}
async function ipoeEnable(c) {
  if (!c.static_ip) throw new Error("IPoE customer has no static IP.");
  const conn = connForCustomer(c);
  const list = ipoeSuspendList();
  await conn.addrListRemove(list, c.static_ip);
  Audit.add({ type: "auto", customer_id: c.id, action: "ipoe-enable", detail: c.static_ip + " removed from '" + list + "'", ok: true });
  if (c.mac && c.vlan_iface) {
    try { await ipoeProvision(c); } catch (e) {
      Audit.add({ type: "auto", customer_id: c.id, action: "ipoe-enable", detail: "re-provision: " + e.message, ok: false });
    }
  }
}

/** Remove a customer from the suspend redirect path (address-list + wrong PPP profile). */
async function clearCustomerFromSuspendList(c) {
  if (!hasRouterConfigured()) return;
  const conn = connForCustomer(c);
  const isIpoe = (c.conn_type || "pppoe") === "ipoe";
  if (isIpoe) {
    if (c.static_ip) try { await conn.addrListRemove(ipoeSuspendList(), c.static_ip); } catch {}
    return;
  }
  if (!c.username) return;
  try {
    const active = ((await conn.listPppoeActive().catch(() => [])) || [])
      .find((x) => String(x.name || "").toLowerCase() === String(c.username).toLowerCase());
    if (active?.address) try { await conn.addrListRemove(ipoeSuspendList(), active.address); } catch {}
  } catch {}
}

/** Active, non-expired customers must NOT stay on suspended profile or address-list (fixes pay portal on active clients). */
async function reconcileActiveCustomersOnRouter() {
  if (!hasRouterConfigured()) return { skipped: true, reason: "no router" };
  const now = billingNow();
  const suspProfile = suspendedProfileName();
  let cleared = 0, fixedProfile = 0, errors = [];
  for (const c of Customers.list()) {
    if (c.status === "suspended" && c.auto_suspended) continue;
    if (Customers.isExpiredAsOf(c.expiry, now)) continue;
    if (c.status !== "active") continue;
    try {
      await clearCustomerFromSuspendList(c);
      cleared++;
      if ((c.conn_type || "pppoe") !== "ipoe" && c.username && usesPppProfile(c)) {
        const got = await readCustomerRouterProfile(c);
        if (got && profileNamesMatch(got, suspProfile)) {
          const conn = connForCustomer(c);
          const activeProfile = c.plan_profile || "default";
          await conn.updatePppoe(c.username, { profile: activeProfile });
          try { await conn.disconnectPppoe(c.username); } catch {}
          fixedProfile++;
          Audit.add({ type: "auto", customer_id: c.id, customer_name: c.name, action: "reconcile-profile", detail: `${suspProfile} → ${activeProfile}`, ok: true });
        }
      }
    } catch (e) {
      errors.push({ id: c.id, name: c.name, error: e.message });
      Audit.add({ type: "auto", customer_id: c.id, customer_name: c.name, action: "reconcile-active", detail: e.message, ok: false });
    }
  }
  if (cleared || fixedProfile) console.log(`[auto] reconcile active: cleared ${cleared} from suspend list, fixed ${fixedProfile} PPP profile(s)`);
  return { cleared, fixedProfile, errors };
}
...
// PPP profile used when suspending (Settings override, default SUSPENDED).
function suspendedProfileName() {
  const v = (Settings.get("suspended_profile") || "").trim();
  return v || "SUSPENDED";
}

async function ensurePppProfileExists(conn, profileName) {
  const want = String(profileName || "").trim();
  if (!want) throw new Error("Suspended PPP profile name is not configured.");
  const profiles = (await conn.pppProfiles().catch(() => [])) || [];
  const hit = profiles.find((p) => String(p.name || "").toLowerCase() === want.toLowerCase());
  if (!hit) throw new Error(`PPP profile "${want}" not found on the router. Create it under PPP → Profiles (with address-list=suspended).`);
}

async function readPppoeProfile(conn, username) {
  const want = String(username || "").toLowerCase();
  const row = ((await conn.listPppoe().catch(() => [])) || []).find((r) => String(r.name || "").toLowerCase() === want);
  return row ? String(row.profile || "") : "";
}

function profileNamesMatch(a, b) {
  return String(a || "").trim().toLowerCase() === String(b || "").trim().toLowerCase();
}

async function readCustomerRouterProfile(c) {
  if (!c?.username || !hasRouterConfigured()) return "";
  if ((c.conn_type || "pppoe") === "ipoe") return "";
  const conn = connForCustomer(c);
  const want = String(c.username).toLowerCase();
  if ((c.plan_type || "pppoe") === "hotspot") {
    const row = ((await conn.listHotspotUsers().catch(() => [])) || []).find((r) => String(r.name || "").toLowerCase() === want);
    return row ? String(row.profile || "") : "";
  }
  return readPppoeProfile(conn, c.username);
}

async function isOnSuspendedRouterProfile(c) {
  const got = await readCustomerRouterProfile(c);
  return !!got && profileNamesMatch(got, suspendedProfileName());
}

function usesPppProfile(c) {
  return (c.conn_type || "pppoe") !== "ipoe" && !!String(c.username || "").trim();
}

// Switch a customer's router profile (ACTIVE plan profile <-> SUSPENDED profile).
// User stays enabled so they can reconnect on SUSPENDED and hit the pay redirect.
async function applyProfileSwitch(c, target) {
  ...
  const conn = connForCustomer(c);   // ← send to THIS customer's router
  const isHotspot = (c.plan_type || "pppoe") === "hotspot";
  const suspProfile = suspendedProfileName();
  const activeProfile = c.plan_profile || "default";
  if (target === "suspend") {
    if ((c.conn_type || "pppoe") !== "ipoe") {
      const current = await readCustomerRouterProfile(c);
      if (current && profileNamesMatch(current, suspProfile)) return;
    }
    if (isHotspot) {
      await conn.updateHotspotUser(c.username, { profile: suspProfile });
      await conn.setHotspotUserDisabled(c.username, false);
    } else {
      await ensurePppProfileExists(conn, suspProfile);
      await conn.updatePppoe(c.username, { profile: suspProfile });
      await conn.setPppoeDisabled(c.username, false);
      const got = await readPppoeProfile(conn, c.username);
      if (got.toLowerCase() !== suspProfile.toLowerCase()) {
        throw new Error(`PPPoE profile not applied: expected "${suspProfile}" but router shows "${got || "(empty)"}"`);
      }
    }
  } else {
    if (c.expiry && Customers.isExpiredAsOf(c.expiry, billingNow())) {
      throw new Error(`Cannot activate ${c.username}: billing expired ${c.expiry}. Renew expiry first.`);
    }
    await clearCustomerFromSuspendList(c);
    if (isHotspot) {
      await conn.updateHotspotUser(c.username, { profile: activeProfile });
      await conn.setHotspotUserDisabled(c.username, false);
    } else {
      await conn.updatePppoe(c.username, { profile: activeProfile });
      await conn.setPppoeDisabled(c.username, false);
  ...
  try { if (isHotspot) await conn.disconnectHotspot(c.username); else await conn.disconnectPppoe(c.username); } catch {}
}

// Pay bill or renew plan using prepaid wallet credit (customer portal).
async function payWithWalletCredit(customerId, purpose = "bill", planId = null) {
  const cust = Customers.get(customerId);
  if (!cust) throw new Error("Account not found.");
  const bal = Number(cust.credit) || 0;
  if (bal <= 0) throw new Error("Your wallet balance is ₱0. Top up first or pay with GCash.");

  const isIpoePrepaid = (cust.conn_type || "pppoe") === "ipoe" && (purpose === "renew" || purpose === "bill");
  const effectivePlanId = planId || (isIpoePrepaid ? Number(cust.plan_id) || null : null);
  if (isIpoePrepaid && effectivePlanId) {
    const plan = validatePrepaidPlanForCustomer(Plans.get(effectivePlanId), cust);
    const price = Number(plan.price) || 0;
    if (price <= 0) throw new Error("Invalid plan price.");
    if (bal < price) throw new Error(`Not enough wallet balance. Need ₱${price.toLocaleString()}, you have ₱${bal.toLocaleString()}.`);
    Customers.addCredit(cust.id, -price, "wallet prepaid " + plan.name);
    const r = await renewCustomerWithPlan(cust, effectivePlanId, { method: "wallet", note: "self-pay from wallet" });
    const fresh = Customers.get(cust.id);
    return {
      message: `Paid ₱${price.toLocaleString()} for ${r.plan}. Active until ${String(r.expiry).slice(0, 10)}.`,
      wallet: Number(fresh.credit) || 0, expiry: r.expiry, status: fresh.status || "active", paidPeso: price, plan: r.plan,
    };
  }
  ...
  if (inv) {
    const balance = Math.max(0, Number(inv.amount) - (Number(inv.paid_amount) || 0));
    if (balance <= 0) throw new Error("No balance due on your bill.");
    const payAmt = Math.min(balance, bal);
    Customers.addCredit(cust.id, -payAmt, `wallet → invoice #${inv.id}`);
    const r2 = Invoices.pay(inv.id, { amount: payAmt, method: "wallet", note: "self-pay from wallet" });
    Audit.add({ type: "auto", customer_id: cust.id, customer_name: cust.name, action: "wallet-pay",
      detail: `₱${payAmt} → invoice #${inv.id}${r2.fully ? " (fully paid)" : ""}`, ok: true });
    if (r2.fully) {
      try { await renewCustomer(Customers.get(cust.id), cust.plan_mins || 43200); } catch (e) {
        Audit.add({ type: "auto", customer_id: cust.id, action: "wallet-pay", detail: `invoice paid but renew failed: ${e.message}`, ok: false });
      }
    }
    const fresh = Customers.get(cust.id);
    return {
      message: r2.fully
        ? `Paid ₱${payAmt.toLocaleString()} from your wallet. Your bill is paid and service is reconnecting.`
        : `Applied ₱${payAmt.toLocaleString()} from your wallet to your bill.`,
      wallet: Number(fresh.credit) || 0,
      expiry: fresh.expiry || "",
  ...
  const price = Number(cust.plan_price) || 0;
  if (price <= 0) throw new Error("No plan price set on your account — please contact us.");
  if (bal < price) {
    throw new Error(`Not enough wallet balance. Need ₱${price.toLocaleString()}, you have ₱${bal.toLocaleString()}. Top up or pay with GCash.`);
  }
  Customers.addCredit(cust.id, -price, purpose === "renew" ? "wallet renewal" : "wallet bill pay");
  try {
    const exp = await renewCustomer(cust, cust.plan_mins || 43200);
    Payments.record({ customer_id: cust.id, amount: price, method: "wallet", note: "self-pay from wallet" });
    Audit.add({ type: "auto", customer_id: cust.id, customer_name: cust.name, action: "wallet-renew",
      detail: `₱${price}, expiry ${exp}`, ok: true });
    const fresh = Customers.get(cust.id);
    return {
      message: `Paid ₱${price.toLocaleString()} from your wallet. Active until ${String(exp).slice(0, 10)}.`,
      wallet: Number(fresh.credit) || 0,
      expiry: exp,
      status: fresh.status || "active",
      paidPeso: price,
    ...
    Customers.addCredit(cust.id, price, "no-income: refund — wallet pay failed");
    throw new Error("Payment recorded but reconnect failed; your wallet was refunded. " + e.message);
  }
}

// Redeem referral points toward an unpaid invoice or plan renewal.
async function redeemReferralPoints(customerId, purpose = "bill", planId = null) {
  const cust = Customers.get(customerId);
  if (!cust) throw new Error("Account not found.");
  const pts = Number(cust.referral_points) || 0;
  const pesoPerPt = Referrals.pointPeso();
  if (pts <= 0) throw new Error("You have no referral points yet. Share your referral code when friends apply.");
  ...
  if (isIpoePrepaid && effectivePlanId) {
    ...
    const price = Number(plan.price) || 0;
    if (price <= 0) throw new Error("Invalid plan price.");
    const ptsNeeded = Math.ceil(price / pesoPerPt);
    if (pts < ptsNeeded) {
      throw new Error(`Need ${ptsNeeded} points for ${plan.name} (₱${price.toLocaleString()}). You have ${pts} points.`);
    }
    const redeem = Referrals.redeemPoints(cust.id, ptsNeeded, "prepaid " + plan.name);
    if (!redeem.ok) throw new Error(redeem.error || "Could not redeem points.");
    try {
      const r = await renewCustomerWithPlan(cust, effectivePlanId, {
        method: "referral", note: `${redeem.used} referral points — ${plan.name}`,
      });
      Payments.record({ customer_id: cust.id, amount: price, method: "referral", note: `${redeem.used} referral points — ${plan.name}` });
      const fresh = Customers.get(cust.id);
      return {
        message: `Renewed ${plan.name} using ${redeem.used} points! Active until ${String(r.expiry).slice(0, 10)}.`,
        usedPoints: redeem.used, paidPeso: price, referral_points: Number(fresh.referral_points) || 0, expiry: r.expiry, plan: r.plan,
      };
    } catch (e) {
      Referrals.addPoints(cust.id, redeem.used, `refund: renewal failed`, null);
      throw new Error("Renewal failed on the router; your points were restored. " + e.message);
    }
  }
  ...
  if (inv) {
    const balance = Math.max(0, Number(inv.amount) - (Number(inv.paid_amount) || 0));
    if (balance <= 0) throw new Error("No balance due on your bill.");
    const maxPeso = pts * pesoPerPt;
    const payPeso = Math.min(balance, maxPeso);
    const usePts = Math.ceil(payPeso / pesoPerPt);
    const redeem = Referrals.redeemPoints(cust.id, usePts, `invoice #${inv.id}`);
    if (!redeem.ok) throw new Error(redeem.error || "Could not redeem points.");
    const appliedPeso = redeem.used * pesoPerPt;
    const r2 = Invoices.pay(inv.id, { amount: appliedPeso, method: "referral", note: `${redeem.used} referral points` });
    Audit.add({ type: "auto", customer_id: cust.id, customer_name: cust.name, action: "referral-pay",
      detail: `${redeem.used} pts (₱${appliedPeso}) → invoice #${inv.id}${r2.fully ? " (fully paid)" : ""}`, ok: true });
    if (r2.fully) {
      try { await renewCustomer(Customers.get(cust.id), cust.plan_mins || 43200); } catch (e) {
        Audit.add({ type: "auto", customer_id: cust.id, action: "referral-pay", detail: `invoice paid but renew failed: ${e.message}`, ok: false });
      }
    }
    const fresh = Customers.get(cust.id);
    return {
      message: r2.fully
        ? `Paid ₱${appliedPeso.toLocaleString()} using ${redeem.used} points. Your bill is fully paid.`
        : `Applied ${redeem.used} points (₱${appliedPeso.toLocaleString()}) to your bill.`,
      usedPoints: redeem.used, paidPeso: appliedPeso, referral_points: Number(fresh.referral_points) || 0, invoice: r2.invoice,
    };
  }

  if (purpose !== "renew" && purpose !== "bill") throw new Error("No unpaid bill found.");
  const price = Number(cust.plan_price) || 0;
  if (price <= 0) throw new Error("No plan price on your account — please contact us.");
  const ptsNeeded = Math.ceil(price / pesoPerPt);
  if (pts < ptsNeeded) {
    throw new Error(`Need ${ptsNeeded} points to renew (₱${price.toLocaleString()}). You have ${pts} points.`);
  }
  const redeem = Referrals.redeemPoints(cust.id, ptsNeeded, "plan renewal");
  if (!redeem.ok) throw new Error(redeem.error || "Could not redeem points.");
  try {
    const exp = await renewCustomer(cust, cust.plan_mins || 43200);
    Payments.record({ customer_id: cust.id, amount: price, method: "referral", note: `${redeem.used} referral points renewal` });
    Audit.add({ type: "auto", customer_id: cust.id, customer_name: cust.name, action: "referral-renew",
      detail: `${redeem.used} pts → renew until ${exp}`, ok: true });
    const fresh = Customers.get(cust.id);
    return {
      message: `Renewed using ${redeem.used} points! Active until ${String(exp).slice(0, 10)}.`,
      usedPoints: redeem.used, paidPeso: price, referral_points: Number(fresh.referral_points) || 0, expiry: exp,
    };
  } catch (e) {
    Referrals.addPoints(cust.id, redeem.used, `refund: renewal failed`, null);
    throw new Error("Renewal failed on the router; your points were restored. " + e.message);
...
function ipoeSuspendList() {
  return String(Settings.get("ipoe_suspend_list") || process.env.IPOE_SUSPEND_LIST || "IPOE-EXPIRED").trim() || "IPOE-EXPIRED";
}

function validatePrepaidPlanForCustomer(plan, cust) {
  if (!plan) throw new Error("Plan not found.");
  if (String(plan.status || "active").toLowerCase() === "inactive") throw new Error("That plan is no longer available.");
  const conn = String(cust.conn_type || "pppoe").toLowerCase();
  const ct = String(plan.client_type || plan.type || "pppoe").toLowerCase();
  if (ct !== "both" && ct !== conn) throw new Error("That plan is not available for your connection type.");
  if (conn === "ipoe" && String(plan.type || "").toLowerCase() === "pppoe" && ct !== "both") {
    throw new Error("That plan is for PPPoE only.");
  }
  return plan;
}

function planMatchesConnTypePortal(p, connType) {
  const ct = String(connType || "pppoe").toLowerCase();
  const clientType = String(p.client_type || p.type || "pppoe").toLowerCase();
  return clientType === "both" || clientType === ct;
}

function portalPlanPayload(p) {
  const mins = planDurationMins(p);
  const isHotspot = String(p.type || "").toLowerCase() === "hotspot";
  const row = {
    id: p.id, name: p.name, price: Number(p.price) || 0,
    speed: isHotspot ? clientSpeedLabel(p) : (planDisplaySpeed(p) || ""),
    duration_value: Number(p.duration_value) || Math.max(1, Math.round(mins / 1440)),
    duration_type: p.duration_type || "day",
    duration_label: fmtPlanDuration(p),
    validity_mins: mins, validity_days: p.validity_days,
    type: p.type || "pppoe", client_type: p.client_type || p.type || "pppoe",
    status: p.status || "active", features: p.features || "",
  };
  // Hotspot/voucher: never expose real MikroTik rate or profile to clients.
  if (!isHotspot) {
    row.rate_limit = planRateLimit(p) || "";
    row.download_speed = p.download_speed || "";
    row.upload_speed = p.upload_speed || "";
  }
  return row;
}

function adminHotspotPackage(p) {
  if (!p) return null;
  return {
    id: p.id, name: p.name, price: Number(p.price) || 0,
    display_speed: planDisplaySpeed(p) || String(p.speed || "").trim(),
    real_rate: planRateLimit(p) || "",
    download_speed: p.download_speed || "",
    upload_speed: p.upload_speed || "",
    router_profile: p.router_profile || "default",
    validity_mins: planDurationMins(p),
    validity_label: fmtPlanDuration(p),
    features: p.features || "",
    enabled: String(p.status || "active").toLowerCase() !== "inactive",
  };
}

function clientSpeedLabel(p) {
  const d = planDisplaySpeed(p);
  if (!d) return "WiFi access";
  if (/mbps|mb\/s/i.test(d)) return d;
  if (/[MmKkGg]$/.test(d)) return d;
  return d + " Mbps";
}

async function renewCustomerWithPlan(c, planId, payOpts = {}) {
  const plan = validatePrepaidPlanForCustomer(Plans.get(planId), c);
  const mins = planDurationMins(plan);
  const price = Number(plan.price) || 0;
  Customers.update(c.id, { plan_id: planId });
  Customers.setLastPaidPlan(c.id, planId);
  const now = await routerNow();
  const fresh = Customers.get(c.id);
  const base = (fresh.expiry && fresh.expiry >= now) ? fresh.expiry : now;
  const newExpiry = addMinutes(base, mins);
  Customers.setExpiry(c.id, newExpiry);
  Customers.setStatusAndAuto(c.id, "active", false);
  const inv = Invoices.addPrepaidRenewal({
    customer_id: c.id, plan_id: planId, amount: price,
    note: `Prepaid: ${plan.name} (${fmtPlanDuration(plan)})`,
  });
  if (payOpts.skipInvoicePay !== true) {
    Invoices.pay(inv.id, {
      amount: payOpts.amount != null ? payOpts.amount : price,
      method: payOpts.method || "cash",
      reference: payOpts.reference || "",
      note: payOpts.note || `Prepaid renewal — ${plan.name}`,
    });
  }
  const updated = Customers.get(c.id);
  await applyProfileSwitch(updated, "active");
  if ((updated.conn_type || "pppoe") === "ipoe" && updated.mac && updated.static_ip) {
    try { await ipoeProvision(updated); } catch (e) {
      Audit.add({ type: "auto", customer_id: c.id, customer_name: c.name, action: "ipoe-reprovision", detail: e.message, ok: false });
    }
  }
  return { expiry: newExpiry, plan: plan.name, planId: plan.id, invoiceId: inv.id, price };
}

// Reconnect + extend expiry by N minutes (from the later of current expiry / now).
async function renewCustomer(c, mins, planId = null) {
  if (planId) return (await renewCustomerWithPlan(c, planId, { method: "renewal", note: "admin/manual renew" })).expiry;
  const now = await routerNow();
  const base = (c.expiry && c.expiry >= now) ? c.expiry : now;
  const newExpiry = addMinutes(base, Number(mins) || (Number(c.plan_mins) || 43200));
  Customers.setExpiry(c.id, newExpiry);
  Customers.setStatusAndAuto(c.id, "active", false);
  await applyProfileSwitch(Customers.get(c.id), "active");
  return newExpiry;
}

// Remove a customer's presence from a SPECIFIC router connection (used when moving routers).
// Idempotent and tolerant: each delete is wrapped so a missing entry or unreachable router
// doesn't abort the whole move.
async function deprovisionOn(conn, c) {
  const isIpoe = (c.conn_type || "pppoe") === "ipoe";
  const isHotspot = (c.plan_type || "pppoe") === "hotspot";
  const errs = [];
  if (isIpoe) {
    const mac = normalizeMac(c.mac);
    if (mac) { try { await conn.leaseRemoveByMac(mac); } catch (e) { errs.push("lease: " + e.message); } }
    if (c.static_ip) {
      try { await conn.queueSimpleRemoveByName("ipoe-" + c.static_ip); } catch {}
      try { await conn.addrListRemove(ipoeSuspendList(), c.static_ip); } catch {}
    }
  } else if (isHotspot) {
    try { await conn.disconnectHotspot(c.username); } catch {}
    try { await conn.deleteHotspotUser(c.username); } catch (e) { errs.push("hotspot: " + e.message); }
  } else {
    try { await conn.disconnectPppoe(c.username); } catch {}
    try { await conn.deletePppoe(c.username); } catch (e) { errs.push("pppoe: " + e.message); }
  }
  return errs;
}

// Move a customer from their current router to a new one:
//   1) deprovision on the OLD router (so no ghost account is left behind)
//   2) update router_id
//   3) provision on the NEW router
// opts.force = proceed with the DB reassignment even if the old router is unreachable
// (the operator is warned to clean the old router by hand later).
async function moveCustomerToRouter(id, newRouterId, opts = {}) {
  const c = Customers.get(id);
  if (!c) throw new Error("Customer not found.");
  const oldRouterId = c.router_id || null;
  const target = newRouterId ? Routers.get(Number(newRouterId)) : Routers.getDefault();
  if (newRouterId && !target) throw new Error("Target router not found.");
  if (oldRouterId && target && oldRouterId === target.id) return { ok: true, unchanged: true, message: "Already on that router." };

  const result = { ok: true, oldRouterId, newRouterId: target ? target.id : null, oldErrors: [], newError: null, warned: false };

  // 1) deprovision on the old router (best-effort). If the old router can't be reached or the
  //    delete fails, that's a real problem — don't silently leave a ghost account. Require force.
  if (oldRouterId) {
    const oldR = Routers.get(oldRouterId);
    if (oldR) {
      let connErr = null;
      try {
        const oldConn = connForRouter(oldR);
        result.oldErrors = await deprovisionOn(oldConn, c);
      } catch (e) {
        connErr = e.message;
        result.oldErrors = ["unreachable: " + e.message];
      }
      // Treat connection/timeout errors during cleanup as "couldn't remove from old router".
      const cleanupFailed = connErr || result.oldErrors.some((x) => /reach|timed out|timeout|refused|ECONN|unreachable|network/i.test(x));
      if (cleanupFailed && !opts.force) {
        return { ok: false, error: `Could not remove the account from the current router "${oldR.name}" (${result.oldErrors.join("; ")}). ` +
          `Fix the connection, or use "Move anyway" — then delete the old account on "${oldR.name}" by hand so it isn't left behind.`, oldErrors: result.oldErrors };
      }
      if (cleanupFailed) result.warned = true;
    }
  }

  // 2) reassign in the database
  Customers.setRouter(id, target ? target.id : null);

  // 3) provision on the new router (best-effort; report if it fails so operator can retry)
  try {
    const fresh = Customers.get(id);
    const conn = connForCustomer(fresh);
    const isIpoe = (fresh.conn_type || "pppoe") === "ipoe";
    const isHotspot = (fresh.plan_type || "pppoe") === "hotspot";
    if (isIpoe) { await ipoeProvision(fresh); }
    else {
      const profile = (fresh.status === "suspended" ? suspendedProfileName() : (fresh.plan_profile || "default"));
      if (isHotspot) await conn.createHotspotUser({ name: fresh.username, password: fresh.password, profile });
      else await conn.createPppoe({ name: fresh.username, password: fresh.password, profile, comment: fresh.name });
    }
  } catch (e) {
    result.newError = e.message;
  }

  Audit.add({ type: "manual", customer_id: id, customer_name: c.name, action: "move-router",
    detail: `${oldRouterId ? "router #" + oldRouterId : "(none)"} → ${target ? target.name : "(default)"}` +
      (result.oldErrors.length ? " · old-cleanup issues: " + result.oldErrors.join("; ") : "") +
      (result.newError ? " · NEW PROVISION FAILED: " + result.newError : ""),
    ok: !result.newError });

  if (result.newError) return { ok: false, error: `Reassigned, but provisioning on "${target ? target.name : "default"}" failed: ${result.newError}. Use "Provision now" on the customer once the router is reachable.`, ...result };
  return { ...result, message: `Moved to ${target ? target.name : "default router"}.` + (result.warned ? " (Old router was unreachable — remove the old account there manually.)" : "") };
}
...
async function customerAction(id, action) {
  ...
  if (action === "provision") {
    // Identify which router this customer will be provisioned to, so any error names it clearly.
    let targetName = "the router";
    try {
      const tr = (c.router_id ? Routers.get(c.router_id) : null) || Routers.getDefault();
      if (tr) targetName = `"${tr.name}" (${tr.host || "?"})`;
    } catch {}
    try {
      if (isIpoe) { await ipoeProvision(c); }
      else {
        const conn = connForCustomer(c);   // ← provision on THIS customer's router
        const profile = c.plan_profile || "default";
        if (isHotspot) await conn.createHotspotUser({ name: c.username, password: c.password, profile });
        else await conn.createPppoe({ name: c.username, password: c.password, profile, comment: c.name });
      }
    } catch (e) {
      throw new Error(`Could not provision on ${targetName}: ${e.message}. Check Routers/Sites — the customer may be pointed at the wrong router, or that router's host/credentials are wrong (use the Test button).`);
    }
...
setAiConfigProvider(() => {
  const hasEnvKey = !!(process.env.GEMINI_API_KEY || process.env.OPENAI_API_KEY || process.env.ANTHROPIC_API_KEY);
  const enabled = Settings.get("ai_enabled", hasEnvKey ? "1" : "0") === "1";
  const provider = Settings.get("ai_provider", "") || (process.env.GEMINI_API_KEY ? "gemini" : process.env.OPENAI_API_KEY ? "openai" : process.env.ANTHROPIC_API_KEY ? "anthropic" : "gemini");
  const apiKey = Settings.get("ai_api_key", "") || process.env.GEMINI_API_KEY || process.env.OPENAI_API_KEY || process.env.ANTHROPIC_API_KEY || "";
  const model = Settings.get("ai_model", "") || (provider === "gemini"
  ...
  return { enabled, apiKey, model, provider, base };
});

refreshLicense();
console.log("  >> edition: free (no license required)");
...
server.listen(PORT, () => {
  ...
  console.log(" Login: ENABLED (roles: admin / cashier / technician / staff / helper)");
  ...
  const rpSecs = rescheduleRouterPoll();
  if (_rpTimer) { console.log(`  >> per-router live sync every ${rpSecs}s`); setTimeout(() => pollAllRouters().catch(() => {}), 6000); }
